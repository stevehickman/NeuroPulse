...
import com.neurone.app.NeurOneApplication
        ...
        ProtocolMenuScreen(
            ...
            onSelect = { entry ->
                ...
                val result = when (entry) {
                    is com.neurone.core.protocol.NPProtocolEntry.Single ->
                        app.protocolUploader.upload(entry.protocol)
                    else ->
                        com.neurone.app.session.ProtocolUploader.Result.Failure(
                            "Composite protocol upload is not yet supported.",
                        )
                }
                val message = when (result) {
                    is com.neurone.app.session.ProtocolUploader.Result.Success -> "Protocol sent to hub"
                    is com.neurone.app.session.ProtocolUploader.Result.Failure -> result.message
                }
