...
- [x] ISC-82: `NPLimitsStore` (core) — global/helmet/individual tiers + profiles + active context + `resolvedLimits` + `makeValidator()`; global tier persists as NPPS text via `KeyValueStore`; profiles + the active profile persist as JSON (2026-09-23 — the active profile names the device user, NP-SW-FAULTMSG-001); helmet/individual limit maps in-memory — OI-AND-LIMITS-01. Profile picker: `ProfilesScreen` (Settings). Unit-tested (`NPLimitsStoreTests`, 10/10: tier precedence, null-block, unlimited, active-profile resolution, NPPS persistence reload, validator enforcement, profile + active persistence, unknown-id / change notification, delete-active, corrupt record).
...
- 2026-09-08: **Consent dashboard built (`ConsentDashboardScreen.kt`) — closes OI-CONSENT-01.** The 5-tab scaffold's Privacy tab had been a blanket-consent toggle plus a portal link, so `grantClinicianAccess`, `revokeClinicianAccess`, `acceptInvitation`, `declineInvitation` and `withdrawFromStudy` were unit-tested in `core` and reachable from nothing — the ISC-45 lesson ("the gate must predate the surface it guards, or the surface ships ungated") in its other half: a tested decision whose surface never arrived. The screen ports iOS `ConsentDashboardView`; the five per-platform waivers in `scripts/check-consent-reachability.ts` are deleted, and that gate fails if a waiver outlives its gap, so they could not have been left behind. Two departures from iOS are recorded on the pending-decisions entry rather than absorbed silently (L3 stays a switch here; the grant form shows derived elements because iOS's use-case picker turns out to decide nothing — OI-CONSENT-04). **Not compiled here** — the authoring environment for this change had no Android SDK, so `.github/workflows/android-ci.yml` is the first compile; every `R.string` reference and `stringResource` argument count was checked against the generated `strings.xml` first.
- 2026-09-09: **§6.1's clinician access-expansion workflow built on both platforms — closes OI-CONSENT-02.** `expandClinicianAccess()` had been a public tier setter with no caller anywhere, and it was worse than unimplemented: `ClinicianConsentGrant.approvedElements` derives from `tier`, a tier is timeless, so setting it granted each newly added element over every session ever recorded — §6.1's retroactive decision taken silently and taken yes. `:core` gained `ClinicianAccessScope` (elements · effective-from · `includesPriorData`), `elementsVisibleForDataRecordedOn()`, `ClinicianAccessExpansionRequest` with a persisted sealed `ExpansionDecision`, and a new `ConsentEngine.kt` holding the differential document; the store gained `addExpansionRequest` / `approveExpansion` / `denyExpansion` / `askQuestionAboutExpansion`, and `expandClinicianAccess` is **private** — which is why it left the reachability gate's SURFACE. Day granularity throughout, matching the rest of these models. `ConsentDashboardScreen` gained the persistent request section and a two-step review screen: approving answers only the forward-looking question, and the retroactive one follows on its own step. Only the differential document is ported from iOS's `ConsentEngine`, not the use-case library — the grant form here derives elements from the tier, and iOS's picker decides nothing (OI-CONSENT-04). **`:core` compiled and its 23 tests run green here** by excluding `:app` locally, since this environment's network policy refuses `dl.google.com` and the Android Gradle Plugin cannot resolve; `:app` is uncompiled, so `android-ci.yml` is its first compile, and every `R.string` reference and `stringResource` argument count was checked against the canonical keys by hand.
...
- Compose screens still to build/replace: ~~Session~~ (ISC-56), ~~Protocol menu~~ (ISC-57), ~~Consumables~~ (ISC-61), ~~Consent onboarding L1–L4~~ (ISC-62), ~~Settings~~ (ISC-63), ~~OTA~~ (ISC-69), ~~Under-16~~ (ISC-70). ~~Setup/hardware wizard~~ (ISC-71–72), ~~Session History + Adaptive-Adjustments~~ (ISC-76–77), ~~Protocol editors (script + form + New/Edit/Delete)~~ (ISC-73–75). ~~Protocol composer (layer builder)~~ (ISC-80), ~~Research suggestion portal~~ (ISC-78–79). ~~Limits settings editor~~ (ISC-81–83), ~~deep per-modality parameter editor~~ (ISC-84), ~~Consent dashboard~~ (OI-CONSENT-01, 2026-09-08). **The Android app is now at full functional parity with iOS (minus Apple Watch).** *(That claim was overstated when first written: the consent dashboard was missing, and with it the only Android path to clinician-grant revocation and per-study withdrawal — OI-CONSENT-01, closed 2026-09-08.)* Only externally-blocked items remain deferred: on-device BLE (needs hardware, ISC-60), wire-JSON schema freeze across hub/iOS/Android (ISC-68), and the smaller OI items (Ed25519 <API33 fallback, helmet/individual limits persistence, host-side OTA download).
