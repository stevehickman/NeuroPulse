...
//
// UNITS — read before changing any number below (OI-CHARGE-05, 2026-09-09).
// `I(mA) × t(s) / A(cm²)` is **mC/cm²**, because mA × s = mC. So this check
// enforces 40 **mC**/cm², the clinically recognised human tDCS ceiling, and it
// said "µC/cm²" while doing so until 2026-09-09. The safety MCU separately
// enforces 40 µC/cm² for real. The two sides are therefore 1000× apart — a
// larger divergence than the 2.8× this file was written for — and which one is
// right is OI-CHARGE-05's decision, not something to settle by editing a test.
...
describe('tDCS charge-density pre-flight (OI-CHARGE-04)', () => {
  it('divides by ONE electrode area, not the sum across the montage', () => {
    // 1 mA × 1000 s = 1000 mC. On a 35 cm² pad that is 28.6 mC/cm² — under the
    // 40 mC/cm² ceiling. Summing the pair's two pads (70 cm²) would give
    // 14.3 mC/cm², which is also under; the pairs below are what separate the
    // two models.
    expect(chargeErrors(tdcsProtocol(
      { intensityMilliamps: 1, electrodeAreaCm2: 35, electrodePairs: [['F3', 'F4']] }, 1000,
    ))).toHaveLength(0);

    // 1 mA × 1500 s = 1500 mC → 42.9 mC/cm² on one 35 cm² pad: over.
    // The retired summed model gave 21.4 mC/cm² and accepted this.
    expect(chargeErrors(tdcsProtocol(
  ...
  it('is stricter on a smaller declared pad, which is the point of declaring it', () => {
    ...
    // regardless of what the app believed) is 40 mC/cm² at 1000 s — exactly at
  ...
  it('accepts the ceiling exactly (inclusive) and rejects just past it', () => {
    // 1.4 mA × 1000 s / 35 cm² = 40.0 mC/cm².
    expect(chargeErrors(tdcsProtocol(
  ...
  it('computes mC/cm², not µC/cm² — the dimension the label got wrong', () => {
    ...
    expect(chargeErrors(oneMilliCoulomb)).toHaveLength(0);   // 1 ≤ 40

    // 41 s of the same is 41 mC/cm² — just over. If the constant were ever
    // reinterpreted as µC/cm² without re-deriving it, this would fail at 0.041 s.
    expect(chargeErrors(tdcsProtocol(
      { intensityMilliamps: 1, electrodeAreaCm2: 1, electrodePairs: [['F3', 'F4']] }, 41),
    )).toHaveLength(1);
  });
