...
- **HealthKit data residency.** HealthKit data (HRV SDNN, heart rate) read only for real-time session display. Not persisted beyond the active session. Not transmitted to NeuroPulse servers or any analytics vendor. Per NP-APP-ROADMAP-001 Rev B §9.1.
...
- [ ] ISC-107: `OTAManager` fetches the latest firmware manifest from a NeuroPulse distribution endpoint when the hub connects and `OTAView` is opened.
- [ ] ISC-108: `OTAView` shows current firmware version (from hub GATT), latest available version, and a "Update" button — disabled if already at latest.
- [ ] ISC-109: `OTAManager.startUpdate()` sends the OTA `INITIATE` opcode, streams firmware chunks (≤512 bytes) via `sendOTACommand(.chunk, payload:)`, and sends `VERIFY` and `COMMIT` opcodes per the 9-step OTA sequence.
- [ ] ISC-110: `OTAView` shows a chunked progress bar with transferred bytes / total bytes during the update.
- [ ] ISC-111: If a hub disconnect occurs mid-OTA, `OTAManager` does not corrupt the active firmware bank — the bootloader's inactive-bank write architecture ensures the running bank is untouched.
- [ ] ISC-112: After a successful OTA commit, `OTAView` shows "Update complete — hub will restart" and waits for reconnect before resuming normal operation.
- [ ] ISC-113: `Anti:` `OTAManager` does not send unsigned firmware chunks — every chunk upload uses the GATT write path which the hub-side verifies via Ed25519 before flashing.
...
|-----|------|-------|-----------|------|
| ISC-1 | build | `xcodebuild build -scheme NeuroPulse -configuration Release` | 0 errors, 0 warnings | Xcode CLI |
| ISC-2 | build | `xcodebuild build -scheme NeuroPulse -destination 'platform=iOS Simulator'` | 0 errors | Xcode CLI |
...
| ISC-85 | grep | `grep -n 'ageConfirmed = true' app/ios/NeuroPulse` | 0 matches | grep |
| ISC-92 | grep | `grep -rn 'Analytics\|Crashlytics\|configure()' app/ios/NeuroPulse --include="*.swift"` | all calls are inside `if consentShown` guard | grep |
| ISC-93 | grep | `grep -rn 'session_count\|sessionCount' app/ios/NeuroPulse --include="*.swift"` | 0 analytics event property matches | grep |
| ISC-96 | grep | `grep -rn 'HKHealthStore.*save\|HKHealthStore.*add' app/ios/NeuroPulse --include="*.swift"` | 0 matches | grep |
...
| gatt-stop-characteristic | Add stop-command GATT write characteristic to GATTCharacteristics.swift and NeuroPulseGATTManager | ISC-27 | — | true |
...
- **2026-06-08**: Consumable tracker testability refactor (ISC-154). `ConsumableTracker` was untestable due to a hard `NeuroPulseGATTManager` dependency. Resolved by extracting `ConsumableCountsProviding` protocol with `NeuroPulseGATTManager` extension conformance; test target uses `MockCountsProvider` (`CurrentValueSubject`-backed). Two model bugs fixed: (1) `intranasalSleeves.lowThreshold` was 5 (making single-use sleeve always `isLow` even when fresh; corrected to 0 — alert only when session count ≥ sessionLimit); (2) `activeReminders` had incorrect operator precedence in filter expression (simplified to `states.filter { $0.isLow }`). Critical init-order bug fixed: `loadPersistedSnooze()` must run before `observeCounts()` — the `CurrentValueSubject` / `@Published` publisher delivers its current value synchronously on subscription, which calls `persistSnooze()` with all-zero snooze counts, overwriting persisted state before `loadPersistedSnooze()` had a chance to run. Reordered to `loadPersistedSnooze() → observeCounts()`. 23 consumable tests (7 model + 16 tracker), all passing. Privacy analysis: NP-PRIV-ANALYSIS-003 Rev A — 0 Critical/High/Medium, 2 Low (lock-screen notification content, notification permission timing). Both Low findings resolved (Rev B, 2026-06-08): notification `title` changed to generic `"NeuroPulse"` with `subtitle` for lock-screen privacy; `requestNotificationPermission()` removed from `init()` and deferred to `ConsumableView.onAppear` via `requestNotificationPermissionIfNeeded()`. See `docs/neuropulse_priv_analysis_consumable_tracker.md`.
...
| ISC-154 | `ConsumableInventoryTests` (7 tests) + `ConsumableTrackerDirectTests` (16 tests) = 23 tests; 0 failures. Covers: blocking threshold detection (`testSessionBlockedWhenIntranasalExceeded`), non-blocking threshold (`testSessionNotBlockedWhenOnlyHydrogelLow`), snooze cap enforcement (`testSnoozeCapRespectedByTracker_performanceCritical`, `testTrackerSafetyBlockingCannotBeSnooze`), reactive count updates (`testInventoryUpdatesWhenProviderPushesNewCounts`), bounds guards (`testSnoozeOutOfBoundsIndexIsNoOp`, `testMarkReplacedOutOfBoundsIndexIsNoOp`). Privacy analysis NP-PRIV-ANALYSIS-003 Rev B: 0 Critical/High/Medium, 2 Low — all resolved. LOW-1: generic lock-screen notification title (`ConsumableTracker.scheduleLocalNotification`). LOW-2: permission deferred to `ConsumableView.onAppear`. | 2026-06-08 |
