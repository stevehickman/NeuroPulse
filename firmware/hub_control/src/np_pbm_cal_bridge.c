...
typedef char _np_cal_bridge_uid_len_match[
    (NP_PBM_MODULE_UID_LEN == NP_HEXMAP_UID_LEN) ? 1 : -1];
typedef char _np_cal_bridge_uid_size_match[
    (sizeof(np_pbm_module_uid_t) == sizeof(np_module_uid_t)) ? 1 : -1];

/*
 * The calibration payload is a flat float[9] on the map side and an array of
 * np_pbm_cal_t on the PBM side. Ordering is 3*wl + coeff with coeff in
 * {K_PD1, K_PD2, K_ratio_nom} — stated in np_module_map.h and mirrored here.
 */
typedef char _np_cal_bridge_payload_shape[
    (NP_HEXMAP_CAL_FLOATS == (NP_PBM_WL_COUNT * 3u)) ? 1 : -1];

static bool bridge_cal_provider(const np_pbm_module_uid_t *uid,
                              ...
                              void *ctx)
{
    ...
    memcpy(uid_out->b, map_uid.b, NP_PBM_MODULE_UID_LEN);
    return true;
}

void np_hub_pbm_cal_bridge_install(void)
{
    np_pbm_dose_set_cal_provider(bridge_cal_provider, NULL);
    np_pbm_session_set_uid_resolver(bridge_socket_uid, NULL);
}
