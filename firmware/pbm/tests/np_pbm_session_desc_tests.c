 ...
 * src/np_pbm_hal.c. Build with -DNP_BUILD_TESTS=ON
 * (see firmware/pbm/CMakeLists.txt).
 */
...
static np_pbm_preset_t make_preset(uint8_t cur_a, uint8_t freq_hz, uint8_t duty)
{
    np_pbm_preset_t p;
    memset(&p, 0, sizeof(p));
    p.cur_a = cur_a;
    ...
    p.freq_hz = freq_hz;
    p.duty = duty;
    p.channel_mask = NP_PBM_CH_ALL_EN;
    return p;
}
...
{
    check(sizeof(np_pbm_session_desc_hdr_t) == 8U, "hdr is 8 bytes");
    check(sizeof(np_pbm_preset_t) == 6U, "preset is 6 bytes");
    check(sizeof(np_pbm_preset_group_t) == NP_PBM_SOCKET_MASK_BYTES + 6U,
...
{
    size_t hdr = sizeof(np_pbm_session_desc_hdr_t);
    size_t grp = sizeof(np_pbm_preset_group_t);

    check(np_pbm_session_desc_wire_len(0U) == hdr + 64U,
          "wire_len(0 groups) == header + signature only");
    check(np_pbm_session_desc_wire_len(1U) == hdr + grp + 64U,
          "wire_len(1 group) == header + 1 group + signature");
    check(np_pbm_session_desc_wire_len(NP_PBM_SESSION_MAX_PRESET_GROUPS) ==
              hdr + (size_t)NP_PBM_SESSION_MAX_PRESET_GROUPS * grp + 64U,
          "wire_len(max groups) == header + max groups + signature");
    check(np_pbm_session_desc_wire_len(NP_PBM_SESSION_MAX_PRESET_GROUPS + 1U) == 0U,
    ...
    check(np_pbm_session_desc_signed_len(1U) == np_pbm_session_desc_wire_len(1U) - 64U,
          "signed_len(1) == wire_len(1) - signature");
    check(np_pbm_session_desc_signed_len(1U) < np_pbm_session_desc_signed_len(
              NP_PBM_SESSION_MAX_PRESET_GROUPS),
...
static void test_serialize_parse_roundtrip(void)
{
    ...
    uint8_t buf[512];
    size_t written = np_pbm_session_desc_serialize(&desc, buf, sizeof(buf));
    check(written == np_pbm_session_desc_wire_len(2U), "serialize writes exact wire_len");

    np_pbm_session_desc_t parsed;
    np_pbm_status_t rc = np_pbm_session_desc_parse(buf, written, &parsed);
    check(rc == NP_PBM_OK, "parse accepts a well-formed blob");
    ...
    check(memcmp(parsed.groups[0].mask, desc.groups[0].mask, NP_PBM_SOCKET_MASK_BYTES) == 0,
    ...
    uint8_t zero_mask[NP_PBM_SOCKET_MASK_BYTES];
...
static void test_parse_rejects_bad_input(void)
{
    ...
    uint8_t buf[512];
    size_t wire_len = np_pbm_session_desc_serialize(&desc, buf, sizeof(buf));

    np_pbm_session_desc_t parsed;

    check(np_pbm_session_desc_parse(buf, wire_len, &parsed) == NP_PBM_OK,
          "sanity: well-formed 1-group blob parses");

    check(np_pbm_session_desc_parse(buf, wire_len - 1U, &parsed) == NP_PBM_ERR_INVALID_ARG,
    ...
    bad_count_buf[2] = NP_PBM_SESSION_MAX_PRESET_GROUPS + 1U; /* hdr.group_count offset */
    check(np_pbm_session_desc_parse(bad_count_buf, wire_len, &parsed) == NP_PBM_ERR_INVALID_ARG,
          "parse rejects group_count beyond NP_PBM_SESSION_MAX_PRESET_GROUPS");

    check(np_pbm_session_desc_parse(NULL, wire_len, &parsed) == NP_PBM_ERR_INVALID_ARG,
...
static void test_expand_basic(void)
{
    ...
    uint8_t ids[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS];
    np_pbm_preset_t presets[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS];
    uint8_t count = 0U;

    np_pbm_status_t rc = np_pbm_session_desc_expand(
        &desc, ids, presets, &count, NP_PBM_SESSION_MAX_ACTIVE_SOCKETS);

    check(rc == NP_PBM_OK, "expand succeeds within capacity");
...
static void test_expand_dedup_first_group_wins(void)
{
    ...
    check(rc == NP_PBM_OK, "expand succeeds on overlapping masks");
    check(count == 1U,
...
static void test_expand_overflow_fails_closed(void)
{
    ...
    uint8_t ids[4];
    np_pbm_preset_t presets[4];
    uint8_t count = 0xAAU; /* sentinel to confirm it gets overwritten to 0 */

    np_pbm_status_t rc = np_pbm_session_desc_expand(&desc, ids, presets, &count, 4U);

    check(rc == NP_PBM_ERR_INVALID_ARG,
...
static void test_session_start_beyond_old_domain(void)
{
    ...
    np_pbm_status_t rc = np_pbm_session_start(&ctx, &desc);
    check(rc == NP_PBM_OK, "session_start accepts a 2-group split-zone v5 descriptor");
    ...
    check(ctx.stage == NP_PBM_STAGE_RAMP_UP, "session reaches RAMP_UP");
}
...
static void test_session_start_rejects_over_capacity(void)
{
    ...
    check(rc == NP_PBM_ERR_INVALID_ARG,
          ...
          "addresses more sockets than NP_PBM_SESSION_MAX_ACTIVE_SOCKETS");
    check(ctx.stage == NP_PBM_STAGE_IDLE,
...
static void test_t2_combined_wire_and_start(void)
{
    ...
    size_t wire_len = np_pbm_t2_combined_desc_wire_len(1U);
    check(wire_len == 4U + sizeof(np_pbm_session_desc_hdr_t) +
                          sizeof(np_pbm_preset_group_t) +
                          sizeof(np_t2_1170_preset_t) + 64U,
          "t2 wire_len matches the documented layout");

    uint8_t buf[512];
    size_t written = np_pbm_t2_combined_desc_serialize(&desc, buf, sizeof(buf));
    check(written == wire_len, "t2 serialize writes exactly wire_len bytes");

    np_t2_combined_desc_t parsed;
    check(np_pbm_t2_combined_desc_parse(buf, written, &parsed) == NP_PBM_OK,
...
{
    check(np_pbm_fai_sm09().passed, "FAI-SM-09 passes against the rewritten session ctx");
    check(np_pbm_fai_sm10().passed, "FAI-SM-10 passes against the v5 descriptor shape");
    check(np_pbm_fai_sm11().passed, "FAI-SM-11 passes against the per-socket UHDR/SHDR shape");
    check(np_pbm_fai_t2_01().passed, "FAI-T2-01 passes against the inlined T2 descriptor");
    check(np_pbm_fai_t2_02().passed, "FAI-T2-02 passes against the rewritten throttle cascade");
    check(np_pbm_fai_t2_03().passed, "FAI-T2-03 passes against the per-socket combined UHDR/SHDR");
    check(np_pbm_fai_t2_04().passed, "FAI-T2-04 passes against the rewritten abort path");
}
...
static np_pbm_module_uid_t uid_seed(uint8_t seed)
{
    np_pbm_module_uid_t u;
    memset(&u, 0, sizeof(u));
    u.b[0] = seed;
    return u;
}
...
static np_pbm_module_uid_t g_known_uid;
static float                   g_known_k_pd1 = 0.777f;
static int                     g_provider_calls;
static int                     g_provider_partial_writes;

static bool test_cal_provider(const np_pbm_module_uid_t *uid,
                              np_pbm_cal_t cal_out[NP_PBM_WL_COUNT],
                              void *ctx)
{
    (void)ctx;
    g_provider_calls++;
    if (memcmp(uid->b, g_known_uid.b, NP_PBM_MODULE_UID_LEN) != 0) {
        /* Deliberately scribble before failing: np_pbm_dose_load_cal() must
...
{
    np_pbm_dose_set_cal_provider(NULL, NULL);
    np_pbm_session_set_uid_resolver(NULL, NULL);
    g_provider_calls          = 0;
...
static void test_cal_load_fails_closed(void)
{
    ...
    check(np_pbm_dose_load_cal(NULL, cal) == NP_CAL_DEFAULT,
    ...
    np_pbm_module_uid_t some = uid_seed(0x42);
    memset(cal, 0, sizeof(cal));
    check(np_pbm_dose_load_cal(&some, cal) == NP_CAL_DEFAULT,
    ...
    np_pbm_dose_set_cal_provider(test_cal_provider, NULL);
    g_provider_calls = 0;
    np_pbm_module_uid_t zero;
    memset(&zero, 0, sizeof(zero));
    check(np_pbm_dose_load_cal(&zero, cal) == NP_CAL_DEFAULT,
          "load_cal: zero uid → NP_CAL_DEFAULT");
    check(g_provider_calls == 0, "load_cal: zero uid short-circuits before the provider");

    check(np_pbm_module_uid_is_zero(&zero), "uid_is_zero: true for all-zero");
    check(!np_pbm_module_uid_is_zero(&some), "uid_is_zero: false for a real uid");
...
static void test_cal_load_provider_hit_and_miss(void)
{
    ...
    /* Hit. */
    memset(cal, 0, sizeof(cal));
    np_pbm_module_uid_t known = uid_seed(0xAB);
    check(np_pbm_dose_load_cal(&known, cal) == NP_CAL_FACTORY,
    ...
    memset(cal, 0, sizeof(cal));
    np_pbm_module_uid_t stranger = uid_seed(0xCD);
    check(np_pbm_dose_load_cal(&stranger, cal) == NP_CAL_DEFAULT,
...
static bool test_uid_resolver(uint8_t socket_id, np_pbm_module_uid_t *uid_out, void *ctx)
{
    (void)ctx;
...
static void start_two_socket_session(np_pbm_session_ctx_t *ctx)
{
    np_pbm_session_desc_t desc;
    memset(&desc, 0, sizeof(desc));
    desc.hdr.version     = NP_SES1064_VERSION;
...
static void test_session_cal_source_default_without_resolver(void)
{
    ...
    np_pbm_session_build_shdr_summary(&ctx, NP_PBM_FAULT_NONE, &shdr);
...
static void test_session_cal_source_varies_per_socket(void)
{
    ...
    g_known_uid = uid_seed(0xAB);
    np_pbm_dose_set_cal_provider(test_cal_provider, NULL);
    np_pbm_session_set_uid_resolver(test_uid_resolver, NULL);
