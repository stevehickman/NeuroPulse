...
{
    if (counts >= NP_PBM_ADC_NO_MODULE_MIN) {
        return NP_SLOT_ABSENT;
    }
    if (counts <= NP_PBM_ADC_SMART_MAX) {
...
static np_slot_type_t debounce_majority(const np_sm_slot_ctx_t *s)
{
    ...
    for (uint8_t i = 0; i < NP_PBM_DEBOUNCE_READS; i++) {
        np_slot_type_t t = np_pbm_classify_adc(s->adc_reads[i]);
        if ((uint8_t)t < 3U) {
            tally[t]++;
        }
    }
    ...
    for (uint8_t t = 0; t < 3U; t++) {
        if (tally[t] >= NP_PBM_DEBOUNCE_MAJORITY) {
                              ...
                              uint32_t                 device_session_count)
{
     ...
     */
    for (uint8_t i = 0; i < NP_PBM_ZONE_COUNT; i++) {
                              ...
                              void (*insert_cb)(uint8_t slot))
{
    ...
    for (uint8_t slot = 0; slot < NP_PBM_ZONE_COUNT; slot++) {
        ...
        case NP_SM_IDLE: {
            ...
            np_slot_type_t t = np_pbm_classify_adc(counts);
            if (t == NP_SLOT_ABSENT) {
                break;
            }
            /* Insertion detected — begin settle delay. */
            s->settle_until_ms = now_ms + NP_PBM_INSERTION_SETTLE_MS;
        ...
        case NP_SM_DEBOUNCING: {
            ...
            s->next_read_ms = now_ms + NP_PBM_DEBOUNCE_INTERVAL_MS;

            if (s->read_count < NP_PBM_DEBOUNCE_READS) {
                break;
            }
            ...
            if (majority == NP_SLOT_BASE_MODULE) {
                 ...
                 */
                np_pbm_hal_tia_gain_set(slot, NP_TIA_GAIN_LOW);
                s->tia_gain = NP_TIA_GAIN_LOW;
                np_pbm_hal_i2c_mux_enable(slot, true);
                s->i2c_probed = false;
        ...
        case NP_SM_I2C_PROBING: {
            bool ack = np_pbm_hal_i2c_probe(slot, NP_PBM_I2C_ADDR,
            ...
            if (!ack) {
                ...
                np_pbm_shdr_fault_entry_t fe = {
                    ...
                    .socket_id            = slot,
                    .channel              = 0xFF,
                    .fault_reason         = NP_PBM_FAULT_I2C_LOST,
                    .status_reg_value     = 0xFF,
                };
                np_pbm_hal_shdr_log_fault(&fe);
                np_pbm_hal_i2c_mux_enable(slot, false);
                np_pbm_hal_tia_gain_set(slot, NP_TIA_GAIN_HIGH);
        ...
        case NP_SM_ACTIVE: {
            /* Poll for removal every NP_PBM_POLL_ACTIVE_MS. */
            if (now_ms < s->next_read_ms) {
                break;
            }
            s->next_read_ms = now_ms + NP_PBM_POLL_ACTIVE_MS;

            uint16_t counts = 0;
            if (!np_pbm_hal_adc_read_zone_id(slot, &counts)) {
                break;
            }
            if (np_pbm_classify_adc(counts) == NP_SLOT_ABSENT) {
                if (!s->removal_debounce_active) {
                    s->removal_debounce_active = true;
                    s->removal_since_ms = now_ms;
                }
                if (now_ms - s->removal_since_ms >= NP_PBM_REMOVAL_DEBOUNCE_MS) {
        ...
        case NP_SM_REMOVING: {
            ...
            uint16_t counts = 0;
            np_pbm_hal_adc_read_zone_id(slot, &counts);
            if (np_pbm_classify_adc(counts) != NP_SLOT_ABSENT) {
