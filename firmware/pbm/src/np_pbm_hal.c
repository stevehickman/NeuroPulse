 ...
 *
 * OI-PBM-01: np_pbm_hal_adc_read_pd    — STUB (platform team: wire to LPADC1)
 * OI-PBM-02: np_pbm_hal_i2c_write/_read — STUB (platform team: wire to LPI2C3)
 * OI-PBM-03: np_pbm_hal_safety_mcu_enable — STUB (platform team: wire to SPI/GPIO)
 *
 ...
 *   - np_pbm_hal_adc_read_zone_id: returns 1016 (smart module, slot 0) or
 *     ADC values matching ZM-01–05 for slots 1–5 (base modules).
 *   - np_pbm_hal_adc_read_pd: returns nominal PD counts for 100 mW/cm².
 *   - np_pbm_hal_i2c_write/_read: returns NP_PBM_OK (no-op).
 *   - np_pbm_hal_i2c_probe: returns true (ACK simulated).
 *   - np_pbm_hal_safety_mcu_enable: returns NP_PBM_OK.
 *
                                ...
                                uint32_t timeout_ms)
{
    ...
    return (slot == 0U && i2c_addr == NP_PBM_I2C_ADDR);
}
...
{
    (void)slot;
    if (!temp_c_out) { return NP_PBM_ERR_INVALID_ARG; }
...
{
    s_stub_now_ms += NP_PBM_DOSE_TICK_MS;
...
np_pbm_status_t np_pbm_hal_t2_1170_get_dose(float *dose_J_cm2_out)
{
     ...
     */
    if (!dose_J_cm2_out) { return NP_PBM_ERR_INVALID_ARG; }
...
np_pbm_status_t np_pbm_hal_t2_1170_get_temp(float *tec_temp_c_out)
{
    ...
    if (!tec_temp_c_out) { return NP_PBM_ERR_INVALID_ARG; }
...
{
    if (slot >= 5U) { return NP_PBM_ERR_INVALID_ARG; }
     ...
     * After asserting HIGH, wait NP_PBM_TIA_GAIN_SETTLE_US before returning
