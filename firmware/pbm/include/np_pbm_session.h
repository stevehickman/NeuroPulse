...
#ifndef NP_PBM_SESSION_H
#define NP_PBM_SESSION_H
 ...
 * list (see np_pbm_session_desc_expand()) — NOT array index == socket
...
typedef struct {
     ...
     */
    np_pbm_module_uid_t active_uid[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS];
    uint8_t              active_cal_source[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS];

    np_pbm_drv_slot_t     drv[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS];
    np_pbm_dose_state_t   dose[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS];
    np_pbm_cal_t          cal[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS][NP_PBM_WL_COUNT];
    np_pbm_session_record_t record;
    ...
    uint8_t  eeg_tick_in_band[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS]; /* hysteresis counter */
    uint8_t  current_freq_code;      /* uniform-mode shared freq code             */

    /* NTC throttle state */
    float    ntc_temp_c[NP_PBM_SESSION_MAX_ACTIVE_SOCKETS];
    bool     ch_c_throttled;
    bool     ch_b_throttled;
 ...
 */
typedef bool (*np_pbm_socket_uid_fn)(uint8_t                  socket_id,
                                         np_pbm_module_uid_t *uid_out,
                                         void                    *ctx);
 ...
 *         NP_PBM_ERR_INVALID_ARG if the descriptor's groups address more
 *         than NP_PBM_SESSION_MAX_ACTIVE_SOCKETS distinct sockets.
 */
...
/*
 * Session tick — call at NP_PBM_DOSE_TICK_MS (100 ms) intervals.
 * Drives ramp, dose metering, I2C status poll, NTC poll, EEG-adaptive.
 * Returns NP_PBM_OK during normal operation;
 *         NP_PBM_ERR_NO_SESSION if no session is active.
 */
 ...
 * hdr.group_count * sizeof(np_pbm_preset_group_t) — never the full
 * NP_PBM_SESSION_MAX_PRESET_GROUPS capacity. See the struct-level
 * comment on np_pbm_session_desc_t in np_pbm_types.h for why that
 ...
 * Returns 0 if group_count > NP_PBM_SESSION_MAX_PRESET_GROUPS.
 */
 ...
 * `out`. out_cap must be >= np_pbm_session_desc_wire_len(desc->hdr.group_count).
 ...
 * parsed group_count. Validates group_count <= NP_PBM_SESSION_MAX_PRESET_GROUPS
 * and that in_len exactly equals np_pbm_session_desc_wire_len(group_count)
 ...
 * over the first np_pbm_session_desc_signed_len(group_count) bytes of
 ...
 *
 * `capacity` bounds both output arrays. Returns NP_PBM_ERR_INVALID_ARG
