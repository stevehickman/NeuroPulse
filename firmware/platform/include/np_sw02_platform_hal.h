 ...
 *
 * 97 → 98 on 2026-09-23 (NP-FW-HUB-001 Rev 2, OI-FWHUB-01):
 * np_mod_pbm_hal_socket_pwm_set(), the cluster-controller PWM seam for base
...
/* Socket-indexed successor of np_mod_pbm_hal_pwm_set() for base (T1-A) tiles,
 * which have no on-module MCU: the PWM is generated by the socket's cluster
 * controller (NP-HW-HUB-001 Rev 4 §3.2, §9.2), and this seam resolves
 * socket_id → (cluster_id, channel) itself so no caller learns the topology.
 * socket_id is INDEX space (0-based). All-zero arguments = dark.
 * OI-PBM-HAL-04 (np_socket_dispatch.c via np_mod_pbm_socket_drive). */
