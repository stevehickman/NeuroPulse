...
A socket not claimed by any currently-installed zone shows as uninstalled, even if it was previously lit by a zone that's since been toggled off.

**Example:** turn on "Frontal Left" (660 nm, 10 Hz), then turn on "Motor / SMA" (1064 nm, 2 Hz). Sockets exclusive to Frontal Left show 660 nm/10 Hz; sockets exclusive to Motor / SMA show 1064 nm/2 Hz; the sockets both zones share (26–29) show Motor / SMA's setting, because it was selected last. Now reconfigure "Frontal Left" again (e.g. switch it to 808 nm) — the shared sockets switch to Frontal Left's new setting, since Frontal Left is now the more recently selected of the two. Compare this to turning on "Frontal" (which contains "Motor / SMA" as a proper subset) instead of "Frontal Left": no matter which order you select them in, "Motor / SMA" always keeps its own setting for its own sockets.
...
*NP-SIM-001-HOWTO v2.5 — NeurOne Helmet Simulator User Manual — 2026-07-29*
...
*v2.5: removed the floating zone-name label overlay (redundant with the left panel and
cluttered under the real, overlapping named zones); fixed PBM glow to key off each socket's
own installed/frequency state instead of matching zone names against the loaded protocol's
declared zones — manually toggling "All" (or any zone) now actually lights those sockets,
and each zone's own frequency dropdown (including CW) now genuinely drives that zone's
sockets instead of being ignored in favour of a single protocol-wide frequency; documented
and implemented the overlapping-zone resolution rule (§5.1 "Overlapping Zones"); shell
exterior is now white.*
