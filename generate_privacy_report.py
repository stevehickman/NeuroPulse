...
def build_content(st):
    ...
    story += finding_block(st,
        ...
        "\n\n"
        "CONCLUSION (principal decision, 2026-08-12 — NP-FW-EMMC-002 Rev 2 §H). "
        "Option C remains the destination; it is not superseded. It is, however, unreachable "
        "as written, and the reason is a defect in this finding's own remediation rather than "
        "in the finding. Option C says 'process on-device' — but the two thresholds that "
        "define the processing (NP_ACCEL_DROP_THRESHOLD_G = 15.0f; the maintenance rate) are "
        "unvalidated guesses made before any hardware existed, and the Option B/C prohibition "
        "forbids collecting the very data that would validate them. Nobody has established "
        "what impact profile predicts device failure. The remediation therefore forecloses the "
        "evidence needed to make the remediation correct. "
        "\n\n"
        "The resolution is a bounded, self-closing characterisation programme that obtains "
        "that evidence and then retires itself, on the pattern NP-MOD-ID-001 §7 set for the "
        "DOSE-01 relaxation. During a time-boxed window an enrolled device additionally emits "
        "a COARSENED per-gap histogram of impact peaks — counts per fixed bin, never a raw "
        "series, never a per-event g value, never an orientation vector, never a per-event "
        "timestamp. Collection is opt-in and revocable, consented by the WARRANTY OWNER, and "
        "purpose-bound to predictive-maintenance model training only: not to research, not to "
        "warranty adjudication, not to any per-device determination affecting an owner. At "
        "window close the extended rows are DELETED from the fleet database — the fitted model "
        "survives, the evidence does not. "
        "\n\n"
        "The window is denominated in RECORDS, not calendar time, because this device has no "
        "clock that survives being unplugged: it is USB-C powered with no battery or coin "
        "cell, so the SNVS RTC has no backup domain and wall time is re-supplied by the phone. "
        "A calendar expiry would be both losable and settable backwards. A per-device record "
        "budget is monotone by construction, needs no clock, and is the quantity the review "
        "gate actually needs — so exhausting the budget and answering the question are the "
        "same act. Expiry is enforced fail-closed in firmware by a build-time constant, and "
        "CI enforces the biconditional in both directions: a closed window whose schema still "
        "carries the extended columns FAILS, so the data cannot outlive the programme. "
        "\n\n"
        "WHY REGISTRANT CONSENT SUFFICES. Per NP-MOD-ID-001 §7.5.1, 'the operative distinction "
        "is between describing a person and being attributable to one.' SHDR holds no user "
        "identifier; the warranty database — which SHDR cannot join — names the REGISTRANT, "
        "not the wearer; and nothing in either store records the relationship between them. "
        "Consent is owed to an identifiable subject, and there is none in any device "
        "configuration, so the registrant may consent in all of them. "
        "\n\n"
        "AND THE ARGUMENT IS STRONGER HERE THAN IN THE CASE ALREADY DECIDED. The module "
        "duty-map programme accepted registrant consent for data that at least requires the "
        "device to be WORN — a duty map is a record of a wearing session. A DROP DOES NOT. "
        "The headset can be dropped by a family member moving it off a table, by clinic staff "
        "cleaning it, by a courier, by anyone handling it. So on the 'describes a person' limb "
        "— the limb that made the duty-map case arguable at all — impact data is weaker, not "
        "merely equal: an impact histogram is frequently not a record of the wearer's "
        "behaviour, and nothing in it marks which entries are. The precedent is followed a "
        "fortiori, not merely cited. "
        "\n\n"
        "SEPARATELY, AND NOT CREATED BY THIS DECISION: the Option A/B prohibition is already "
        "defeated by aggregation. shdr_accel_records is append-per-gap with a non-unique "
        "warranty_token, so a COUNT of drop_detected rows per token IS the 'drop count as an "
        "integer' the schema bans by name, and the gap_index values where it is true ARE the "
        "inter-drop timing that banning per-event timestamps was meant to prevent. A "
        "column-level control cannot express a row-set-level hazard. Recorded as OI-EMMC2-11 "
        "for principal decision; it is stated here so that this finding's remediation is not "
