...
function reportCeilings() {
  ...
  console.log();
  console.log("  NP-ENV-OPRANGE-001 §2 (decided, principal, 2026-08-31) blocks PBM T1-A above");
  console.log("  +35 C ambient and allows full dose only <= +30 C, with a 1.0 C hysteresis and a");
  console.log("  per-protocol efficacy-floor clamp landing the effective block at 33.8/34.2/34.6 C.");
  console.log("  So the 35 C rows are ALREADY the block threshold and the 43.3 C rows are far");
  console.log("  outside the operating envelope -- they are a comparison against R1's worst case,");
  console.log("  not an operating condition. The 25 C rows are the design case.");
}


/** Highest ambient holding max face <= 42 C, by N. This is the quantity
 *  NP-ENV-OPRANGE-001 §2's hard edges are denominated in, so it is the one
 *  directly comparable to the +30 / +35 band. */
function reportMaxAmbient(w: number) {
  rule(`§4b  Max ambient holding face <= ${FACE_LIMIT} C, at ${w} W/tile`);
  console.log("  NP-ENV-OPRANGE-001 §2: full dose <= +30 C, derate +30->+35, block > +35 C");
  console.log("  (decided 2026-08-31, principal); effective per-protocol block 33.8/34.2/34.6 C.");
  console.log();
  console.log("   N   perfect   0.25   0.50   1.00   2.00  K/W      vs the +35 block");
  for (const n of [1, 6, 12, 20, 80]) {
    const cells = [null, 0.25, 0.5, 1.0, 2.0].map((rs) => {
      let lo = -10, hi = 60;
      for (let k = 0; k < 60; k++) {
        const mid = (lo + hi) / 2;
        const o: Opts = rs === null ? { amb: mid, perfectSink: true } : { amb: mid, rSink: rs };
        if (maxFace(steady(drive(distributed(n), w), o)) <= FACE_LIMIT) lo = mid; else hi = mid;
      }
      return lo;
    });
    const at05 = cells[2];
    const verdict = at05 >= 35 ? "clears" : `${(35 - at05).toFixed(1)} K SHORT`;
    console.log(
      `  ${String(n).padStart(2)}  ${cells.map((c) => f1(c).padStart(6)).join(" ")}      ${verdict}`);
  }
  console.log();
  console.log(`  Read the 0.50 K/W column against the +35 C block. The envelope the principal set`);
  console.log(`  on efficacy-floor and T2-alignment grounds is close to what this model permits`);
  console.log(`  thermally at a realistic heatsink -- two independent routes to the same number.`);
