...
// Android's counterpart to SocketLattice.generated.swift. Both carry the
// physical lattice ONLY — socket count and numbering. Neither carries zone
// membership: "which sockets is 'Frontal Left'?" is answered from the loaded
// .npps namespace via NPZoneRegistry on both platforms, because a zone is
...
// ─── Lattice emitters (NO zone content) ───────────────────────────────────────
//
// These emit the PHYSICAL LATTICE only — how many sockets exist and how they are
// numbered. They deliberately emit NO zone membership.
//
// Earlier revisions emitted `SocketZones.generated.{swift,kt}`: `00-zones.npps`
// inverted into Swift and Kotlin source as `bySocket` / `byZone` / `zoneNames`,
// which the apps then consulted at runtime instead of the file.
//
// That did NOT violate NP-NPPS-REF-001 §8's origin rule — the content was
// derived from `00-zones.npps` and regenerated with it, which §8 allows. It went
// for a different reason: generation happens at BUILD time, so the table could
// only ever hold the SHIPPED zones. A zone a user defines in their own `.npps`
// file (§8 *User-defined zones*) was simply absent from it, and so did not
// resolve on either mobile platform. Both apps now answer every zone question
// from the loaded namespace through `NPZoneRegistry`, exactly as web already
// did, which covers shipped and user zones on one path.
//
// Socket count and numbering stay here because they are hardware, not zones.

function emitKotlinLattice(sockets: SocketGeometry[]): string {
  const min = sockets.length === 0 ? 0 : Math.min(...sockets.map(s => s.id));
  const max = sockets.length === 0 ? 0 : Math.max(...sockets.map(s => s.id));
  return [
    "// Generated by scripts/sync-socket-map.ts — DO NOT EDIT BY HAND.",
    "// Regenerate with: bun scripts/sync-socket-map.ts",
    "//",
...
function emitSwiftLattice(sockets: SocketGeometry[]): string {
  const min = sockets.length === 0 ? 0 : Math.min(...sockets.map(s => s.id));
  const max = sockets.length === 0 ? 0 : Math.max(...sockets.map(s => s.id));
...
const outputs: Array<[string, string]> = [
  ...
  [SWIFT_OUT, emitSwiftLattice(sockets)],
  [KOTLIN_OUT, emitKotlinLattice(sockets)],
];
