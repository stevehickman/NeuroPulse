...
// Android's counterpart to SocketZones.generated.swift. Session compilation
// resolves a named zone to sockets here, exactly as iOS does, so the two
// platforms answer "which sockets is 'Frontal Left'?" from the same generated
// artefact. Both are validated against 00-zones.npps on every run of this
// script, so neither can drift from the authored zones.
const KOTLIN_OUT = join(
  ROOT, "app", "android", "core", "src", "main", "kotlin", "life", "neurone",
  "core", "protocol", "SocketZones.generated.kt",
);
...
/**
 * Android's zone map. Mirrors emitSwiftZones so the two mobile runtimes resolve
 * a zone name to the same sockets, and both are checked against
 * 00-zones.npps by this script on every run.
 */
function emitKotlinZones(
  sockets: SocketGeometry[], zones: Map<string, number[]>,
): string {
  const bySize = [...zones.entries()].sort((a, b) => a[1].length - b[1].length);

  const bySocket = sockets.map(s => {
    const all = bySize.filter(([, ids]) => ids.includes(s.id)).map(([n]) => n);
    const primary = all[0] ?? null;
    const primaryArg = primary === null ? "null" : JSON.stringify(primary);
    const items = all.map(n => JSON.stringify(n)).join(", ");
    return `        ${s.id} to SocketZoneEntry(${primaryArg}, listOf(${items})),`;
  });

  const byZone = [...zones.entries()].map(
    ([name, ids]) => `        ${JSON.stringify(name)} to listOf<Int>(${ids.join(", ")}),`,
  );

  const zoneNames = [...zones.keys()].map(n => `        ${JSON.stringify(n)},`);
  ...
  return [
    ...
    `    const val MIN = ${sockets.length === 0 ? 0 : Math.min(...sockets.map(s => s.id))}`,
    "",
    "    /** Inclusive highest valid socket id. */",
    `    const val MAX = ${sockets.length === 0 ? 0 : Math.max(...sockets.map(s => s.id))}`,
    "",
...
const outputs: Array<[string, string]> = [
  ...
  [KOTLIN_OUT, emitKotlinZones(sockets, parseZoneFile(readFileSync(ZONE_FILE, "utf-8")))],
];
