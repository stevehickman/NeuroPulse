...
>
> **T2.** §4's second open question is answered — `SPEC-TMS-01…05` gives the coil an electrical
> specification and the recharge figure is **169 W**, so the T2 + TMS row is **~176 W at a 240 W EPR
> contract**. §4.4's *"~200 W across two PD sinks"* is no longer the relevant widening, because
> `NP-PWR-THERM-001` §11 puts the inlet on the TMS driver rather than on the hub. **The first open
> question — `OI-PWR-02`, the field target — is NOT answered and is now BLOCKING**: at 0.5 T the coil
> is 4.1× short of 120 % RMT, and clinical equivalence at this geometry is ~2,886 W. So the honest
> range §6 describes has collapsed to **two values 17× apart**, and §9 of that study carries the
> lower one **labelled**, never averaged between them.
...
| **OI-PWR-02** | **BLOCKING as of 2026-09-15, and QUANTIFIED rather than resolved (`NP-PWR-THERM-001` §7, PWRTH-D-4).** TMS field-strength requirement: confirm whether 0.1–0.5 T (CLAUDE.md §3) is achievable-and-correct for the 120 % RMT protocols the evidence base targets. **§4.2's concern is confirmed, for a reason §4.2 did not give.** Put through a first-order induced-field comparison against a clinical figure-8 — ratio only, so the geometry factors common to both cancel — the `SPEC-TMS-01` coil at **0.5 T reaches 17 % of the reference coil's maximum cortical field at 20 mm and is 4.1× short of 120 % RMT.** The shortfall is **depth falloff of a 25 mm wing** (0.48 against 0.76 at 20 mm), which §4.2 does not charge it for at all — **not** the peak-field comparison §4.2 does make, and the 1.8× this coil *gains* on pulse width (142 µs against ~250 µs, since the induced field goes as B·ω) is already credited in that 4.1×. Closing it by field alone needs **2.06 T → 613 J/pulse → 2,886 W**, which no PD class, mains accessory or head-worn thermal path in the programme reaches; the other lever is a larger wing, which trades back against the ring period. **Consequence: the TMS supply question has two answers 17× apart and this item decides which**, so it is now the gating item for every T2 power, thermal and mechanical decision | Everything downstream of §4 — and now `NP-PWR-THERM-001` §9's T2+TMS row, which is written at the lower answer and labelled |
| **OI-PWR-03** | ~~Real TMS coil inductance/geometry and capacitor-bank design, replacing the E ∝ B² scaling estimate in §4.1 with an actual calculation~~ **ANSWERED 2026-09-15 by `SPEC-TMS-01…05` (`NP-PWR-THERM-001` §8).** Figure-8, two 25 mm wings in series, 10 turns each, **L = 18.2 µH**, `R_ac` 11.7 mΩ, Q 69; **28 µF at 1,600 V**; **36.0 J/pulse at 0.5 T**; series ring **142 µs**; `I_peak` **1,989 A**; dB/dt 22.1 kT/s. Recharge **169 W** at 10 Hz biphasic-with-recovery (**423 W** monophasic — and no PD class delivers that, which is why the topology is specified rather than left open). **It reproduces §4.1's scaling estimate at both ends of the field range from inputs the estimate shares none of** (1.4 J at 0.1 T against ~1–2 J; 36.0 J at 0.5 T against ~16–40 J; 423 W and 134 W against ~400 W and ~130 W), and `check-power-envelope.ts --validate` pins those correspondences. **Residue: the geometry is chosen by that study, not specified by ME** — wing radius is the single largest lever and sets depth falloff, L, the ring period and the energy at once (`OI-PWRTH-09`, co-decide with `OI-PWR-02`) | **ANSWERED.** Residue is `OI-PWRTH-09` |
| **OI-PWR-04** | **NARROWED 2026-09-15 to (d), or (a) if the applicator must be passive (`NP-PWR-THERM-001` §11, PWRTH-D-7).** With `SPEC-TMS-04`'s **176 W** the option set can be priced for the first time. **(a) shared rail, one inlet — feasible NOW and was not before**: a 240 W EPR contract covers it, and the reason Rev 1 §4.3 rejected it was an *unbounded* requirement, not a large one (the bank is 28 µF). **(b) mains-tethered** — unchanged, and still costs Mode 3 device-wide plus IEC 60601-1 isolation ownership. **(c) a second PD sink on the hub** — **DECLINED**: its whole case was *scoping the Mode 3 loss to the modality that forces it*, and no power bank sources 176 W, so TMS is outside Mode 3 by arithmetic whatever the inlet count is, while `OI-PWR-12`'s cost (a 5 A switched aggressor inside the measured-shielding assembly) stands. **(d) a PD inlet on the TMS driver's own enclosure** — pays none of that: the head-worn hub stays single-inlet so `OI-PWR-12` does not arise, it stays SELV behind a certified brick, `OI-PWR-13`'s arbitration is unneeded for the head-worn assembly, and the 1,600 V bank and the 33 W of winding dissipation are already in the applicator regardless (`NP-PWR-THERM-001` §8.4). **No longer sequenced after `OI-PWR-11`** — §12 answers that item and the answer does not bear on (d). **`OI-PWRTH-07` (passive vs active applicator) is what decides (d) vs (a)** | ME + EE. Gated by `OI-PWR-02`'s field answer, not by `OI-PWR-11` |
...
| **OI-PWR-11** | ~~**CLAUDE.md §2.2 ships Pro Full two 65 W chargers; §4.5 negotiates one 100 W EPR contract. 130 W is not 100 W.**~~ **ANSWERED 2026-09-15 (`NP-PWR-THERM-001` §12, PWRTH-D-8): whatever the second brick is, it is not the TMS supply.** `SPEC-TMS-04` puts the requirement at **176 W**; 2 × 65 = 130 W is **short by 36 %**, and the monophasic figure is 430 W. **Reading 2 — a dual-inlet architecture assumed and never specified — is REFUTED**, so §4.4 was a proposal and not a specification being rediscovered (and §11 of that study declines it on its own merits regardless). **Reading 3 survives only for 1170 nm** (65 W covers a 44–46 W applicator and nothing at TMS). **Reading 1 — an undocumented spare — survives and is the only reading consistent with both §4.5 and `SPEC-TMS-04`.** The residual choice between readings 1 and 3 is a **commercial** question, not a power one, and is `OI-PWRTH-03` with `OI-PWRSRC-15`. **§2.2 remains locked and unmodified** | **ANSWERED.** Residual is `OI-PWRTH-03`, Principal + Commercial |
