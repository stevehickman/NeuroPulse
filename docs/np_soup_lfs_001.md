...
| **`OI-LFS-10`** | **HAZARD HALF DECIDED 2026-09-24 (Rev 5, §13.8) — Config `read_size`/`prog_size` 512, `cache_size` 512; the remaining half is OPEN.** *Decided:* the principal took the 512-byte program (the XTS data unit), so `ECR-EMMC-002` now covers the Config column too; `cache_size` 512 is forced by `lfs_init()` and equals `EMMC-FS-01`'s. §12's and §13's suites re-run on it and pass; the 256 configuration is kept as a test that must still lose data, so the evidence stays live. *Open:* `lookahead_size` (512 built vs 64 specified), `block_cycles` (500 vs 200) and `name_max`/`attr_max` (unset vs 64/256) — adopt `EMMC-FS-01`'s values or ECR it with the reasons in `np_lfs_instance.h`. None of the three is a power-loss hazard; the lookahead choice is the performance one (512 B covers the whole partition, 64 B covers one-eighth of it) | FW | **Documentation consistency of `L-5` against `EMMC-FS-01`** — no longer any reliance on power-loss results |
...
**Still open in `OI-LFS-10`:** `lookahead_size` (512 built, 64 specified), `block_cycles` (500 vs
