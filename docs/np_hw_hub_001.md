...
> 3. **The label has a better home, with a real generator.** `socketMap.generated.ts` already carries
>    `lobe` and `side` per socket, and `scripts/sync-socket-map.ts` re-derives all eight lobe zones and
>    **fails the build** if they drift from `00-zones.npps` — the sync guarantee firmware never had.
>    Anatomical labelling is an app-side lookup keyed on socket id.
>
...
> rather than deleted. Host suite: 18/18 ctest green, map-test check count 145 → 148.
>
