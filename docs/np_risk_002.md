...
|---|---|---|---|
| C1 | Safety-MCU cardiac interlock: HR change > 15 BPM → CVNS enable cut **within one main-loop iteration** (`NP-FW-CVNS-001` §5.4). *Corrected 2026-09-25: this row said "computed worst case < 5.1 ms", which assumed a TIM6 ISR that does not exist* | 2 | Implemented, host-tested. **FAI-CV02 not run** |
...
|---|---|---|---|
| **6** | **2026-09-25** | **NeurOne Systems Engineering + principal** | **OI-RISK2-05 closed; no RISK-25 score changes (GitHub #437).** The principal chose to **build** the two missing controls: a safety-MCU **IWDG**, and a Class C **pre-arm hold plus 3 s R-peak staleness cutoff**. `NP-FMEA-001` Rev 12 re-derives the rows against the code. §4.3.2 is corrected: C1's "< 5.1 ms" assumed a non-existent ISR, and C2's "no enable until armed, cross-validated" was not implemented and has no cross-validation. C2a and C2b are added. 25-a's basis is annotated (IWDG now present; the outcome is still stimulation-off). 25-b's owed test moves to the new **OI-RISK2-07**. **RISK-25 stays S5 × P2 = ALARP:** its P1 target waits on hardware verification (§4.3.1), which none of this supplies. |
