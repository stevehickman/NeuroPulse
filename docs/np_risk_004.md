...
|---|---|---|---|---|---|---|---|
| **RISK-SHELL-03** | **HIGH** *(re-scored 2026-09-25 from CRITICAL, §2.3; target MEDIUM once HUB-REQ-C07 is verified)* | `SAFE_EN[n]` polarity inverted between the interconnect spec and the safety MCU | `NP-DRV-SHELL-002` §6 declares active-high; `np_safety_config.h:7-8` declares active-low-enables | Implementing to the MCU convention makes power-on reset **enable** cranial stimulation across the lattice | **Polarity resolved 2026-09-25 (§2.2 adopted):** the MCU pin `PBM_CRANIAL_EN#` is inverted to an active-high `PBM_CRANIAL_PERMIT`, which ANDs with an active-high `SAFE_EN[n]`. Pull-downs hold both disabled when undriven, and the pull-up and buffer share one rail (`NP-HW-HUB-001` **HUB-REQ-C06**). **The residual single fault is detected and cut (2026-09-25, OI-RISK4-06):** the safety MCU reads back `PBM_CRANIAL_PERMIT` and drives an independent Class C vault-feed cut, `VAULT_FEED_EN` (**HUB-REQ-C07**). *Previously: none; `OI-CONV-01`. `OI-HUB-C07` closed 2026-08-16 and the basis changed, see §2.1* | Safety + EE Lead | **OPEN — re-scored HIGH 2026-09-25 (§2.3). Closes on verification of HUB-REQ-C06/-C07 (`NP-HW-HUB-001` OI-HUB-C22, OI-HUB-C23, bench fault injection)** |
...
### 2.2 RISK-SHELL-03 — resolution ADOPTED 2026-09-25 (GitHub #437); score re-assessed separately in §2.3
...
| Re-score RISK-SHELL-03 | this register | **Done separately, as required: §2.3, CRITICAL → HIGH, after the read-back decision (OI-RISK4-06, closed)** |
...
### 2.3 RISK-SHELL-03 — re-scored CRITICAL → HIGH (2026-09-25, OI-RISK4-06)

**This is the hazard-analysis act that §2.1 and §2.2 required, and it follows a design decision.**
`OI-RISK4-06` asked first whether an enable read-back is a required control. The principal
decided on 2026-09-25 that it is, on the cranial permit net, with an independent actuator:
`NP-HW-HUB-001` §7.2.6, **HUB-REQ-C07**. Options considered and not taken: read-back on all ten
enable nets (+10 I/O, which forces a larger safety-MCU package), detection with no actuator, and
no read-back.

**Why CRITICAL no longer applies.** CRITICAL was assigned to a **systematic** cause: two documents
specifying opposite polarities for one interface, so a board built to one and firmware written to
the other energises the whole lattice **on every unit, at every power-on**. That cause is removed
at its source. Both documents now describe the same two nets, and HUB-REQ-C06 fixes the inverter
between them. A systematic defect removed by correcting the specification needs no probability
argument.

**Why it is HIGH, and not lower yet.** What remains is a **random single fault** (a pull-up open,
the pin shorted LOW with the buffer powered, or a buffer fault) that energises the cranial lattice
without a grant. The consequence is the same class as RISK-26 (thermal injury at the scalp, HIGH).
HUB-REQ-C07 makes that outcome need a **second, independent** fault: the fault must also defeat
the read-back or the separate `VAULT_FEED_EN` switch. At boot the feed stays off until the permit
reads disabled. **But that control is specified, not built or verified** (ISO 14971:2019 §7.2
requires verification of implementation and effectiveness). The score therefore drops only as far
as removing the systematic cause justifies.

**Target MEDIUM**, when all of the following are met: (1) `NP-HW-HUB-001` OI-HUB-C22 (schematic),
(2) OI-HUB-C23 (firmware and host tests), and (3) bench fault injection of each single fault above,
each measured to feed-off.

**Outside this entry, and still open:** a hung safety-MCU main loop holds `VAULT_FEED_EN` and every
enable pin at their last state. That is `NP-RISK-002` OI-RISK2-05 (no IWDG), and this re-score does
not assume it is solved. The other nine enable lines were outside the decision's scope, and stay at
`NP-FMEA-001` FMEA-M01-05's S5×P1 ALARP.
...
| RISK-SHELL-03 | **Defined 2026-09-25 (§2.2 adopted):** (1) `NP-DRV-SHELL-002` **SH2-DRC-13**, as restated, at schematic + BSP review; (2) `NP-HW-HUB-001` **HUB-REQ-C06** at the Hub PCB Rev C schematic review (**OI-HUB-C22**); (3) `NP-FMEA-001` **OI-FMEA-01** bench cutoff timing, measured at the named nodes; (4) **HUB-REQ-C07**: firmware host tests (OI-HUB-C23), plus bench fault injection of pull-up open, pin shorted to GND and buffer supply lost, each measured to feed-off. *Previously: no verification defined, pending `OI-CONV-01` / `OI-FMEA-01`, see §2.1* |
