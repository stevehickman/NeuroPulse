...
- [x] ISC-13: Address→physical resolution returns ~~lobe/side/~~x-y/type; out-of-range/empty → NOT_PRESENT. — `np_module_map_resolve`, tested.
  > **Amended 2026-07-29 (OI-HUB-C14).** `np_physical_loc_t` no longer carries `lobe`/`side` — it returns `{ x_mm, y_mm, elem_type }`. The anatomical fields had zero production readers and no generator; labelling is now an app-side lookup against `socketMap.generated.ts`. The criterion is still met as amended: resolution returns the socket's metric location and element type, and out-of-range/empty still → `NOT_PRESENT`.
- [x] ISC-14: Groups resolve from ~~lobe+side (8 predefined L/R × 4 lobes),~~ socket sets, and address sets, with type include/exclude filter (mask 0 = all). — `np_module_map_resolve_group`~~`/_predefined`~~, tests incl. mask-0 both modes.
  > **Amended 2026-07-29 (OI-HUB-C14).** The eight predefined lobe groups and `np_module_map_predefined()` are RETIRED — they were never delivered functionality (no production caller, no production geometry table, no generator emitting firmware C). Zone membership is data owned by `00-zones.npps` and reaches firmware as a socket bitmap → `NP_GROUP_KIND_SOCKET_SET`. The criterion is still met as amended: socket sets and address sets resolve with the type filter, and the mask-0-both-modes tests were re-expressed over `KIND_SOCKET_SET`. See `docs/np_hw_hub_001.md` §4.5.2 and `docs/np_hex_zm_001.md` §4.
...
- [ ] ISC-88: Socket-addressed commands are executed end to end — socket-indexed control registry + per-socket safety-MCU enable. (OI-HUB-SOCKET-01; parse + resolve delivered, dispatch fails closed.) **2026-09-23: the registry is delivered (`src/np_socket_dispatch.c`, NP-FW-HUB-001 Rev 2 §3.4); the per-socket enable was superseded by the single `NP_SAFETY_EN_PBM_CRANIAL` bit (NP-HW-HUB-001 §7.2), which the registry owns. Still unchecked: every drive is refused at the power governor until OI-FWHUB-09 / OI-HEXTILE-09.**
