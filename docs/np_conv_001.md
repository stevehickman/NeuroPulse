...
**Its inverted form is `PBM_CRANIAL_PERMIT`, not `PBM_CRANIAL_EN`** (adopted 2026-09-25,
`NP-HW-HUB-001` HUB-REQ-C06, closing OI-CONV-01). An inverting buffer on the hub PCB turns the
pin into an active-high net, and that net needs a name. **When a signal passes through an
inverter, the two sides take different stems, never the same stem with and without `#`.** A lone
`#` disappears in the same diffs and fonts that §1.3 retired `_n` for. This line has already lost
its `#` once, in `NP-FW-HUB-001` before Rev 10.
