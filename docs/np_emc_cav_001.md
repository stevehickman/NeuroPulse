...
**Change Summary:** Rev 11 adds **`PACK-1`** (§8.8), resolving board vs. boss: they want different **radii** on the same axis and sit **one hex edge** apart. Safe at nominal pitch, **tight at the scan-compressed rim**. See §11.
...
3. **⚠ The 23.09 mm is a NOMINAL figure, and the lattice does not hold nominal everywhere.** The
   socket map is **scan-grounded, not idealised**, and its in-row spacing varies:

   | Rows | Measured in-row spacing | One hex edge |
   |---|---|---:|
   | **5–8 (mid-vault)** | 38.6 – 41.0 mm — **at nominal** | 22.3 – 23.7 mm |
   | 1, 3, 10 | 27.5 – 28.3 mm | ~15.9 – 16.3 mm |
   | **11 (rim)** | **24.0 mm — the tightest** | **13.9 mm** |

> **So `PACK-1` is comfortable over the mid-vault and tight at the crown and rim, where one hex edge
> falls to ~13.9 mm. The packing check must be run at the TIGHTEST cluster, not at nominal.**
> Where the intra-cluster run is too short, **the board continues outward into the inter-cluster
> seam** — which is free, because the neighbouring cluster's own boss is at *its* internal vertex,
> not its perimeter. That keeps the board on-axis and keeps `SYM-1`; it does not require moving the
> boss off-axis, which is the move that would cost the symmetry.
...
| **`OI-EMCCAV-14`** | **`PACK-1`'s clearance is a centre-to-centre budget, not a verified fit — the cluster board FOOTPRINT is not stated anywhere.** §8.8 puts the boss and the board **one hex edge** apart on the cluster mirror axis: **23.09 mm** at the nominal 40 mm lattice, but the socket map is **scan-grounded** and compresses — in-row spacing is 38.6–41.0 mm over the mid-vault (rows 5–8) and **24.0 mm at row 11**, where one hex edge is only **13.9 mm**. A board carrying an STM32G071, a PCA9548A, two mux banks, a TIA op-amp, a gain switch and ~400 passives has a real footprint that `NP-DRV-SHELL-002` §3.2 does not give. **Run the packing check at the TIGHTEST cluster, not at nominal**; where the intra-cluster run is short the board continues into the **inter-cluster** seam (free — the neighbour's boss is at its own internal vertex). Same missing input as `OI-EMCCAV-12` | EE + ME (**`MECH-2`**) | No — but it is the fit check `PACK-1` rests on |
