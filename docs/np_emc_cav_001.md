...
**Change Summary:** Rev 6 adds §8.6 — the fluxgates need **zero** gap (correcting Rev 5), the boss is the only real constraint and is undimensioned, and it should project **outward**. Corrects an `OI-THCOOL-06` status error this document propagated. See §11.
...
#### 8.6.3 Does the boss need to project inward? **No — and outward is strictly better**
...
| **`OI-EMCCAV-10`** | **Which way does the posterior blind-mate boss project — and reopen `OI-THCOOL-06` against the collar, not the loop.** §8.6.3: nothing requires an **inward** projection, and **outward is strictly better** — it decouples the vault Gap from the boss entirely (FLUSH-1's whole prize) at the cost of a local emboss inside the Boa-arch / neck-attach volume that is already the bulkiest part of the assembly. The magnetic cost is real but largely sunk: the boss is **already the one aperture** (`NP-DRV-SHELL-002` §4.3), continuity is already interrupted there, and the accepted treatment — a **mu-metal chimney collar** — is itself a projection, so the question is only which way it points. **`OI-THCOOL-06`'s original bench measurement is exactly the number needed**, but it was closed 2026-08-30 with *"reopen only if the [pneumatic] loop is revived"* — **too narrow a trigger**: it is about a formed collar at the boss, whatever passes through it | ME (**`MECH-1`**) + EMC | **Time-boxed — `NP-DRV-SHELL-002` §4.3: "must be settled before MECH-1 cuts the posterior boss"** |
