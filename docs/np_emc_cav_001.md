...
**Change Summary:** Rev 9 folds in **`PCB-1`** (0.80 mm board) and **`PLATE-1`** (the plate pockets), closing two of §8.7's four missing inputs. Gap floor firms to **2.75–3.25 mm**. See §11.
...
> **⚠ `PLATE-1` spends plate section where `NP-HEX-ZM-001` §5.4a says not to, and the mitigation is
> free.** That section caps the cluster at 7 tiles because an 8th raises **plate-mode deflection
> ×3.07** — *"the one that matters"* — and the plate already carries **34.2–57.0 N** across 19
> contacts on a 6-tile plate. Pocketing removes section. **But the controller board's *position*
> within the cluster footprint is unspecified** in both owning documents: sited **off mid-span** the
> pocket lands where the bending moment is low and costs almost nothing; sited **at the centre** it
> lands at maximum moment under a distributed load. Ribbing is the fallback, not the first move.
> **`OI-EMCCAV-13`.**
...
| **`OI-EMCCAV-13`** | **`PLATE-1` removes plate section, and the free mitigation is a position nobody has specified.** `NP-HEX-ZM-001` §5.4a caps the cluster at 7 tiles because an 8th raises **plate-mode deflection ×3.07** — *"the one that matters"* — and the plate already carries **34.2–57.0 N** across 19 contacts on a 6-tile plate (`NP-DRV-SHELL-002` §5.1.5). Pocketing removes section. **But the cluster controller board's POSITION within the cluster footprint is unspecified in both owning documents:** off mid-span the pocket lands at low bending moment and costs almost nothing; at the centre it lands at maximum moment under a distributed load. **Specify the position before the pocket geometry; ribbing is the fallback, not the first move** | ME (**`MECH-2`**) + EE | **Gates the pocket geometry** |
