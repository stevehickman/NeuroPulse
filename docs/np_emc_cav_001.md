...
> **Rev 18 (2026-09-24): these sweeps are now in the `EMF-1` test plan, `NP-HEX-ZM-001` §5.6.1
> (`OI-EMCCAV-04`, GitHub #401), re-stated against the as-designed stack.** The table below is kept as
> written, when the absorber still existed. There, `EMF-1a`–`EMF-1d` run on the deleted-station, 3 mm
> re-lofted bowl; `EMF-1a`/`EMF-1b` run to 6 GHz, and each sweep carries a pass criterion traced to
> `REQ-CAV-01` / `REQ-CAV-02`. *"Absorber fitted vs. removed"* survives only as its removed half. **The fitted half
> returns as `EMF-1f`** (principal direction 2026-09-24), a foam-fitted comparison on a separate outer
> bowl at the pre-re-loft radius, liner fitted then removed, empty and head-fitted, to 6 GHz.
...
> **⤷ Superseded 2026-09-23:** the principal took `REQ-CAV-04` without these runs, and §8.2 (Rev 3)
> had already found that they gate confidence, not the decision. The caution is kept as written.
>
...
> **✅ DECIDED 2026-09-23 — the principal TOOK `REQ-CAV-04` (GitHub #391).** Layer 4 is deleted and
> the 3 mm re-loft of the outer bowl is carried with it as one change. Executed across the document
> set per §8.4 (Rev 14): CLAUDE.md §1 / §4.3 now read *4-layer*, the Layer 4 number is **retired and
> held** (Layer 5 is not renumbered), `NP-HELMET-GEOM-001` §2 marks the station deleted and the
> radial total **27–32 mm**, and the re-loft rides `OI-ART-01`'s `NP-TOOL-SHELL-001` re-scope
> (GitHub #331). The reasoning below is retained as the rationale of record, in the tense it was
> written; where it says *"recommended"*, read *"taken"*.
...
> 2026-09-24), on a second outer bowl at the pre-re-loft radius. The article was chosen that way because
> a temporary liner in the re-lofted bowl would take 3 mm out of the cavity and shift the modes being
> compared. `EMF-1f` does not gate the deletion; it is scheduled, as asked here, before the re-loft is
> cut into CAD.
...
> **Executed 2026-09-23 (Rev 14).** Every row of the update table below now reads *4-layer* (or marks
> the station deleted); the four keying rows were left untouched, as instructed.
...
*Rev 22 (`OI-THCOOL-21`): this table's "Outward total" column is the **as-was** path, absorber in —
the comparison it serves (gap vs the absorber) needs the absorber in it. With the station deleted
(§8.2, taken) the same gaps give **0.335 at 6 mm · 0.258 at 4 mm · 0.220 at 3 mm** (gap now **69 %** of
the path); the "Recovered" column is unchanged. `check-cavity-q.ts --validate` pins both sets.*
...
> **★ `OI-EMCCAV-11` resolved in part, 2026-09-23 (GitHub #404; Rev 17).** Siting is now stated once,
> in **`NP-HEX-ZM-001` §5.3(c)**'s fluxgate-siting paragraph. Everything below cites it.
>
> - **Body location: SETTLED by the record.** The body is on L1, in the socket-wall + FPC-channel
>   station, and never in the Gap. Every document that sites it agrees. The only exceptions are
>   §221 and `NP-THERM-COOL-001` §6.9.1, both from one 2026-08-31 analysis, and both cite §5.3c for a
>   claim §5.3c does not make. §221's stated reason is corrected by a dated entry in
>   `completed-decisions.md`.
> - **Gap contributor list: SETTLED, and unchanged.** The fluxgates are off it under *either*
>   keep-out reading below, because a keep-out is planform and only a radial obstruction sets the Gap.
> - **The "likely reconciliation" above is NOT supported.** It assumed a magnetic keep-out that
>   reaches into the gap. **No fluxgate keep-out is specified anywhere in the record**: no radius, no
>   volume, no planform, and no part, package, count or site pattern to derive one from. Every
>   fluxgate protection in force is a **material** rule (`REQ-EMI-10`; `REQ-NET-04`; §6.9.1's
>   *"electrically insulating and non-magnetic"* pad). So §221's no-continuous-pad conclusion stands
>   on the controller boards (§8.7.3) and on bowl separation. It does not stand on the fluxgates.
>
> **What the record does not settle, and this document does not pick: is a planform keep-out needed?**
> That is an **EE decision** and remains open under `OI-EMCCAV-11`. Sibling GitHub #405 (gap-pad
> coverage) depends on it.
>
> | | **Reading A — material rules suffice, no geometric keep-out** | **Reading B — a planform keep-out around each fluxgate site** |
> |---|---|---|
> | What it says | A pad that is insulating and non-magnetic, which §6.9.1 already requires everywhere, has no conductive or ferrous content for the fluxgate to see, so it may land over a sensor site | A region above each site in which no pad lands, of radius *r_ko* |
> | The Gap (radial) | **No contribution** | **No contribution** — planform only |
> | Gap-pad coverage (#405, `OI-EMCCAV-12`, `OI-THCOOL-15`) | **No fluxgate term.** The cap is set by the 18 controller boards and service separation, and #405 can be computed now from those | **A fluxgate term**, *N_sites × π r_ko²* over the vault area. **It cannot be computed:** *N_sites*, the sites and *r_ko* are all unrecorded, so #405 must carry it as a parameter or wait for EE |
> | What would justify it | Nothing further. The material rules already state the protection | A mechanism the material rules do not cover. The only candidate the record hints at is **thermal**, not magnetic: `NP-ENV-OPRANGE-001` §2 names fluxgate *temperature drift* as the thing that degrades cancellation, and a pad is a local heat path. A pad over a sensor site would shift that sensor's temperature relative to its neighbours. **That is a hypothesis for EE, not a finding.** If it holds, the keep-out is sized by the thermal gradient the sensor tolerates, and that has to be traceable to the part's offset tempco (CLAUDE.md §18) |
> | Cost of choosing wrong | Choosing A when B was needed means a pad over a sensor introduces offset drift, which the geomagnetic zero-field nulling at session start (`durability-maintenance.md`) may or may not absorb | Choosing B when A sufficed means pad coverage is given up for nothing. On `NP-THERM-COOL-001` §6.9.1's table, dropping from 20 mm to 16 mm pads (22.7 % → 14.5 % coverage) raises R_gap 0.0094 → 0.0144 and cuts the tile ceiling 36.3 → 33.8 |
>
> **One more open condition, and it would change the settled half.** The body's siting is recorded,
> but its **fit** is not verified: no fluxgate part is selected, so no one has checked whether a
> 3-axis sensor fits the 2.0–2.5 mm station. A part that protrudes gap-ward would be a **local radial**
> Gap contributor at its sites, and would reopen the contributor list at those sites only.
...
*Rev 22 (`OI-THCOOL-21`): as-was outward totals. Current (station deleted): 6 mm **0.335** · 3.0 mm
**0.220** · 2.75 mm **0.210**; recoveries unchanged.*
...
*(2026-09-23, Rev 17: the fluxgate half is now settled, per §8.6.1's resolution note. The body is
not in the Gap, and **no fluxgate keep-out is specified**. Whether pad coverage needs a fluxgate term
at all is Reading A vs B there, an EE decision.)*
...
>
> **⚠ And Rev 12's closing paragraph was wrong too — corrected at Rev 13.** It argued from
> *"congruent **flat** hexagons cannot tile positive Gaussian curvature without opening gaps"* and
> concluded the gaps widen with curvature, handing `PACK-1` free clearance at crown and rim.
> **The premise is false: the module surface is not flat.**
>
> `NP-HEX-ZM-001` §3.1 committed **"Option A (rigid, MEDIAN-CURVED 40 mm hexagon)"** — a single
> **compromise curvature**, `R_m ≈ 87 mm`, **3.1 mm dome depth** at W = 40, chosen to minimise the
> discrepancy between module and site curvature **across all socket positions**. That is precisely
> what its *"worst-case mismatch"* column tabulates, per candidate tile width. **And the residual is
> already bounded and already allocated:**
>
> | | |
> |---|---|
> | Worst-case curvature mismatch at W = 40 | **1.04 mm** |
> | Typical, over most of the vault | **~0.25 mm** |
> | Absorbed by | **the PDMS window standoff + a ≤0.8 mm compliant gasket** |
>
> **So the mismatch is taken up RADIALLY at L0, not in-plane as gap width.** There is no uncounted
> free clearance: the quantity exists, is small, is bounded, and has a named home.
> **`OI-EMCCAV-15` is withdrawn.**
>
> **`PACK-1` is unaffected, and the curvature makes it *more* exact, not less.** Its 23.09 mm is the
> module's own hex edge. Measured as an **arc** on the R = 87 mm module surface it is **23.094 mm**;
> as a **chord**, **23.026 mm** — a **0.068 mm (0.29 %)** difference, immaterial against a 23 mm
> packing budget. And because every module is the same curved part, that figure is identical at
> every cluster.
>
> **Nothing decided in this document spends the L0 mismatch budget.** `PCB-1` and `PLATE-1` are on
> L1's **gap-facing** side, `BOSS-1` is on the **outer bowl**; the standoff and gasket are at **L0**,
> scalp-facing. Different stations, no interaction.
...
4. **It does not clear `RISK-SINK-03`.** The `SPEC-SINK-04` graphite spreader sits on the **exterior**
   ...
   stays **BLOCKING**. **`OI-EMCCAV-02`** — *taken up 2026-09-24 (Rev 20, GitHub #399):
   `NP-THERM-SINK-001` Rev 2 §7.1 re-scopes `RISK-SINK-03` to those two mechanisms, clears the
   Helmholtz loading at first order, and narrows external RF to the film's terminations
   (`EMF-1-SINK-2`). `OI-SINK-01` stays BLOCKING.*
5. **It did not itself modify any locked decision.** §8.2 raised a reopening and performed none;
   **the principal took it on 2026-09-23 (GitHub #391)**, and it is that decision — not this
   analysis — that changed CLAUDE.md §1 and §4.3 to *4-layer*.
6. **It does not budget the heat that moves when the station changes.** Lowering outward resistance
   ...
   budgeted it. **`OI-EMCCAV-07`.** **→ Answered 2026-09-23 by `NP-THERM-BOWL-001` (GitHub #403):**
   the BN-boss via lands ~90 % of tile heat on the bowl in parallel with this station, so the deletion
   moves the bowl **0.002 K**. Under the face-42 °C governor the coil holds **~42 °C**, and **43.1 °C** at
   the +50 °C SOFT bound. What remains is not a heat question. It is the unstated coil **drive
   topology** and `REQ-EMI-11`'s lack of a temperature trigger (`OI-THBOWL-01`, `-02`).
...
| ~~**`OI-EMCCAV-15`**~~ | ~~Quantify how the inter-tile gaps widen with curvature~~ **❌ WITHDRAWN 2026-09-21 (Rev 13) — the premise was false.** It rested on *"congruent **flat** hexagons cannot tile positive Gaussian curvature without opening gaps"*, but the module surface is **not flat**: `NP-HEX-ZM-001` §3.1 committed a **median-curved** 40 mm hexagon at `R_m ≈ 87 mm`, 3.1 mm dome — **one compromise curvature chosen to minimise mismatch across all socket positions**. The residual is **1.04 mm worst case, ~0.25 mm typical**, and §3.1 already allocates it to *"the PDMS window standoff + a ≤0.8 mm compliant gasket"*. **The mismatch is radial at L0, not in-plane gap width — there is no uncounted free clearance** | — (withdrawn) | — |
...
| **`OI-EMCCAV-11`** | **⚑ PARTLY RESOLVED 2026-09-23 (GitHub #404, §8.6.1 resolution note).** **Settled by the record:** the sensor **body** is on L1, inside the socket-wall + FPC-channel station, never in the Gap. This is stated once, in `NP-HEX-ZM-001` §5.3(c), and `NP-HELMET-GEOM-001` §2 and `NP-THERM-COOL-001` §6.9.1 now cite it. §221's stated reason is corrected by a dated entry in `completed-decisions.md`. **The Gap's contributor list is unchanged:** the fluxgates are off it under either keep-out reading. **Still open, as an EE decision:** (1) **is a planform keep-out needed?** No keep-out is specified anywhere, so the "likely reconciliation" this row once proposed is unsupported. Reading A (material rules suffice) and Reading B (a keep-out of radius *r_ko*, which the record cannot yet size) are in §8.6.1, with their consequences for gap-pad coverage (#405). (2) **Fit:** no fluxgate part is selected, so nobody has checked that a 3-axis sensor fits the 2.0–2.5 mm station. A gap-ward protrusion would reopen the contributor list locally | EE (keep-out, part) + ME (fit) | No — but #405's coverage cap waits on (1) |
| ~~**`OI-EMCCAV-07`**~~ | **✅ ANSWERED 2026-09-23 by `NP-THERM-BOWL-001` (GitHub #403).** ~~Nobody has budgeted the heat that moves when this station is re-specified.~~ **The heat barely moves.** The BN-boss via runs **in parallel** with the station between junction and bowl, so 0.410 → 0.335 shifts the bowl by **0.002 K** and the admissible drive by **< 0.01 W**. At the CLAUDE.md §4.5 peaks taken literally the coils reach 44–46 °C (T1) and 53–54 °C (T2), but every such row is face-inadmissible. **Under the face-42 °C governor the coil holds ~42 °C at 25 and 35 °C ambient, and 43.1 °C at +50 °C** with PBM blocked. **The residual is a drift question, not a heat question**, and it moves to that document's open items. **`OI-THBOWL-01`** covers the coil drive topology: under voltage mode copper moves the `REQ-EMI-11` gain 3.9 % per session and 19.5 % across the envelope (a 14.2 dB feed-forward ceiling); current mode is immune at first order. **`OI-THBOWL-02`**: `REQ-EMI-11` has no temperature trigger. `NP-ENV-OPRANGE-001` §2's SOFT designation stands | Thermal + EE Lead | — (answered; residual → `OI-THBOWL-01…05`) |
| ~~`OI-EMCCAV-07` (original text)~~ | ~~**Nobody has budgeted the heat that moves when this station is re-specified.** Lowering outward resistance puts more heat into the **outer bowl**, which carries the mu-metal and the Helmholtz coils. `NP-ENV-OPRANGE-001` §2 already makes the cancellation envelope **SOFT** on temperature drift, and `REQ-EMI-11` calibrates a coil-drive→field transfer that moves with coil resistance. Applies to the substitution as much as to a deletion~~ | ~~Thermal + EE Lead~~ | ~~No — but it is a **new** coupling~~ |
...
| 22 | 2026-09-24 | NeurOne EMC / Systems Engineering | **`OI-THCOOL-21` (GitHub #407) — current-case figures added beside §8.5's and §8.7.2's gap tables; nothing rewritten.** Both tables' "outward total" columns were computed with the absorber in, and §8.5's argument (*"2 mm of gap beats deleting the absorber"*) only means something that way, so they stay as the labelled as-was set; the current totals (station deleted) are **0.335 / 0.258 / 0.220 / 0.210** at 6 / 4 / 3 / 2.75 mm, gap share 56 % → 69 %. `scripts/check-cavity-q.ts` takes its thermal constants from the shared `scripts/thermal-outward-path.ts` and `--validate` gains five `[current]` anchors. **One consequence of §8.2's decision that its per-area comparison could not price**: the binding re-loft also shrinks the helmet exterior 4.6 %, and `NP-THERM-SINK-001` Rev 3 §3a finds that makes the scalp **slightly warmer**, not cooler (+0.2 K at N = 6, library floor) — raised to the principal as `OI-SINK-10`, **not** as a reopening of `REQ-CAV-04`. |
