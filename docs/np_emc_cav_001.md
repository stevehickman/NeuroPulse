...
> `REQ-CAV-01` / `REQ-CAV-02`. *"Absorber fitted vs. removed"* survives only as its removed half. Whether
> to add a foam-fitted comparison run is flagged there for the principal and not decided.
...
> *removed* half as a real configuration, so the *fitted* half is flagged there for the principal, with
> its cost. The re-lofted bowl has no station, so a temporary liner takes 3 mm out of the cavity; the
> like-for-like alternative is a second bowl at the pre-re-loft radius. Until the principal decides,
> the plan runs the as-designed stack only.
...
| **`OI-EMCCAV-04`** | Add `EMF-1a`–`EMF-1d` (§7) to the `EMF-1` fixture's test plan. Four sweeps on a fixture already committed. **✅ PLAN WRITTEN 2026-09-23 (Rev 18, GitHub #401): `NP-HEX-ZM-001` §5.6.1**, re-stated against the as-designed stack, with each sweep's pass criterion traced to `REQ-CAV-01` / `REQ-CAV-02`, and `EMF-1a`/`EMF-1b` swept to 6 GHz. **Residual, flagged for the principal and not decided:** whether to add a foam-fitted comparison run (`pending-decisions.md`). ~~Gates §8.2's deletion~~ — ~~**Rev 2: no longer gating**, because §8.2 now recommends substitution, which does not depend on them.~~ **Corrected Rev 18:** not gating, because §8.2 was rewritten at Rev 3 to recommend **deletion**, the principal **took** it on 2026-09-23 (`REQ-CAV-04`, GitHub #391), and §6.1's result is structural, so these sweeps gate confidence, not the decision. They are what would let `REQ-CAV-04`'s justification be stated as *measured*; **sweep `EMF-1a` to 6 GHz** for `OI-EMCCAV-06` (done: `EMF-1a` and `EMF-1b` both run to 6 GHz) | EMC | No. **Plan written Rev 18**; the one residual (foam-fitted comparison) is the principal's |
