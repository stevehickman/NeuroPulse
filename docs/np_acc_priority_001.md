...
| **9** | **Cervical tcVNS gel pads** (5-pack) | Same interlock chain as the accessory it serves: impedance confirmation plus the cardiac-rhythm interlock | T2 only | **Specification gap — `OI-ACC-03`:** specified in `docs/reference/modality-stack.md` but absent from the §2.3 consumables table; no price, interval or GM% |
...
| **14** | **Audio cup foam** (set) | Seal and comfort | Home Standard and up | Shipping — **`OI-ACC-02`: its prompt is the one calendar-triggered consumable reminder in the document set, and CLAUDE.md §2.3 states all consumable prompts are measurement-triggered, never calendar-triggered.** One of the two is wrong |
