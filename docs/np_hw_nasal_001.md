...
| **REQ-NASAL-11** | Duty ceiling **25 %** (`INS_DUTY_MAX` = `0x32`), and the protocol grammar bounds `duty_cycle` 1–25 % | `firmware/hub_control/modules/np_mod_intranasal.c`; `NP-NPPS-REF-001` §4.2 |
...
| Command block | `np_mod_intranasal_params_t` — `side`, `freq_code`, `duty` (≤ `0x32`), `cur_660`, `cur_808` | `np_hub_types.h` |
...
1. **No irradiance ceiling.** `cur_660` and `cur_808` are uncalibrated 8-bit drive registers with
   no mW/cm² of record at either end. The scalp figure is not transferable by default: it was
