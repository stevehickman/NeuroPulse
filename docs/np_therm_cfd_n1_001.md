...
7. **The ambient gate has no N input (§4b).** `NP-ENV-OPRANGE-001` §2's +35 °C block is recorded as
   more conservative than the physics requires, against `NP-THERM-COOL-001` §7's **single-tile**
   37.9 °C ceiling. At N = 1 this model corroborates that figure (39.1–40.6 °C). At **N ≥ 20 and a
   realistic 0.5 K/W it inverts** — 32.9 °C admissible against a 35 °C gate. Conditional on
   `OI-N1-02`, and not a live hazard because §6a's per-protocol ceilings bind first, but it is a
   flat threshold against an N-dependent ceiling. **`OI-N1-08`.**
8. **18 of 23 authored protocols are thermally bound, not power bound (§6a)** — the reverse of the
...
9. **`OI-PWRSRC-07`'s premise is wrong (§7).** It infers "~39 % of steady rise" in a 6–30 min session
   ...
   rung.**
10. **`OI-THCOOL-08`: the cooling-option ratios shrink by ~2.4× (§8).** `NP-THERM-COOL-001` §5's
...
11. **`OI-PWR-08`: §3.5's ~100 °C is the right sign and order (§9).** A model valid at N = 80 gives
...
**On the ambient rows.** `NP-ENV-OPRANGE-001` §2 blocks PBM T1-A above **+35 °C** ambient and allows
full dose only **≤ +30 °C** (decided by principal 2026-08-31; the threshold moved +43 → +38 → +35),
with a 1.0 °C hysteresis (`OI-THCOOL-16`) and a per-protocol efficacy-floor clamp landing the effective
block at **33.8 / 34.2 / 34.6 °C** for a 40 / 60 / 120 J/cm² protocol (`OI-THCOOL-17`,
`NP-THERM-COOL-001` D-4). So the **35 °C rows are already at the block**, and the **43.3 °C rows are far
outside the operating envelope** — they are retained only as the comparison against R1's worst case,
never as an operating condition. **The 25 °C rows are the design case.**

### 4b. Max ambient by N — and a gap in the ambient gate

`NP-ENV-OPRANGE-001`'s hard edges are denominated in ambient, so that is the quantity to compare
against. Highest ambient holding max face ≤ 42 °C, distributed, at the library floor:

| N | perfect | 0.25 | 0.50 | 1.00 | 2.00 K/W | vs the +35 block @ 0.50 |
|---:|---:|---:|---:|---:|---:|---|
| 1 | 39.1 | 39.4 | 39.6 | 40.0 | 40.6 | clears |
| 6 | 39.0 | 38.3 | 37.8 | 36.8 | 35.3 | clears |
| 12 | 39.0 | 37.3 | 35.7 | 33.1 | 29.2 | clears |
| 20 | 39.0 | 35.8 | **32.9** | 28.2 | 21.1 | **2.1 K SHORT** |
| 80 | 38.9 | 24.8 | **12.2** | −8.9 | −10.0 | **22.8 K SHORT** |

At N = 1 the model gives 39.1–40.6 °C, bracketing `NP-THERM-COOL-001` §7's 37.9 °C single-tile
full-dose ceiling — an independent corroboration of the figure the block was set against.

**But the block has no N input, and that is the gap.** `NP-ENV-OPRANGE-001` §2 records the +35 °C
edge as *"deliberately more conservative than the physics requires — `NP-THERM-COOL-001` §7 puts
T1-std's full-dose ceiling at 37.9 °C."* **That 37.9 °C is a single-tile number.** On this model the
conservatism it describes is real at N ≤ 12 and **inverts at N ≥ 20**: at a realistic 0.5 K/W the gate
admits ambients the assembly cannot hold, by 2.1 K at N = 20 and 22.8 K at N = 80. The gate is a flat
ambient threshold against an N-dependent ceiling.

**Two honest bounds on that.** It is conditional on `OI-N1-02` — with a perfect sink every row sits at
~39 °C and the gate is conservative at every N. And no authored montage runs 20 tiles at the library
floor today (§6a's ceilings are the binding limit first). **It is a gate-design gap, not a live
hazard** — but it is the same shape as `OI-N1-03`: a safety gate whose input variable does not include
the term that moves the answer. **`OI-N1-08`.**
...
| RISK-N1-05 | The ambient gate is a flat threshold against an N-dependent ceiling; at N ≥ 20 and a realistic heatsink it admits ambients the assembly cannot hold | None — `NP-ENV-OPRANGE-001` §2's hard edges have no N or per-tile-drive input | **No verification defined** — `OI-N1-08`; conditional on `OI-N1-02` |
