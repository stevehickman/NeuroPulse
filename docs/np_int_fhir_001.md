# FHIR R4 ImplementationGuide: NeurOne T2 Clinical Profile
...
This document defines the NeurOne FHIR R4 ImplementationGuide (IG) — the machine-validatable specification governing all FHIR resources created, transmitted, or stored by the NeurOne T2 clinical platform. All T2 EHR integration code must conform to this IG before any data is transmitted to a Business Associate.

**Design principle:** NeurOne FHIR resources use the **minimum necessary** patient demographic data consistent with HIPAA §164.502(b). Patient resources contain only an opaque clinic-assigned identifier — no name, date of birth, address, or contact information. All clinical observations use FHIR-standard LOINC coding where codes exist; NeurOne-defined codes are used for device-specific observations without established LOINC equivalents.

**Scope:** T2 NeurOne Pro clinical operations only. T1 NeurOne Home wellness operations do not use FHIR.
...
| Field | Value |
|---|---|
| Canonical URL | `https://fhir.neurone.life/ig/NeurOne-T2-Clinical` |
| FHIR version | 4.0.1 |
| IG version | 1.0.0 |
| Status | Active |
| Jurisdiction | US (urn:iso:std:iso:3166#US) |
| Copyright | NeurOne Inc. — All rights reserved |
| Publisher | NeurOne Inc. |
| Contact | fhir@neurone.life |

---

## 3. Permitted Resource Types

Only the following FHIR resource types may appear in NeurOne T2 clinical data exchanges. Any FHIR resource not listed below must not be created, transmitted, or stored by NeurOne systems.

| Resource type | Profile URL | Purpose | Status |
|---|---|---|---|
| `Patient` | `https://fhir.neurone.life/StructureDefinition/NP-Patient` | Patient identity (opaque identifier only) | Required |
| `Observation` | `https://fhir.neurone.life/StructureDefinition/NP-Observation` | Clinical measurements (EEG, HRV, dose) | Required |
| `DiagnosticReport` | `https://fhir.neurone.life/StructureDefinition/NP-DiagnosticReport` | Session summary report | Required |
| `Procedure` | `https://fhir.neurone.life/StructureDefinition/NP-Procedure` | Neuromodulation session record | Conditional (Full Clinical tier only) |

**Explicitly prohibited resource types:** `RelatedPerson`, `Coverage`, `Claim`, `ClaimResponse`, `ExplanationOfBenefit`, `MedicationRequest`, `AllergyIntolerance`, `Condition` (NeurOne is not a diagnostic system), `CarePlan`, `ImagingStudy`.
...
| `Patient.identifier.value` | 1..1 | Opaque MRN assigned by the clinic | Must not be NeurOne account ID, email, or UHDR token |
...
The Patient identifier is the clinic's own MRN for the patient. NeurOne does not assign, know, or store the mapping between the MRN and the patient's identity. The clinic holds this mapping in their EHR.
...
**Validation rule:** The FHIR validator shall reject any Patient resource where `identifier.value` matches the pattern of an email address (`.*@.*`), a US SSN (`\d{3}-\d{2}-\d{4}`), a NeurOne account token, or a UHDR token.
...
| `Observation.performer` | 0..0 | **PROHIBITED** — NeurOne observations are device-generated, not clinician-authored |
...
### 5.2 NeurOne Observation types

**EEG observations** — no established LOINC codes for quantitative EEG spectral features. NeurOne local coding system used:

| Observation type | Code | System | Unit | Notes |
|---|---|---|---|---|
| Alpha band power ratio | `NP-EEG-ALPHA` | `https://terminology.neurone.life/observations` | `{ratio}` (dimensionless, [0,1]) | Weekly session average |
| Theta band power ratio | `NP-EEG-THETA` | `https://terminology.neurone.life/observations` | `{ratio}` | Weekly session average |
| Delta band power ratio | `NP-EEG-DELTA` | `https://terminology.neurone.life/observations` | `{ratio}` | Weekly session average |
| Gamma band power ratio | `NP-EEG-GAMMA` | `https://terminology.neurone.life/observations` | `{ratio}` | Weekly session average |
| Neurofeedback performance score | `NP-NFB-SCORE` | `https://terminology.neurone.life/observations` | `{score}` (0–100) | Per-session; weekly average |
...
| HRV coherence score | `NP-HRV-COHERENCE` | `https://terminology.neurone.life/observations` | `{score}` (0–10) | NeurOne-defined coherence metric |
...
| Observation type | Code | System | Unit | Notes |
|---|---|---|---|---|
| PBM session dose | `NP-PBM-DOSE` | `https://terminology.neurone.life/observations` | `J/cm2` | Per zone; weekly aggregate |
| BES session dose | `NP-BES-DOSE` | `https://terminology.neurone.life/observations` | `mC` (millicoulombs) | Weekly aggregate |
| Session count | `NP-SESSION-COUNT` | `https://terminology.neurone.life/observations` | `{count}` | Count of completed sessions in period |
...
| `DiagnosticReport.code` | 1..1 | `NP-SESSION-REPORT` from `https://terminology.neurone.life/reports` |
...
| `Procedure.code` | 1..1 | SNOMED CT `229070002` (Biofeedback therapy) or NeurOne code `NP-NEUROMOD-SESSION` |
...
If the Business Associate implements real-time LSL streaming from the NeurOne hub for research or clinical monitoring purposes, the following security requirements apply:

(a) **Encrypted transport required.** All LSL streams carrying NeurOne data must be tunnelled through TLS 1.2 or higher. Acceptable implementations: `stunnel` with TLS certificate pinning, WireGuard VPN with certificate-based authentication, or OpenVPN with TLS mutual authentication.
...
(c) **NeurOne warranty.** NeurOne warrants the integrity of data exported from the hub via USB-C or BLE. NeurOne does not warrant the security of LSL deployments that do not use encrypted transport per (a).
...
The NeurOne T2 clinical platform must include a FHIR validator integration that runs against all resources before they are transmitted to a Business Associate or accepted from a Business Associate. The validator must:

(a) Validate all resources against the NeurOne IG profile versions published at the canonical URL (§2).
