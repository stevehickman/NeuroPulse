...
4. **SW-02 efficacy-floor admission (Class B, new at 0x02):** recompute `duty_floor_pct` from
   ...
   so the floor edge is inside §6.1's scope — **and which module holds the latch for a refusal raised
   here was not specified until `OI-POE-09` closed: it is a SECOND latch, owned by SW-02, distinct
   from the §6.1 Class C object, never writing to it, and composed with it at the admission test
   (`NP-PWR-THERM-001` §10.1, PWRTH-D-5). Its band is `Δ = max(1.0 °C, 50/D °C)` — ≥ 2.0× the
   efficacy floor on re-admission (PWRTH-D-6).** §6.1 states the latch is SW-01 state, which was
...
| OI-POE-09 | ~~**Who holds the hysteresis latch on the efficacy-floor edge, and is Δ = 1.0 °C the right band for it?**~~ **CLOSED 2026-09-15 by `NP-PWR-THERM-001` §10 (PWRTH-D-5, PWRTH-D-6).** **(a) Ownership — TWO INDEPENDENT LATCHES, one per IEC 62304 class.** The shared-latch option buys §6.1's *"one boolean and one timer"* property and pays for it by making a **certified** object writable from an **efficacy** computation; `NP-THERM-COOL-001` §7.4.4's own argument for siting the clamp in SW-02 is that an efficacy computation inside the Class C boundary re-opens that boundary every time `NP-PWR-BUDGET-001` §3.4's floor is revised, and a *write* into Class C state is the same coupling through a narrower aperture, not an escape from it. Cost is exactly two booleans and two timers, bounded and known. **What replaces the withdrawn Rev 10 claim, stated so it is not assumed a second time:** *a floor refusal sets the **SW-02 efficacy latch**, a distinct object from the §6.1 Class C hysteresis latch; the two never write to each other; admission requires **both** to be clear, and the composition is at the admission test, not in either latch's state* — which is §5's existing `min()` shape and introduces no new composition semantics. **(b) Sizing — the band is chosen in DOSE: `Δ = max(1.0 °C, 50/D °C)`** for a protocol of full dose `D` J/cm², i.e. **re-admit only at ≥ 2.0× the efficacy floor**, composed by `max()` exactly as §6.1 requires margins to enter. A pure dose rule is **unimplementable above 50 J/cm²** — `OI-POE-06` records that `adc_to_celsius()` returns whole degrees, so 0.42 °C would silently become 1.0 °C with nobody having chosen it — and the `max()` is what makes the rule chosen where the sense path can express the choice and safe where it cannot. Changes behaviour **only for protocols at or below 50 J/cm²** (40 J/cm² re-arms at 32.5 °C rather than 32.8 °C — *more* conservative), touches no Class C state, and cannot widen any thermal bound because +35 °C is not SW-02's to move (§1). **`OI-POE-06` is not closed by this and becomes slightly more valuable: at a 0.1 °C ambient path the dose rule binds unaided at every dose and the `max()` goes inert** | FW + Thermal. **CLOSED** |
