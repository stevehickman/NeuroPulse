...
>
> §4 records **six** independent blocking defects: **A, B, C, D and F are closed; E is open.** The bootloader configures and links cleanly (`rc=0`, OCRAM 100.00%). The safety MCU **compiles** for STM32G071 — all 10 Class C translation units, 0 errors, 0 warnings — and its compile leg gates from phase 4. It does **not link**: Defect E (§4.4, OI-SWCI-17) leaves 24 first-party `np_hal_*` platform symbols undefined, so no `np_safety_mcu` image has ever been produced, and phase 4's exit criterion was corrected accordingly (§6). As of Rev H the platform *contract* is single-sourced in `np_safety_hal.h` and OI-SWCI-18 is closed (§4.4.1) — **that is a precondition for phase 7, not progress against it.** No driver and no stub exists; the image still does not link. The main-firmware super-project builds clean as of phase 5 — `rc=0`, 0 errors, 0 warnings, 0 unresolved symbols.
>
...
| 7 | safety-mcu | Implement the SW-01 platform layer (Defect E, §4.4, OI-SWCI-17) and re-point the CI leg from `np_safety_mcu_objs` to the full build. **Unblocked, not started (2026-08-10)** — the precondition work is done: the platform contract is single-sourced in `np_safety_hal.h` and OI-SWCI-18 is closed (§4.4.1). No driver is written and no stub exists; the leg still targets `np_safety_mcu_objs` | Safety MCU **links**; the image exists |
...
**Phase 7 is unblocked but not started (2026-08-10) — and "unblocked" means its *precondition* is met, not that its blocker is gone.** Phase 7 needs a linking image; a linking image needs Defect E; Defect E is untouched. Phase 7 remains **blocked on OI-SWCI-17**. What landed is the precondition work — the SW-01 platform contract is single-sourced in `firmware/safety_mcu/include/np_safety_hal.h` and OI-SWCI-18 is closed (§4.4.1) — so the day the drivers are written, a wrong signature is a compile error in every consumer at once instead of a silently mislinked call in Class C software. **Nothing in phase 7's exit criterion is met by it.** No driver exists, no stub exists, `cmake --build … --target np_safety_mcu` still exits `rc=1` on 24 undefined symbols, and the CI leg still gates `np_safety_mcu_objs`. Defect E and OI-SWCI-17 are unchanged and open. This is a precondition, not progress against the criterion, and it is recorded that way deliberately: the header's own banner refuses stubs for the same reason §6 opens by refusing a permanently-red required check — a green leg over an unimplementable image would be the worse of the two failures.
