...
**Known accepted exceptions — two, both for the same reason.** A guard that scans the whole repository has to be triggerable from anywhere the thing it scans can live, and *a guard that cannot see the edit that breaks it is not a guard*. Each is deliberate and stands; they are enumerated here so a later reader does not read either as a violation of this principle.

| Workflow · job | Out-of-surface entries | The guard, and the edit that breaks it |
|---|---|---|
| `web-ci.yml` · `web-ci` | `CLAUDE.md`, `docs/**`, `app/android/**`, `firmware/**` | `check-section-refs.ts`. A `§N` citation can live anywhere; a CLAUDE.md reorganization dangles 100 of them and touches no web code. |
| `tooling-ci.yml` · `doc-naming` | `docs/**` | `check-doc-filenames.ts`. NP-CONV-001 §4.0 is violated only by a document **added, renamed or moved** — which is a change to `docs/**` and to nothing else. |

**The second exception was added on 2026-09-01, after the guard was found to be unreachable by the only change that can break it.** `check-doc-filenames.ts` had been wired into `tooling-ci.yml`'s `scripts` job since #299, whose relevance list is `scripts/**`, `tsconfig.scripts.json`, `package.json`, `bun.lock`. None of those matches `docs/**`, so the job reported `skipped` on every docs-only PR — correctly, per §6.7, and uselessly. Measured on [#306](https://github.com/stevehickman/NeuroPulse/pull/306), a docs-only PR adding five files under `docs/reference/`: `scripts/ type-check + document-naming guard` skipped. The guard ran only when a change happened to *also* touch `scripts/**` — incidental to whether it could fail, and absent from every docs-only PR.

The fix is a **separate job with its own list**, not a wider `NP_SCOPE_SCRIPTS`, because the two surfaces do not overlap: `tsc -p tsconfig.scripts.json` is broken by `scripts/**` and by nothing under `docs/`; the naming guard is broken by `docs/**` and by nothing in `scripts/` but its own source. Folding `docs/**` into the scripts list would have run a type-check over every documentation commit to buy nothing — a real §5.0 cost, paid for no coverage. The exception is therefore narrower than web-ci's: one entry, on one job.

Both lists carry `--check-tree` and `--relevant` assertions in their `changes` job, so a list that stops matching fails CI instead of silently narrowing (`ci-changed-scope.sh`; the exception cases are asserted first in each). The `doc-naming` assertions additionally pin `docs/superseded/**` — the guard's second scan directory, reached only by the matcher's `prefix/**` shape covering nested paths.

**This defect class has now been found three times, all by inspection, and OI-SWCI-08 is still the general remedy.** §5.0 already anticipates it: *"a guard that fails CI when a module in the build graph is missing from the corresponding `paths:` list would make the drift self-detecting rather than audit-dependent. Not built here; see OI-SWCI-08."* The relevance-list half of that idea generalises to gates — **a gate's relevance list must cover the population its `CI-Scans` declares** — and every instance below was found by a human comparing the two by hand, which is exactly the audit-dependence OI-SWCI-08 exists to remove:

| Gate | Declares it scans | Its list saw | Status |
|---|---|---|---|
| `check-doc-filenames.ts` | `docs/` and `docs/superseded/` | `scripts/**` only | **fixed 2026-09-01** — `doc-naming` job |
| `check-gate-coverage.ts` | `scripts/`, `ci/`, and every workflow | `scripts/**` + `tooling-ci.yml` | **fixed 2026-09-01** — `gate-coverage` job |
| `check-section-refs.ts` | CLAUDE.md citations in every tracked file of 11 extensions | `web-ci.yml`'s `NP_SCOPE_PATHS` | **OPEN — OI-SWCI-38** |

**OI-SWCI-38 (new, 2026-09-01).** Measured on this branch: **34 tracked files carrying a literal `CLAUDE.md §N` citation fall outside `web-ci.yml`'s relevance list**, almost all under `app/ios/**` and `app/NeurOneShared/`. The list enumerates `CLAUDE.md`, `docs/**`, `app/android/**`, `firmware/**` and the web surface — `app/android/**` is there because #249's audit found five dangling Kotlin citations, and the iOS side was never added. An iOS-only PR that adds or breaks a citation therefore does not run the guard. Not fixed here, because it is a different workflow and a different guard, and bundling it would put three unrelated scope decisions behind one review.
...
| OI-SWCI-38 | **Raised 2026-09-01, as the third instance of one defect class — see §5.0.** `check-section-refs.ts` declares `CI-Scans: CLAUDE.md section citations in every tracked file of 11 source extensions`, but the `web-ci.yml` relevance list that decides whether it runs enumerates `CLAUDE.md`, `docs/**`, `app/android/**`, `firmware/**` and the web surface. **Measured: 34 tracked files carrying a literal `CLAUDE.md §N` citation fall outside that list**, almost all under `app/ios/**` and `app/NeurOneShared/`. `app/android/**` is on the list only because #249's audit happened to find five dangling Kotlin citations; the iOS side was never added, so an iOS-only PR that adds or breaks a citation does not run the guard. Fix is the same shape as the two closed on 2026-09-01 — widen the list to the population the gate declares, and assert the widened entries — but it belongs in `web-ci.yml`, which is why it was not bundled. The general remedy that would have caught all three without a human diffing `CI-Scans` against a relevance list by hand is **OI-SWCI-08**, still unbuilt | Quality | — |
