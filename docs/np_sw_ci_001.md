...
**OI-SWCI-08 remains open and is still the right general remedy.** Four instances of one defect class, all caught by inspection, is exactly the audit-dependence it exists to remove: nothing mechanically compares a gate's declared `CI-Scans` against the relevance list that decides whether it runs.
