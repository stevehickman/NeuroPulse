...
| HUB-MDR-03 | Probe dock retention force ≤2N insertion/extraction — bench measurement on first-shot part | Open — **GATED: OI-NASAL-09** (the measurement needs a mating probe; none is dimensioned). The ≤2 N figure's provenance is **`OI-HTOOL-10`** (Rev 4; was `NP-CONV-001` OI-CONV-08 (c)) |
...
| HUB-MDR-10 | Fan intake/exhaust grille louvre slot width ≤6mm (finger-probe safety) confirmed in CAD | Open. **The ≤ 6 mm limit stands: it is the `RISK-HUB-02` hazard control** (`NP-RISK-004`), which CLAUDE.md §18 does not reach. Its sizing basis is stated wrongly in F-04 and is **`OI-HTOOL-11`** (Rev 4) |
...
| **OI-HTOOL-10** | **HUB-MDR-03's ≤ 2 N dock force has no derivation, and the dock has no retention floor at all** (raised Rev 4, from `NP-CONV-001` `OI-CONV-08` (c), GitHub #394). F-01 states *"light snap-detent, ≤2N insertion/extraction force"*. No HFE, risk or accessibility document sources 2 N. `NP-HFE-001` CT-02's ≤ 1 N is the retired zone-module eject lever, and `RISK-22` is module extraction, not docking. **Searched, not found, so raised and not retired** (`NP-CONV-001` §7.1): the figure is plausibly derivable, from the one-handed undock force of `NP-HFE-001`'s reduced-grip population as the maximum and from retention against probe mass and cable drag as the minimum. The second half is a missing requirement, not an unfounded one. A detent with no floor can release the probe onto the floor, and the probe carries an authenticated sleeve (CLAUDE.md §2.3). **Tooling-blocking** for the detent geometry, alongside `OI-HTOOL-07`. Derive both bounds, or record that nothing requires them | HFE + ME. **HUB-MDR-03, FAI-HUB-20** |
| **OI-HTOOL-11** | **F-04 states the louvre's basis wrongly, and the limit stands regardless** (raised Rev 4, from `OI-CONV-08` (c)). F-04 says ≤ 6 mm is *"sized to the IEC 60529/60601-1 finger-probe test dimension"*. The jointed test finger (IEC 61032 probe B, used by both standards) is **12 mm** in diameter, so 6 mm is about 2× tighter than the test dimension, and the probe's reach through a slot also depends on the slot's depth to the blade, which F-04 does not state. **The ≤ 6 mm figure is the `RISK-HUB-02` hazard control and is not retired or relaxed** (CLAUDE.md §18 scope). Correct F-04 to name the clause and the probe, state the slot-to-blade depth, and record the 2× as a chosen margin or re-derive it | ME + Safety. **HUB-MDR-10, FAI-HUB-15** — non-blocking; the conservative figure is safe to cut |
...
|---|---|---|---|
| 4 | 2026-09-25 | NeurOne Systems Engineering | **`NP-CONV-001` §7.1 audit, cluster (c) (`OI-CONV-08`, GitHub #394). Two dispositions, and they go opposite ways.** HUB-MDR-03's ≤ 2 N dock force has no derivation anywhere in the set. It is **raised as `OI-HTOOL-10`, not retired**: it is plausibly HFE-derivable, and the search found a missing *floor* beside the unfounded ceiling. HUB-MDR-10's ≤ 6 mm louvre **stands**, because it is the `RISK-HUB-02` hazard control. Only its stated basis is wrong (the IEC 61032 finger is 12 mm), and that is `OI-HTOOL-11`. No geometry, limit, checklist item or FAI item changes. |
