...
| **A14.3 — gel pads (consumable)** | Self-adhesive hydrogel, 5-pack; performance guaranteed for one application, reuse permitted while `REQ-CVNS-06` passes (§6) | Commercially defined. **No formulation, adhesion or impedance specification** |
...
> **Rev 2 (2026-09-22) — closed by principal decision: single use is not enforced, because a
> failing pad is detected.** *"Single-use"* is a **performance guarantee** — the pad is guaranteed to
> work for one application — not a reuse prohibition, and `NP-REG-CVNS-001` §2.4 (Rev 2) now says so.
> A pad is replaced when it fails the pre-enable impedance check (`REQ-CVNS-06`, ≤ 5.0 kΩ at 1 kHz,
> safety MCU refuses enable) or lifts mid-session (`REQ-CVNS-08`, 500 ms). That is a condition
> measurement, so the pad has a `CLAUDE.md` §2.3 trigger and needs neither authentication nor a
> use count; the consumables row is `docs/reference/commercial-model.md` §2.3 (`OI-ACC-03`, closed).
> **What reuse still costs is skin, not dose:** at `REQ-CVNS-04`'s maxima the commanded density on
> the declared 2 cm² is 1 µC/cm² per phase against the 40 µC/cm² ceiling, so no contact area that
> still passes 5.0 kΩ approaches it; the repeat-application hazard moves to `OI-CVNSHW-07`. **The
> message now exists on the app side:** it names the side of the neck the failing pad is on and lights
> it on a neck diagram, taking the side from the hub (`OI-ACC-07`). That makes the bilateral
> electrode-to-side mapping a fact the hub must report correctly — it waits on `OI-CVNSHW-01` — and
> leaves the two pads of a unilateral montage indistinguishable until `OI-CVNSHW-06` gives the neck
> module a geometry.
...
| Skin irritation, burns or sensitisation under a hydrogel on the neck, **including on repeat application** | No pad specification exists (§6); reuse is permitted (§6, Rev 2), so the biocompatibility basis must cover it (`OI-CVNSHW-07`) |
| Cable entrapment or tension at the neck | A neck-worn tethered accessory; no strain relief or breakaway specified |
| Reuse of a pad past its working life | Permitted by decision (§6, Rev 2); a failing pad is refused pre-enable (`REQ-CVNS-06`) and caught mid-session (`REQ-CVNS-08`). Charge density is not at stake at any passing impedance (§6). The app names the failing pad's side (`OI-ACC-07`); a wrong electrode-to-side mapping would send the wearer to the wrong pad (`OI-CVNSHW-01`) |
...
|---|---|---|---|
| **OI-CVNSHW-01** | Assign the four cable conductors and confirm the assignment supports per-electrode 1 kHz impedance measurement and 500 ms single-electrode open detection. A change to the count is regulatory-visible (`NP-REG-CVNS-001` §2.4). The assignment also fixes which side of the neck each electrode's pad is on in a bilateral montage — the side the hub reports to the app in `CVNS_PAD_STATUS` (`OI-ACC-07`) | EE | A14 tooling; 510(k) device description |
...
| **OI-CVNSHW-05** | ~~No row in `docs/reference/commercial-model.md` §2.3; single use unenforced~~ **CLOSED 2026-09-22 (principal).** Row added under `OI-ACC-03`. *"Single-use"* is a performance guarantee, not a reuse prohibition; single use is **not enforced**, because a failing pad is detected by `REQ-CVNS-06` / `REQ-CVNS-08` (§6). Repeat-application skin hazard → `OI-CVNSHW-07`; telling the wearer which pad location failed → `OI-ACC-07` (app side done) | **CLOSED** — was Product + Quality | — |
...
| **OI-CVNSHW-07** | Specify gel pad formulation, adhesion, shelf life and biocompatibility basis (hydrogel on neck skin, **including repeat application** — reuse is permitted, §6 Rev 2) | ME + Quality | Hazard analysis; 510(k) biocompatibility |
...
| 2 | 2026-09-22 | NeurOne Systems Engineering | **`OI-CVNSHW-05` CLOSED by principal decision.** *"Single-use"* is a performance guarantee, not a reuse prohibition: single use is not enforced, because a failing pad is detected by the pre-enable impedance gate (`REQ-CVNS-06`) and mid-session open detection (`REQ-CVNS-08`). The consumables row is added under `NP-ACC-PRIORITY-001` `OI-ACC-03`. §6 records the decision and shows the per-phase charge ceiling is not at stake at any passing impedance; §2 A14.3, §7's reuse and skin rows and `OI-CVNSHW-07` now cover repeat application. The app now names the failing pad's side, taken from the hub (`OI-ACC-07`), which makes `OI-CVNSHW-01`'s conductor assignment also the source of that side. **No requirement, constant or limit changed.** |
