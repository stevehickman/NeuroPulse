...
**Related Issues:** PR #283 (Rev 2 — §7.3 L3 encoding, HFE-R-16/17, OI-HFE2-10/11)
...
| OI-HFE2-02 | **CLOSED 2026-09-26 (#439).** ~~App module-status pipeline is still the retired 5-slot model: `NeurOneGATTManager.zoneModules: [UInt8] = [0,0,0,0,0]`, and `ZoneModuleConfiguration` (`app/ios/NeurOne/Models/ZoneModuleInfo.swift`) truncates with `rawSlotData.prefix(5)` and decodes each byte through the retired 5-zone `ZoneID` enum; `SetupError.zoneModulesMissing([Int])` carries slot indices. L2 needs a socket-indexed inventory over BLE. Flagged in NP-FW-ZA-001's supersession note; still not scoped~~ **Both mobile pipelines are now socket-indexed.** iOS had already been re-cut: `ZoneModuleConfiguration` is a sparse `[UInt8: ZoneModuleStatus]` map fed by the `np_zone_notify.h` v2 frames (`ZoneModuleFrame.swift`), with the socket geometry read once from `SOCKET_MAP`, and `SetupError.zoneModulesMissing` carries socket ids. Android was the live remainder: `NeurOneGattManager.zoneModules` was still `listOf(0, 0, 0, 0, 0)` decoded one byte per slot. It now ports the iOS decoder, fragment reassembler and model (`core/.../models/ZoneModuleFrame.kt`, `ZoneModuleInfo.kt`), subscribes to `SOCKET_MAP`, and rejects the retired 5-byte payload rather than misreading it. **What this does not close:** L2 itself (the confirm-and-correct loop and its instruction grammar) is unbuilt, and the required *build* map it compares against has no home (`NP-HW-EEGNET-001` `OI-EEGNET-24`) | L2 implementation | App |
...
| **OI-HFE2-10** | **Validate the §7.3 nested-figure encoding, and specifically the 3-versus-4 (open U vs closed square) discrimination — the weakest pair in the set.** Rev 2 substitutes a *shape* claim for Rev 1's *counting* claim; that is very likely favourable, but it is an empirical claim of the same kind as §7.1's counting-depth result and gets the same treatment. Run inside `OI-HFE2-03`'s formative, against **HFE-R-16/17**. **Decision consequence:** if it fails, the bar-row fallback caps the taxonomy at four types, which constrains the product — `NP-HW-EEGNET-001` §1.8 has a fifth (electrode-only) tile type under consideration, and `OI-EEGNET-21` depends on this item. **This makes a tactile-discrimination formative a predecessor of a tile-taxonomy decision** — an ordering that is correct (marking is cheap to change until the insert is cut; taxonomy is not) but is unlike anything else in the document set, and should be entered into deliberately rather than discovered late | HFE + Module tooling | **OI-HFE2-03 formative; before insert tooling** |
...
| OI-HFE2-09 | **CLOSED 2026-09-26 (#439) — already resolved when raised.** ~~Firmware `np_module_map.h` still carries `np_lobe_t` / `NP_LOBE_*` and a `lobe` field in `np_socket_geom_t`, after ZONE-1 deleted the lobe concept from the generator and web app. L2's instruction grammar must not consume it. Out of scope here; noted because it is a live inconsistency~~ `np_lobe_t` / `np_side_t`, the `lobe` field and `np_module_map_predefined()` were removed from `firmware/hub_control/include/np_module_map.h` at `OI-HUB-C14`; `np_socket_geom_t` is metric only, and the header records the removal in place. A 2026-09-26 sweep of `firmware/` (excluding `vendor/`) finds no `np_lobe_t`, `NP_LOBE_*` or `lobe` field, and the `np_zone_notify.h` wire carries no lobe or side. L2's instruction grammar has nothing lobe-shaped to consume | — | Firmware |
