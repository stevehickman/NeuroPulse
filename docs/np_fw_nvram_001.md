...
> 3. **`OI-NVRAM-05` — documentation corrected, one design decision raised.** Every comment and
>    specification clause that called an SNVS flag a power-loss recovery flag now says warm-reset
>    only. The anonymisation half needs nothing more, because Scratch is erased on every boot. The
>    factory-reset half is still a real defect, and §3.4.1 specifies the durable marker it needs and
>    recommends one of three options. **The option is not chosen here.**
...
least harmful. **Not adopted here.** The choice touches a privacy finding (HIGH-03), the warranty
owner (§6.0) and possibly §5's no-clock premise, so it is the principal's. `OI-NVRAM-05` stays open
on that choice, and still blocks `NP-MOD-ID-001` §10's factory-reset rotation test.
...
| **OI-NVRAM-05** | **Rev 3 (2026-09-26, #444): documentation corrected, anonymisation half closed, factory-reset half awaiting a principal decision.** Every comment and clause that called either flag a power-loss flag now says warm-reset only (§3.4.1 lists them). `np_factory_reset_config.h` had called the SNVS domain *"battery-backed"*, and it is not. `LPGPR2` needs nothing more, because Scratch is erased on every boot. **Still open:** a power loss during R-3…R-10 is undetected, and the state it leaves is tabulated in §3.4.1. The durable marker is specified there with three options, A (recommended), B and C. **Choosing one is the principal's decision**, because it touches NP-PRIV-001 HIGH-03, the warranty owner and possibly §5's no-clock premise. ~~`NP_SNVS_RESET_IN_PROGRESS` and `NP_SNVS_ANON_IN_PROGRESS` are documented as power-loss recovery flags and can only be warm-reset flags — the SNVS LP domain has no `VBAT` supply. The factory-reset case is the serious one: a power loss during R-4…R-9 is undetected and the device boots half-erased~~ | Principal (option), then FW + Privacy | **`NP-MOD-ID-001` §10's factory-reset rotation test** |
...
> `OI-NVRAM-05`'s documentation is corrected. Its factory-reset marker is specified with a
> recommended option (§3.4.1) and waits on the principal. One new item, `OI-NVRAM-16`, must land
> before Map 3's row can grow. **Still not built:** Map 3's owner (sync, watermarks, the §6.3
