...
**References:** NP-HEX-ZM-001 Rev 2; NP-HW-HEXTILE-001 Rev 3; NP-DRV-SHELL-002 Rev 2; NP-HW-HUB-001 Rev 3; NP-HELMET-GEOM-001 Rev 1; NP-TOOL-HUB-001 Rev 3; **NP-TOOL-SHELL-002 Rev 1** (supersedes NP-TOOL-SHELL-001 Rev 3); NP-TOOL-LENS-001 Rev 2; NP-TOOL-HEXTILE-001 Rev 1; NP-FAI-001 Rev 2 §2.1, §3.1; NP-RISK-002 Rev 2; NP-CONV-001 Rev 6 §4, §6; NP-HW-TCAP-001 Rev 2; **NP-HW-AUDIO-001 Rev 1; NP-HW-NASAL-001 Rev 1; NP-HW-VNSCLIP-001 Rev 1; NP-HW-CVNS-001 Rev 1; NP-HW-TMS-001 Rev 1; NP-HW-TACSDRV-001 Rev 1**; NP-PWR-THERM-001 Rev 1 §8; `docs/superseded/README.md`
**Related Issues:** **GitHub Issue #331 (Rev 9: OI-ART-01 discharged as to the document; succeeded by NP-TOOL-SHELL-002 OI-TSHELL2-01)**; **GitHub Issue #394 (Rev 8: OI-ART-02, -03, -06 closed; OI-ART-08 row added)**; GitHub Issue #343 (OI-ART-05); **GitHub Issue #332 (OI-ART-04 — owning specifications for A11–A15; OI-TACS-02 — the driver stage, now A16)**
...
| **A5** | **L1 inner-bowl laminate** (rigid-flex into moulded carrier) | `NP-DRV-SHELL-002` §4.1, `NP-HELMET-GEOM-001` §2 | DESIGN STUDY | ⛔ `NP-TOOL-SHELL-002` (DRAFT; carrier features requirements-only, S2-F09) — geometry blocked on **MECH-1, OI-SHELL2-02** | ✅ `NP-RISK-004` | ⛔ **MECH-1** |
| **A6** | **Outer bowl** (stations L2 + L3; *"L0" until Rev 9*, see `NP-TOOL-SHELL-002` §1.4) — CFRP shell + 4-layer EMF stack (Layer 4 absorber deleted 2026-09-23; bowl re-lofted 3 mm inward, `REQ-CAV-04`) | `NP-HEX-ZM-001` §5, `NP-TOOL-SHELL-002` | DESIGN STUDY — two-bowl tooling spec DRAFT; latch, boss, Hall and lip geometry undesigned (`MECH-1`) | ⛔ `NP-TOOL-SHELL-002` (DRAFT, Rev 9) — loft specified (S2-F02); S2-F03…F08 requirements-only, blocked on **MECH-1**; six pre-`MECH-1` inputs listed in its §5.5 | ✅ `NP-RISK-004` | ⛔ **MECH-1, EMF-1, EMF-3, RISK-20** |
...
### 2.5 Found by the `OI-ART-06` completeness audit (Rev 8, 2026-09-25)

`OI-ART-06` asked for §2 to be checked against *"the design brief BOM and the box-contents list"*,
and Rev 6 asked for a BOM/CAD parts list. **Neither instrument exists as a parts list.**
`docs/np_cost_001.md` and `NP-DB-005` cost at configuration level, with no line-item BOM, and `cad/`
holds only a head scan. The audit was therefore run against every source that does enumerate
physical parts: the Home Standard box contents (`docs/reference/commercial-model.md` §2.1), the
consumables table (§2.3), `NP-ACC-PRIORITY-001` §5's 19 items, the accessories roadmap, CLAUDE.md
§4.4's fit system, the tooling specifications' *"separate tool"* statements, and the decisions of
2026-09-20 to 2026-09-23. **It found nine artifacts with no row.** Two were already known
(`OI-ART-08`, `OI-TACSDRV-03`). Rows are added with what exists, and with the absences named. **No
governing specification is written here, and none is implied.** A row with ⛔ in every column is the
finding, as A16's was.

| # | Artifact | Source that names it | Governing spec | Spec maturity | Tooling spec | Risk register | FAI checklist |
|---|---|---|---|---|---|---|---|
| **A17** | **T2 clinical electrode cap** (21-ch wet-gel qEEG, HD-tDCS 4×1, clinical tACS). **Added for `OI-ART-08`** | CLAUDE.md §3 (T2); `NP-HW-TCAP-001` §8 | `NP-HW-TCAP-001` Rev 2 (wiring and channel assignment only) | DRAFT, no mechanical content | ⛔ none | ⛔ none (`OI-TCAP-06`) | ⛔ **F1, F2, F4.** Named here as `NP-FAI-TCAP-001`, a named absence. FAI-HD01, -HD03 and -HD04 (`NP-FW-HD-001` §12) inspect this part (`NP-FAI-001` `OI-FAI-06`) |
| **A18** | **T1 tES stimulation pads and leads** (tDCS, and BES/tACS where no T1-B pod fits) | `NP-FW-MMSOCK-001` P-1, **decided 2026-09-23** | ⛔ **none** | — | ⛔ none | ⛔ none | ⛔ F1 |
| **A19** | **40 Hz mastoid vibrotactile LRA pad** (accessory, `NP-ACC-PRIORITY-001` rank 1, class C1) | `docs/reference/accessories-roadmap.md` (provisional); mounts on `NP-TOOL-SHELL-002` S2-F01 (was `NP-TOOL-SHELL-001` F-04) | ⛔ none | provisional | ⛔ none | ⛔ none | ⛔ F1 |
| **A20** | **TEC base-station chiller** (rank 5, C2) | accessories roadmap (provisional); `NP-THERM-SINK-001` | ⛔ none | provisional | ⛔ none | ⛔ none | ⛔ F1 |
| **A21** | **Hip ice / PCM pack** (rank 15) | accessories roadmap (provisional) | ⛔ none | provisional | ⛔ none | ⛔ none | ⛔ F1 |
| **A22** | **Tethered covers**: interface protection covers, accessory port covers (ranks 10–11) and the **lens-rim guard** | box contents; `NP-TOOL-HUB-001` F-02 (anchor and ≤ 20 mm tether only); `NP-TOOL-LENS-001` F-07 (hook only; *"the lens rim guard is a separate tool"*) | ⛔ none for the covers themselves | — | ⛔ none (the anchors are in A8 and A10) | the tether-reach hazard is in `NP-RISK-004` (`RISK-HUB-01`) | ⛔ F1. The port inventory is `NP-TOOL-HUB-001` `OI-HTOOL-08` |
| **A23** | **Hard clamshell case** (in the box; *"doubles as the shipping container"*, `NP-DB-005`) | commercial-model §2.1 | ⛔ none | — | ⛔ none | ⛔ none | ⛔ F1 |
| **A24** | **Fit frame**: 5-position forehead bridge and occipital Boa assembly (Boa reel and lace are purchased) | CLAUDE.md §4.4; `NP-HELMET-GEOM-001` requirement list | ⛔ none. `NP-HELMET-GEOM-001` states requirements, not a part | — | ⛔ none | ⛔ none | ⛔ F1 |
| **A25** | **Posterior aggregation node (PAN)** | `NP-HW-TACSDRV-001` Rev 2 §4.1.1 | ⛔ none | — | — (PCB) | ⛔ none | ⛔ F1. **Recorded against `OI-TACSDRV-03`**, and conditional on `OI-TACSDRV-01` |

> **A18 is the one that matters now.** It is a shipping T1 part, and the hub treats its contact area as
> a **fixed device constant**, `NP_BES_ELECTRODE_AREA_MCM2`, the number the Class C charge interlock
> divides by (CLAUDE.md §3's per-phase and per-session ceilings, enforced against electrode area).
> `OI-CHARGE-07`'s rule makes a fixed product part's area a device property, and so the part is
> **safety-relevant geometry with no specification, no tolerance and no incoming inspection**. A pad
> made larger than the constant under-reports density, which is the unsafe direction. Raised as
> **`OI-ART-11`**.

**Scoped into a parent row, not added:** the intranasal hygiene sleeve (A12, which names it); VNS
clip pads (A13); cervical gel pads (A14); audio-cup foam and mesh frame (A11); electrode hydrogel
tips (A2's T1-B population, or A17 on T2); S1/S2 shades and the EC lens (A10); the T1-C 1064 nm
smart module (A1/A2). **Each parent specification must say so**, and whether it does is `OI-ART-12`.

**Purchased, out of scope per §1:** the braided USB-C cable, the GaN charger, the Boa reel and lace
(and the regrease kit), the cleaning brush, cloths and humidity card, and the S3 Rx insert, which an
optician supplies. **Deferred and out:** the fit-over goggle (`NP-HW-FITOVER-001`, not on the plan).
...
Three active documents were coupled to the retired zone module, and Rev 8 found a fourth. All three of the original set are now resolved (Rev 9 resolved `NP-TOOL-SHELL-001` by superseding it); `NP-PROC-SUP-001` remains. Each was left in place rather than
...
|---|---|---|---|
| ~~`NP-TOOL-SHELL-001` Rev 2~~ **→ superseded by `NP-TOOL-SHELL-002` Rev 1 (2026-09-25)** | ~~F-01 is "zone slot plug anchor posts (**×5**, colour-coded)", keyed to the retired ZM-01…05 colour scheme. Both its stated parents — `NP-DRV-SHELL-001` and `NP-TOOL-ZM-001` — are now superseded. **Corrected 2026-08-18: the coupling is deeper than this row stated.** Rev 1 of this register asserted that F-02, F-03 and F-04 are architecture-independent. That claim was made without opening the file and **two-thirds of it is wrong.** **F-03 is not architecture-independent** — its governing rule is that the EEG cables and the zone-module FPC bundle sit on opposite sides of the shell wall, and `NP-DRV-SHELL-002` §9.1 quotes that rule and calls it unavailable; §5.3 absorbs the EEG harness entirely, so the channel has no cable to route. **F-02 is architecture-independent but conflicts** with `NP-TOOL-HUB-001` F-02 on the same hub ports (50 mm tether vs a safety-derived ≤20 mm maximum). Only **F-04 is architecture-independent and clean.** | It is still the **only** tooling specification the headset shell has, which is why it stays. F-04 remains releasable, and the anchor-post/tether pattern, boss-root radius rule, IPX4-at-boss-base requirement, draft-angle floors, §4 materials and the §5 checklist structure all survive as reusable engineering. **2026-09-23: F-02 retired in place (SHELL Rev 3) — hub port covers are `NP-TOOL-HUB-001` F-02 (`OI-ART-07` closed).**~~ **Resolved.** The architecture was replaced, so under `NP-CONV-001` §4 the successor takes a new serial. F-04 is carried as S2-F01, and the reusable rules are carried in its §6. Rev 3 is at `docs/superseded/np_tool_shell_001.docx` | ~~OI-ART-01~~ (discharged as to the document; residual `OI-TSHELL2-01`) |
| ~~`NP-COORD-001` Rev 1.10~~ **→ Rev 12 (2026-09-25)** | ~~Titled *Zone Module FPC Engineering Coordination Checklist*; G1/G2/G3 gate items are scoped to an assembly that no longer exists.~~ **Resolved.** Re-issued as a gate-definition document (`np_coord_001.md`). What each `NP-DP-001` gate checks is now a per-artifact disposition over §2, and every Rev 11 item ID is dispositioned | The gate citations resolve. Rev 11 is at `docs/superseded/np_coord_001.docx` | ~~OI-ART-02~~ |
| `NP-PROC-FPC-001` **Rev 4** *(cited here as Rev 1 until 2026-09-21; the document was at Rev 3 behind a stale header)* | Specifies the Hirose FH34S 20-pin ZIF and the RA-copper requirements for a **tailed** FPC. Hex tiles have no tail (`NP-DRV-SHELL-002` §8.2 — module swap actuates zero flex). | The LED V_f binning and RA-copper requirements survive and are the only written procurement controls on the emitter supply. **Rev 4 (GitHub #333) strengthens that survival rather than narrowing it**: §2.1's ±0.10 V within-order bin is now stated as load-bearing for **string construction**, so the surviving half of this document is a premise of `NP-HW-HEXTILE-001` §8.1.1's fixed-N strings and not only a current-matching control. §2.6.2's emitter shortlist is corrected but **still selects nothing** — `OI-HEXTILE-02` is open, so `NP-FAI-HEXFPC-001` (§3.2, artifact A2) stays unwritable. | ~~OI-ART-03~~ **Resolved 2026-09-25: split in place at Rev 8** (it was at Rev 7, not Rev 4). Part A (emitters, RA copper, PDMS) is live, and Part B (BCR421W, FH34S, tail gold, dynamic bend, zone-FPC current sizing) is retired and retained. `NP-PROC-FPC-1064-001` Rev 3 likewise |
| `NP-PROC-SUP-001` | **Added Rev 8, found by the `OI-CONV-08` `.docx` pass.** SUP-M-06 (5-colour zone coding), SUP-A-03 (braille dots), SUP-B-04 (per-zone FPC routing channels) and SUP-B-05 (the EEG cable channel) qualify suppliers for features that no longer exist. SUP-M-07's Ra ≤ 1.6 µm is still live (`RISK-20`) | It is the supplier-qualification checklist for every live category too, and SUP-M-07 / SUP-B-01 are BLOCKING | **OI-ART-10** |
...
|---|---|---|---|
| ~~OI-ART-01~~ | **✅ DISCHARGED 2026-09-25 (GitHub #331) as to the document, and succeeded by `NP-TOOL-SHELL-002` `OI-TSHELL2-01` for the geometry.** *Superseded, not re-scoped:* the architecture was replaced, so `NP-CONV-001` §4 gives the successor a new serial, and `NP-TOOL-SHELL-001` Rev 3 moves to `docs/superseded/`. `NP-TOOL-SHELL-002` Rev 1 (DRAFT) dispositions all four predecessor features, carries F-04 as S2-F01, specifies the 3 mm re-loft with no absorber ply (S2-F02), and lists the latches, Hall interlock, boss, lip, coil formers and TMS window as requirements-only features. **The ~80-socket cover question was already answered:** blanking plugs on the hex footprint (`NP-HELMET-GEOM-001` §3.1; FMEA-G03-02), retained by the socket, so no anchor post per socket is needed (`NP-TOOL-SHELL-002` §4). Its §5 registers every `MECH-1` input, six of which must close first, and raises **`OI-TSHELL2-02`: the layer-closed interlock has no electrical owner**. **What the item said could not happen before MECH-1 still cannot:** no geometry is invented, and A6's tooling column stays ⛔. *The item read:* Re-scope `NP-TOOL-SHELL-001` to the two-bowl L0/L1 shell, or supersede it with `NP-TOOL-SHELL-002`. F-01's five colour-coded zone-slot plugs have no referent; the hex lattice has ~80 identical sockets, and whether unpopulated sockets need plugs at all is an open design question, not a renumbering. Cannot be completed before **MECH-1** fixes the clamp and boss geometry. **The re-scope also carries the Layer 4 deletion (2026-09-23, `REQ-CAV-04`, GitHub #391; tracked on #331): the outer bowl is re-lofted 3 mm inward and has no absorber lamination — the re-loft is BINDING with the deletion, because vacating the station without it makes the outward thermal path worse (0.410 → 0.450 instead of → 0.335).** `NP-HELMET-GEOM-001` §2 carries the new radial stack (27–32 mm). | — (discharged) | — |
| ~~OI-ART-02~~ | **✅ CLOSED 2026-09-25 (GitHub #394) by the second branch, and with `OI-CONV-03`.** `NP-COORD-001` Rev 12 is a gate-definition document. The gates stay `NP-DP-001`'s, what each review checks is a per-artifact disposition over §2, and every Rev 11 item ID is dispositioned in its §4. Its residuals are `OI-COORD-01…04`. *The item read:* Re-scope `NP-COORD-001`'s gate structure from the zone-module FPC assembly to the artifact set in §2, **or** demote it to a gate-definition document and move the per-artifact items into the artifacts' own documents. Resolve alongside **OI-CONV-03** (its revision scheme) since both require re-issuing it. | — (closed) | — |
| ~~OI-ART-03~~ | **✅ CLOSED 2026-09-25 (GitHub #394).** `NP-PROC-FPC-001` Rev 8 is split in place: Part A is live and Part B retired and retained, each retirement keyed to an `NP-RISK-002` §3 disposition (`editscripts/patch_art03_proc_fpc_split.py`). `NP-PROC-FPC-1064-001` Rev 3 likewise. The split surfaced `NP-HW-HEXTILE-001` `OI-HEXTILE-26`: the fitted photodiode's published range, 0.9–1.7 µm, may exclude 660 and 808 nm. *The item read:* Split `NP-PROC-FPC-001` into the emitter-supply controls that survive (V_f binning, RA copper, certificates) and the tailed-FPC connector controls that do not. `NP-PROC-FPC-1064-001` needs the same treatment — its module-count and BOM figures already carry a supersession banner. | — (closed) | — |
...
| **OI-ART-08** | **Row added at Rev 8 as A17 (§2.5), and its checklist named: `NP-FAI-TCAP-001`, a named absence.** What remains is the rest of `OI-TCAP-06`: a mechanical specification, a per-artifact risk register (F4) and a gate. *The item read:* **The T2 clinical electrode cap is a manufactured artifact with no row in §2.** `NP-HW-TCAP-001` (Rev 1 §8, carried at Rev 2) says so itself — *"the cap artifact has no register entry"* — and its absence has a concrete consequence this register is supposed to prevent: FAI-HD01's phantom limb, FAI-HD03 and FAI-HD04 (`NP-FW-HD-001` §12) inspect the cap, and with no register row there is no risk register to satisfy `NP-FAI-001` §2 F4 and no artifact to name a checklist after. Surfaced by the OI-ART-05 disposition rather than by §2, which is `OI-ART-06`'s point exactly: an artifact absent from the source lists does not appear here. Add the row (with A-number, governing spec, maturity, and the three document columns), then name its checklist from the register. Resolve with **OI-TCAP-06** and **OI-ART-06**. | Systems | T2 verification planning; `NP-FAI-001` OI-FAI-06 |
| ~~OI-ART-06~~ | **✅ CLOSED 2026-09-25 (GitHub #394) — audited, and nine artifacts found (§2.5).** The instrument it named does not exist: no line-item BOM or CAD parts list is in the repository. The audit therefore ran against every source that enumerates physical parts, as §2.5 lists. **Re-run it when a BOM or CAD tree exists**, which is `OI-ART-12`'s second half. *The item read:* Confirm this register is complete. It was assembled from `CLAUDE.md` §3–§4 and the current specification set; an artifact that appears in neither would not appear here. Verify against the design brief BOM and the box-contents list at §2.1. | — (closed) | — |

| **OI-ART-10** | **`NP-PROC-SUP-001` is architecture-coupled and was not in §5** (Rev 8). Re-scope SUP-M-06, SUP-A-03, SUP-B-04 and SUP-B-05 against the hex-tile shell and the L1 laminate, or retire them in place. Keep SUP-M-07 / SUP-B-01 (`RISK-20`), which are live and BLOCKING | Procurement + Quality | Supplier qualification for A5/A6 |
| **OI-ART-11** | **A18, the T1 tES pads, is safety-relevant geometry with no specification** (Rev 8). The Class C charge interlock divides by `NP_BES_ELECTRODE_AREA_MCM2`, a fixed device constant under `OI-CHARGE-07`, and the pad whose area it is has no drawing, tolerance, material or incoming inspection. A larger pad under-reports charge density. Write the pad's governing specification, **with the area tolerance on the safe side of the constant**, and a risk-register entry | EE + Safety | **T1 BES/tDCS release**; A18 F1/F4 |
| **OI-ART-12** | **Confirm the §2.5 "scoped into a parent row" list, and re-run the audit against a BOM** (Rev 8). Each parent specification (A2, A10–A14) must name the consumable or sub-part it is said to own. When a line-item BOM or CAD tree exists, re-run §2.5 against it | Systems | Register integrity |
...
|---|---|---|---|
| 9 | 2026-09-25 | NeurOne Systems Engineering | **GitHub #331: `OI-ART-01` discharged as to the document.** `NP-TOOL-SHELL-001` is superseded by `NP-TOOL-SHELL-002` Rev 1 (DRAFT) and moved to `docs/superseded/`. The A5 and A6 tooling columns now name it, and both stay ⛔ on `MECH-1`. **A6 is relabelled "outer bowl"**, because under `NP-HELMET-GEOM-001` §2, the owning radial stack, L0 is the scalp face (`NP-TOOL-SHELL-002` §1.4). A19's mount now cites S2-F01. §5's SHELL row is struck as resolved. No readiness status changes and no FAI becomes writable. |
| 8 | 2026-09-25 | NeurOne Systems Engineering | **GitHub #394: `OI-ART-02`, `-03` and `-06` closed; `OI-ART-08`'s row added; nine artifacts found missing and registered (§2.5).** §5: `NP-COORD-001` resolved by Rev 12, and `NP-PROC-FPC-001` split at Rev 8 (it was at Rev 7; this register had cited Rev 4). `NP-PROC-SUP-001` added as a fourth coupled document (`OI-ART-10`). §2.5 records A17–A25. The completeness audit ran against box contents, consumables, the accessory priority set, the fit system and the tooling specifications' separate-tool statements, because no line-item BOM or CAD parts list exists. **A18 (T1 tES pads) carries the interlock's area constant and has no specification** (`OI-ART-11`). Raises `OI-ART-10…12`. No readiness status of A1–A16 changes, and the FAI count stays 1 of what is now 25. |
| 7 | 2026-09-23 | NeurOne Systems Engineering | **A6 and `OI-ART-01` updated for the Layer 4 deletion** (`REQ-CAV-04` taken, GitHub #391). A6 now reads *4-layer*; `OI-ART-01`'s `NP-TOOL-SHELL-001` re-scope carries the binding 3 mm outer-bowl re-loft and the removal of the absorber lamination. No readiness status changes — A6 was already blocked on `MECH-1`, `EMF-1`, `EMF-3` and `RISK-20`. |
