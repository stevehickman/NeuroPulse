...
| **Zone module keying — REDESIGNED 2026-07-28, replaces RISK-15 five-layer scheme** | Included in tooling | All sockets share one identical shape (SMART-1 — no per-position or per-type mechanical key). Two controls instead: an orientation-only mating feature (not rotationally symmetric where pin alignment requires it) and software placement validation (`np_module_map` UID inventory, blocks a protocol from running against a mismatched module map). **Open gap:** braille/raised-numeral and tactile-dot position-ID (old Layers 3–4) were built for 5 positions, not ~30–80 sockets — need a new design, not a bigger version. **Bone-conduction audio (old Layer 5) can't do this job at any socket count** — it needs head contact, and inserting a module needs the helmet off-head; redirect the confirmation to the companion phone/tablet/computer app instead, which already gets module status over BLE during setup. Covers color-blind and blind users — that requirement doesn't change. See `docs/np_hfe_001.md` CT-01, `docs/superseded/np_fw_za_001.md`. |
