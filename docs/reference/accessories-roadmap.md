...
- **Status:** PROVISIONAL. On the roadmap, priority as above — now placed against the whole set:
  **chiller rank 5 of 19** (class **C2**, binding relief — the only C2 item in the document set) and
  **ice pack rank 15 of 19** (class **C4**, envelope), `NP-ACC-PRIORITY-001` §5. D-3's own reasoning
  ...
  options, the Watch app and the deferred fit-over goggle below it. `D-2` (whether the sealed pneumatic loop is in scope at all) is
...
## Fit-Over Goggle Assembly (DEFERRED — not on the development plan)

A goggle assembly sized to be worn over the user's own glasses. **Specified so the option is on the
record; deliberately not being built.** Full concept specification, including the requirements and
what it would cost in currently-locked constraints: `docs/np_hw_fitover_001.md`
(`NP-HW-FITOVER-001` Rev 1).

- **The decision (principal, 2026-09-14):** users who want correction while wearing the goggles
  **buy prescription lenses for the goggle assembly** — the S3 Rx path, already supported by the
  Tier A optician network. **There is no intent at this time to build a fit-over assembly**, and it
  will not be implemented without specific user requests for it.
- **Priority: rank 19 of 19 — bottom of the set**, class **C5**, `NP-ACC-PRIORITY-001` §5. It
  unlocks nothing: access to the visual modality already reaches this cohort via the S3 insert, so a
  second route to it is convenience. It is also the highest-engineering-cost item in the set.
- **Gate: EXTERNAL — demand.** Recorded, never ranked (`NP-ACC-PRIORITY-001` §3.3).
- **Two findings make this a design decision rather than a scheduling one**, and both are safety
  rather than sizing. A user-supplied spectacle lens sits **inside** the 940 nm IR eye-open interlock
  path — where a snap-on shade never does, because shades mount outboard and the sensor looks inboard
  — so an unknown coating could read as a **false eye-open**, the permissive direction on a Class C
  interlock. And Mode F meters a retinal NIR dose through an optic whose 808–830 nm transmission is
  undisclosed and uncharacterised, which is a metered *emission*, not a metered dose;
  `NP-HW-FITOVER-001` §5.2 recommends **withdrawing Mode F** on such an assembly. The deferred
  alternative is therefore **capability-negative** against the committed one.
- **It forecloses nothing by waiting** — a separate assembly needs no first-cut provision, unlike the
  mastoid-pad anchor boss above. Deferring costs nothing that cannot be recovered.
