...
- Layer 2: 0.2mm mu-metal liner (15–25dB ELF magnetic) — PETG laminate encapsulation, silicone RTV sealant at all cutout edges.
  **What actually holds this layer in place is NOT the ELF-attenuation figure above** (`NP-BIB-EMF-001` §4 — ambient ELF at WHO residential levels is 2,000–18,000× below any reported effect, and EEG, unlike MEG, does not require external magnetic suppression). **Four dependencies do, and they are load-bearing in different ways:**
  ...
  | **D2** | **L2 is the degraded-mode fallback** | `NP-ENV-OPRANGE-001` §2 makes the active-cancellation envelope **SOFT** *because* *"fluxgate temp drift degrades cancellation; passive 4-layer shield always present."* (read *5-layer* before 2026-09-23 — the Layer 4 deletion does not touch D2: the absorber never contributed to the ELF figure that sentence is about) Remove L2 and **a soft bound becomes a hard one**, re-opening `NP-ENV-001`'s operating range | `NP-ENV-OPRANGE-001` §2 |
  | **D3** | **L2 carries the shell's magnetic-continuity architecture** | Magnetic shields leak at butt-joints, which is why all shielding is consolidated onto **one unbroken outer bowl**. `NP-HELMET-GEOM-001` §89 routes L2 around the TMS window; `NP-THERM-COOL-001` §6.2 and `OI-THCOOL-06` (closed 2026-08-30, **REOPENED 2026-09-20 against the collar** and BLOCKING again) exist to protect its reluctance at the posterior boss | `NP-HEX-ZM-001` §5.2, §5.3(d) |
...
- ~~Layer 4: Carbon-loaded EMI absorber foam~~ — **DELETED 2026-09-23 (`REQ-CAV-04` taken, GitHub #391). The number is retired and held: Layer 5 is never renumbered**, because every historical citation of "Layer 4" means the absorber. `REQ-CAV-02` (loaded Q ≤ 20 over 420 MHz – 3 GHz, ≥ 26.2 dB at resonance, `NP-EMC-CAV-001` §5) still stands as the cavity requirement — the layer supplied 0.26 dB of it and **the wearer's head supplies 49.8 dB**. **The 3 mm re-loft of the outer bowl is BINDING with the deletion** (`NP-HELMET-GEOM-001` §2): the Pd liner and everything outboard move 3 mm inward, the Gap is unchanged, the outward thermal path goes **0.410 → 0.335 m²K/W** (0.450 had the station been vacated without it), and the radial stack goes **30–35 → 27–32 mm**. The cluster clamp now reacts against rigid Pd/mu-metal/CFRP, and its tolerance stack shrinks **±1.30 → ±0.80**. **Only documented return path:** `OI-EMCCAV-08` (`MECH-2`) — if the spring plungers cannot cover ±0.80, a thin electrically insulating, non-magnetic ceramic-filled pad (`REQ-CAV-03`) sized by that stack, never an absorber
...
> seams and penetrations that set the real floor. (**`OI-THCOOL-06`** was closed 2026-08-30 — `NP-BIB-EMF-001` then carried it as open in error — and
> was **REOPENED 2026-09-20 against the posterior-boss collar**, where it is **BLOCKING on MECH-1
> cutting the boss** now that **`BOSS-1`** commits to an outward emboss.) **What the stack is for, and which layers earn their
...
> - **Layer 4 had a dB figure, did not meet it, and is deleted (2026-09-23).** `OI-BIBEMF-08` is **discharged** by
...
>   supplied **0.26 dB** of that: a 3 mm non-magnetic absorber against a conductor is λ/217 at the
...
>   So the layer cost 18 % of the outward thermal path and two fixes to a BLOCKING `OI-SINK-01` for
...
>   it, **0.335**, the best figure available. **Taken by the principal 2026-09-23 (GitHub #391)**, while no tooling
>   was cut and nothing was published. The residual is mechanical, not EMC (`OI-EMCCAV-08` /
>   `MECH-2`): if the spring plungers cannot take up the ±0.80 tolerance stack, a **thin
>   ceramic-filled pad sized by that stack** (~0.355) returns to the station. `NP-EMC-CAV-001` §8.2.
