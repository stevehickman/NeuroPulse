...
| Audio cup foam (set) | $24/set | **150 sessions — unvalidated placeholder (`OI-ACC-04`; gated on `OI-AUDIOHW-05`/`-06`, and the count has no firmware producer yet — `OI-ACC-08`)** | 58% | Exposure count. Mechanism: compression set under wear (time-under-compression; sessions proxy it). Cannot see tear, contamination, or storage set. Was "6–12 months / Calendar reminder" — `OI-ACC-02`, resolved above. |
