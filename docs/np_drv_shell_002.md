...
**Supersedes:** NP-DRV-SHELL-001 Rev 2 (architecture replaced, not revised — see §1.2)
...
>
> Rev 1 was written on 2026-07-29 and has had one commit since. In the same window
> `NP-HW-HUB-001` reached Rev 3, `NP-HW-HEXTILE-001` reached Rev 2, and four principal decisions
> landed. Five Rev 1 positions do not survive that traffic, and other documents had begun to
> inherit them.
>
> | # | Rev 1 position | Rev 2 position | Basis | Where |
> |---|---|---|---|---|
> | 1 | Cluster carrier is **passive** — I2C segment switch, TIA/mux/ADC, electrode mux, power gate, no MCU | Cluster **controller** — adds an **STM32G071 (UFQFPN32)**, PCA9548A, 16:1 PD current mux, one shared switched-gain TIA (DG2788A), 8:1 NTC mux + ADC, cluster power gate | Principal direction 2026-08-11: NP-HW-HUB-001 Rev 3 §3.1/§8.3 prevails. **HUB-001 §3.2's own justification is NOT carried forward** — see §3.2a | §3.2, §3.3, §5, §7, §10.1 |
...
>
> **The coupling change 1 creates, stated rather than buried.** Rev 1's passive carrier — and
...
> the rule *loss of any one contact must still leave ≥2× derating* → **19**. Rev 1's 18, D-5's 16 and
...
> rather than advisory, and the clamp-plate load — despite two more contacts — lands *below* Rev 1's,
...
`NP-DRV-SHELL-001 Rev 2` specified the routing path for **five** discrete zone-module FPC
...
### 1.2 Why a new document number rather than `NP-DRV-SHELL-001 Rev 3`
...
- **Same number, new revision** is used when the parent's decisions are *inherited*. `NP-HW-FPC-001
  Rev 5` is a variant layered on Rev 4 — "all base module FPC decisions … are inherited unchanged.
...
> **Terminology, Rev 2.** Rev 1 called this board the **cluster carrier**. It is now a **cluster
...
**⚠ Rev 1 said 12. The count is 18.** Rev 1 computed `ceil(80/7) = 12` from CLUSTER-1's flower alone.
...
18 does not fit the 16 positions Rev 1 provisioned, and §7.1 resolves that rather than renumbering
around it.

### 3.2 Cluster controller board contents

**⚠ Rev 1 specified this board as passive — explicitly "no MCU". It is now an active controller.**
Principal direction, 2026-08-11: where Rev 1 and `NP-HW-HUB-001` Rev 3 disagree on the cluster tier,
HUB-001's analysis prevails. Rev 3 §3.1 and §8.3 specify a distributed **cluster-controller** tier
with one **STM32G071 in UFQFPN32** per board.

| Function | Part class | Qty per controller | Note |
|---|---|---|---|
| **Cluster MCU** | **STM32G071, UFQFPN32** | **1** | **NEW in Rev 2.** I2C slave to the hub, I2C master to the PCA9548A, 12-bit ADC, timers. Rationale §3.2a; part choice HUB-001 §8.3 |
...
| **PD current mux** | **16:1 (2 × TMUX1308 class)** | **1 set** | PD1 + PD2 of ≤8 sockets into one TIA. Rev 1 said "mux" without sizing it |
| PD front end | **one shared switched-gain TIA (DG2788A + zero-drift op-amp)** | 1 | Per-*cluster*, per-sample gain — not per socket (§3.3) |
| **NTC mux + ADC** | **8:1 mux → cluster-MCU ADC** | **1** | Rev 1 folded this into "TIA + mux + ADC"; it is a separate chain |
...
**That premise is false, and it was falsified after Rev 3 was written.** `NP-HW-HEXTILE-001`
...
and the tile's own ATtiny generates its PWM. HUB-001's own **OI-HUB-C15** already instructs Rev 3 to
*"drop the cluster-MCU LED-drive rationale in §3.2"*. **This document does not carry it forward, and
readers should not reconstruct it from Rev 3's §3.2 as written.**
...
- **The MCU is a consequence of keeping the AFE, not an independent addition.** If `D-4` had won and
  ...
  gate — and Rev 1's "no MCU" model would have been correct. The two decisions are one decision;
...
`NP-HW-HUB-001 Rev 2` §3 puts one Vishay DG2788A per zone slot on the Hub PCB, switching TIA
...
1. **Gain selection stops being a GPIO problem.** Rev 2 needed one `GAIN_SEL[n]` line per slot.
...
2. **The high-impedance node shrinks from ~200 mm to ≤50 mm**, which is the actual reason to do
   it. Rev 2's own §8.1 layout note asks for `< 5 mm` from switch to op-amp on the *Hub PCB*; that
...
### 3.4 Control network — the tree, and why the Rev 1 tree cannot reach 18
...
**⚠ Rev 1's tree tops out at 16 clusters and there are 18.** Rev 1 specified:
...
`8 branches × 2 clusters × 8 sockets = 128` — which is exactly `NP_HEXMAP_MAX_SOCKETS`, and Rev 1
made much of that agreement. **The socket ceiling is still 128 and is unaffected. The middle term
is what breaks:** `8 × 2 = 16 < 18`. Reaching 18 in Rev 1's tree needs either a third branch tier or
3-deep branches, and Rev 1 provided neither. This is the C2 half of **OI-HEXTILE-14**.
...
3. **The `= 128` coincidence Rev 1 relied on is gone, and it was never load-bearing.** Under D-7 the
   ...
   does not need the bus tree to reproduce it. Rev 1 presented the agreement as corroboration; it
...
This section replaces Rev 1's 18-contact table. It is the one place where this document and
...
| Signal | Net | Rev 1 (this doc) | HEXTILE **D-5** §7.2 | Agreed? |
|---|---|---|---|---|
...
| `SCL` | N2 | 1 | 1 (pin 11) | ✓ |
| `SYNC` | N2 | **1** | **absent** | ✗ |
| `ALERT#` | N2 | 1 | 1 (pin 12) | ✓ — *HEXTILE Rev 2 called this `/ALERT`; **`ALERT#` adopted**, §5.1.7* |
...
**(b) `SYNC` — Rev 1 is right, and HEXTILE has an unowned gap here.**
...
**(c) `DGND` — Rev 1 is right, for a reason Rev 1 did not give.**
...
**(d) reserved ×2 — Rev 1 is wrong. Dropped.**
Rev A held two contacts as *"OTA-extensibility margin"*. Two arguments against, either sufficient.
First, the extensibility Rev 1 named — the `§4a` "grow-to-4" tile type — is **already served**:
...
|---|---|---|---|---|
| 1 | `VLED+` | **3** | N1 | **Principal decision, 2026-08-11** — neither Rev 1's 2 nor D-5's 4. Sized on the *degraded* case, §5.1.5 |
| 2 | `PGND` | **3** | N1 | Paired 1:1 with `VLED+` — REQ-EMI-06 needs full broadside overlap. **LED return only** |
| 3 | `V3V3` | 1 | N1 | Agreed. ≤2 mA standby / ≤25 mA active (HEXTILE §8.3 — *not* Rev 1's ≤50 mA) |
...
| 14 | `ELEC_SHLD` | 1 | N4 | DRL-driven shield for contact 13. **Renamed from Rev 1's `GUARD` — §5.1.7** |
| 15 | `SEAT#` | **1** | — | **Adopted from D-5** — §5.1.3(a). Last to mate |
| | **Total** | **19** | | |

> **19 is the closed count.** Rev 1's 18, HEXTILE D-5's 16, and `NP-HW-HUB-001` §7.5.2's 14–15 are
> all superseded. **`NP-HW-HEXTILE-001` §7.1–7.2 has been co-revised — Rev 3 (2026-08-11) adopts
...
**No UID EEPROM either — a Rev 1 BOM line that D-3 deletes.** Rev 1 budgeted a 24AA02UID on
...
> **DECIDED: 3 `VLED+` + 3 `PGND`.** Neither Rev 1's 2+2 nor `NP-HW-HEXTILE-001` D-5's 4+4. The
...
This is where the 18-vs-16 gap actually lived: HEXTILE allots **4** `VLED+` and **4** `PGND`; Rev 1
...
>
> **(2) It is still below Rev 1's own stated load, because the plate shrank.** Rev 1's
...
> worst-case plate load ~14 %, and **34.2–57.0 N at 19 contacts on a 6-tile plate is below Rev 1's
> 37.8–63.0 N at 18 contacts on a 7-tile plate.** The contact-count increase is more than paid for
> by the partition correction — net, the clamp actuator sees *less* load than Rev 1 specified, not
> more.
>
...
**⚠ Rev 1 computed this table at a 12 V estimate. The rail is 24 V.** `OI-HUB-C17b` **ADOPTED**
...
2. **The 2-contact `VLED+` budget and the 12 V assumption were mutually inconsistent, and adopting
   ...
   derating**. At 24 V the same 2 contacts carry 0.52 A, ~2× derating. Rev 1's contact count was
   only ever viable at a rail Rev 1 had not adopted. §5.1.5 carries the residual question, which is
...
3. **The per-cluster feed cannot be sized at 1/18 of the vault — it must be sized at the whole
   vault.** Rev 1's "7 tiles hot → ~1.75 A" assumed the hot tiles spread across clusters.
   ...
   relative to Rev 1's reading even though the current halved, and it is why the vault-wide budget
   must be enforced in firmware (**OI-HEXTILE-09**, the global concurrent-power governor) and not
   merely assumed from the partition.

**Reconciliation note, flagged not papered over.** Rev 1's ≤3 W per-tile allocation sits *below*
`NP-HW-HEXTILE-001` §9.2's 6.25 W average for a dual-channel tile at the 25 % duty ceiling (25.0 W
instantaneous). The two figures answer different questions — Rev 1's is a sustained thermal share,
...
partition; Rev 1 said 7, which no cluster reaches), not the whole bowl. This is the concrete advantage over Option 2 and it should
...
> classes named in §3.2 and unit costs in NP-HW-HUB-001 Rev 2 §7 / NP-HW-FPC-001 Rev 5 §6.2. They
> are not quotations and must not be carried into a costed BOM without EE Lead confirmation
> (**OI-SHELL2-05**).

**⚠ Rev 1's total ($125–216) was computed at 12 passive carriers. Both inputs changed.** The
controller tier is now costed from `NP-HW-HUB-001` §8.1's component-level derivation × 18, which is
the same **$114.12** that `NP-HW-HEXTILE-001`'s Rev 2 banner records — the two documents agree.
...
2. **Rev A understated it by ~40 %, and both corrections push the same way.** The cluster count rose
   ...
   Rev 1's *"$1.30–2.40 per socket"* is superseded. This is stated because Rev 1's number has been
   quoted downstream.
3. **Where it lands still matters, and that argument survives intact.** The cost is concentrated on
   ...
   single board. `NP-HW-HUB-001` §8.4 makes the same point quantitatively: naively scaling Rev 2 to
...
programme is defending. Rev 1's §4.1 line that gap-facing components are *"out of the scalp thermal
...
| SH2-DRC-02 | Cluster count within the D-7 segment budget and the provisioned connector positions at the final lattice *(Rev A read "≤16 (8 branches × 2)" — that ceiling is retired with the Rev 1 tree, §3.4)* | Count | **≤32 segments and ≤20 connector positions**; 18 at the v1 lattice | ME/EE |
...
*(33 items at Rev 2; 28 at Rev 1; the retired document carried 23.)*
...
- **Lattice, clusters, two-bowl shell, SMART-1:** `docs/np_hex_zm_001.md` (NP-HEX-ZM-001 Rev 1)
  §3.4, §4a, §5.1–5.4a, §7
...
- **Bezel height:** `docs/np_therm_bezel_001.md` (NP-THERM-BEZEL-001 Rev 1) §4.5 — 1.0 mm, the only
  calculated value; the competing 2.5 mm is undrived (§4.1 note)
- **Inter-bowl thermal path:** `docs/np_therm_cfd_c2_001.md` (NP-THERM-CFD-C2-001 Rev 1) §7 —
...
- **Superseded hub design (TIA topology and saturation analysis, still valid per socket):**
  `docs/np_hw_hub_001.md` (NP-HW-HUB-001 Rev 2, retained as Appendix A) §2, §3.1–3.3
- **Superseded module FPC (dual-PD architecture, InGaAs PD choice, on-module driver):**
  `docs/superseded/np_hw_fpc_001.md` (NP-HW-FPC-001 Rev 5) §5, §6
- **Optical resolution floor (why sub-module addressing buys little at depth):**
  `docs/np_opt_psf_001.md` (NP-OPT-PSF-001 Rev 1)
