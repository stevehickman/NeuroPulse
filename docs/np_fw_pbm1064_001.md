...
**Supersedes:** NP-FW-PBM1064-001 Rev A
...
> **⚠ SUPERSEDED (2026-07-28) — zone-slot addressing retired, do not use for new firmware work.** This spec indexes everything (calibration coefficients, I2C buses, ZONE_ID thresholds) by a fixed set of 5 zone slots (ZM-01..05). `NP-HEX-ZM-001` replaced that with ~30–80 sockets, addressed by `np_module_map` UID-based auto-inventory, not ZONE_ID resistor detection. Note that Rev B was authored 2026-07-27 — *after* NP-HEX-ZM-001 (2026-07-15) — and still didn't catch this; the retirement had not propagated here.
>
> **Retired outright:** `K_PD1[0..4][0..2]`/`K_PD2[0..4][0..2]`/`K_ratio_nom[0..4][0..2]` indexed "5 zones × 3 wavelengths × 3 coefficients = 45 values" (§6.6) — needs re-indexing by socket, and the socket count itself isn't locked yet (REG-1); "Hub enables LPI2C3 per slot independently (5 separate buses)" — retired per SMART-1 (every socket now I2C-capable, needs a different I2C fan-out design, not 5 fixed buses); the ZONE_ID 3.3kΩ threshold framing (§4) — retired detection mechanism.
>
...
Determines the per-unit `K_PD1[zone][wl]`, `K_PD2[zone][wl]`, and `K_ratio_nom[zone][wl]` coefficients (§6.2) and writes them to the Config partition at manufacture, replacing the firmware-default fallback (`cal_source = NP_CAL_DEFAULT`) with `cal_source = NP_CAL_FACTORY`.
...
3. Compute `K_PD1[zone][wl] = irradiance_reference_mW_cm2 / PD1_counts` and `K_PD2[zone][wl]` analogously.
4. Compute `K_ratio_nom[zone][wl] = PD1_counts / PD2_counts` at this reference condition — the factory-nominal ratio against which future fouling/aging drift (§6.3) is measured.
...
7. Write `K_PD1[0..4][0..2]`, `K_PD2[0..4][0..2]`, `K_ratio_nom[0..4][0..2]` (5 zones × 3 wavelengths × 3 coefficients = 45 values) to the Config partition, and set `cal_source = NP_CAL_FACTORY`.
