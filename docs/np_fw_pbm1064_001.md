...
7. Write `K_PD1[0..2]`, `K_PD2[0..2]`, `K_ratio_nom[0..2]` (3 wavelengths × 3 coefficients = 9 values, **per module UID** — not per zone/socket, per NP-HW-HUB-001 Rev C §9.5 and OI-HUB-C06) to the Config partition's UID-keyed calibration cache, and set `cal_source = NP_CAL_FACTORY`. (Originally specified as `K_PD1[0..4][0..2]` etc. — a 45-value table indexed by the 5 retired zone-module slots; that indexing was correct for permanently-assigned modules but is wrong under hex tiling, where the same physical module can occupy any socket. See the supersession banner at the top of this document.)
...
| Element | Notes |
|---------|-------|
| Per-socket, per-wavelength dose (J/cm²) | Written at session end per NP-FW-EMMC-001 Rev A §12 |
...
| OI-PBM-04 | ~~Factory calibration procedure: integrating sphere @ 1064nm → Config partition~~ **CLOSED Rev B (§6.6)** | Procedure specified; FAI-SM-06/07/08 hardware bench execution still pending |
...
| OI-HUB-C06 | Calibration coefficients (K_PD1/K_PD2/K_ratio_nom) keyed to module UID via `np_module_map`, not socket — sizing fixed Rev C (§6.2/§6.6), keying still open | Dose-metering accuracy claim |
