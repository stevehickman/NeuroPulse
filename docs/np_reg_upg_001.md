...
> and the module carry-over as invariants (CLAUDE.md Rev 52).
...
### 7.6 Documents changed by Rev 2

| Document | Change |
|---|---|
| `CLAUDE.md` (Rev 51 → 52) | §1 records the upgrade model, module carry-over and the T1 refusal as invariants. The Document Map gains this document |
...
### 7.7 Implementation (Rev 3) — what `OI-UPG-01` built, and what it did not

**Built: the stimulation gate (`REQ-UPG-01`, first sentence) and the write-once identity
(`REQ-UPG-02`), in firmware.**

| Piece | Where | What it does |
|---|---|---|
| **Tier-identity record** | STM32G071 OTP, offset `0x40` (`np_safety_config.h`) | 72 bytes: magic `NPTI`, version, tier code (T1 `0x01` / T2 `0x02`), two reserved bytes, and an Ed25519 signature over `"NeurOne.TierId.1"` ‖ those 8 bytes ‖ **the part's 96-bit factory UID**. OTP can be programmed once and never erased, and **the firmware contains no OTP write path**, so no field route writes it (`REQ-UPG-02`). The UID in the signed message makes it **device-bound** (D-9): a genuine T2 record copied into another unit names the wrong UID and fails there |
| **Tier authority key** | Compiled into the signed safety image (`NP_TIER_AUTHORITY_PUBKEY_INIT`) | **Not** the OTP root session key. That key's holder signs every session descriptor, and a party able to sign sessions must not thereby be able to sign a tier. Compiled in rather than read from OTP, so that nothing on a blank unit's OTP can nominate its own authority. **Today it is an all-zero placeholder, and that fails closed**: no record verifies, so every unit is T1 (`OI-UPG-08`) |
| **SW01-M10 `np_tier_identity`** | `firmware/safety_mcu/src/np_tier_identity.c` (Class C) | At power-on, verifies the record once. **Only a record that verifies, for this UID, naming T2 makes the unit T2.** An image with no key, a blank window, a malformed record, a bad signature, a record from another unit or a signed T1 record all mean T1. The verdict is held as a pattern word and its complement, so a RAM bit-flip reads as not-T2. Every main-loop iteration it strips `NP_SAFETY_EN_T2_MASK` (cervical VNS, TMS, 1170 nm, clinical tACS / HD-tDCS) from the granted mask. It runs after the watchdog tick, the only place a grant is made, and before charge accumulation and the GPIO write. **It withholds, it does not fault:** T1 lines requested alongside stay granted, as the cardiac interlock withholds only cervical VNS |
| **Tier report** | MISO window bytes 20–25 (`np_safety_tier_report_t`, `np_spi_wire_types.h`) | The tier, the reason code, and whether a T2 line was withheld on this beat. **The reason code makes a blank unit visible:** it behaves exactly like a signed T1 unit, and only final acceptance can tell them apart. `REFUSED` records what someone tried to run, so it goes to the user's own app and **never to SHDR** |
| **Hub admission** | `np_protocol_tier_admit()`, called by `np_runner_load()` | Refuses a protocol naming any T2 modality on a unit the MCU does not report as T2. The result is `NP_HUB_ERR_TIER_F4` (-19), or `NP_HUB_ERR_TIER_UNVERIFIED` (-20) before the first report, which is not F4 and is retried. **T2-ness is derived from the modality set.** `NP_PROTO_FLAG_T2_TIER` is still sent by the app and read by nothing, so setting or clearing it changes nothing. That closes `RISK-PWRSRC-10` **for protocols naming a T2 modality**. It does **not** close it for §6.2's other case, a T2-tier protocol built only from T1 modalities (`clinical-03`). No such protocol is designated yet (`OI-PWRSRC-06`), and whether one may run on T1 under supervision is counsel's Q1. That case needs a signed designation the MCU or hub can read, and the modality set cannot supply it This is the only gate for the **qEEG cap**, which senses, has no enable line, and so cannot be withheld by the MCU |

**Verified on host, not on silicon.** `np_tier_identity_tests` signs records with Monocypher's RFC 8032
signer under a test key and verifies them with the `np_crypto` verifier the image links. It covers:
blank; signed T2 for this unit; signed T1; a genuine T2 record from another unit; the tier byte edited
after signing; one signature bit flipped; a different signer; a signature made without the domain
string; four format rejections; each T2 line requested alone beside a T1 line; and the report.
`np_tier_identity_nokey_tests` builds the same source with the shipped placeholder, and shows a
perfectly signed T2 record still yields T1 without the record even being read.
`np_hal_platform_tests` covers the two new HAL reads. `np_protocol_tests` covers the hub admission,
including *"clearing the app flag does not get cervical VNS past a T1 unit"*. Mutation-checked:
forcing the verdict to T2 fails 44 checks in the keyed suite and 4 in the no-key suite, and dropping
the UID from the signed message fails the positive case. **The main-loop placement of the gate is
verified by inspection only**: `np_safety_main.c` has no host test. It adds a row to the SW01-M10 FMEA
(`NP-FMEA-001` §3.10).

**Against §7.5's verification column:**

| Verification asked for | Status |
|---|---|
| T1-identity unit, each of T2-D, A14 and the cap fitted, a protocol per T2 modality: enable never asserted | **Host-tested at the gate** (each T2 line, alone and together) and **at the hub** (each of the six T2 modalities, the cap included). On a unit: not run, and no T2 part exists to fit |
| The same with the identity erased and corrupted | **Host-tested**: blank, all-zero, bad magic / version / reserved / tier code, flipped signature bit, edited tier byte |
| App with the tier forced to T2 in UI state: nothing unlocks | **Stimulation: yes, by construction.** Nothing the app sends reaches the gate. **Software features: not built.** No app code consumes the signed identity yet (`OI-UPG-01`, app half) |
| Analysis: every T2 enable path passes the gate | **Done by inspection.** One grant site (`np_spi_watchdog_tick`), one gate after it, and nothing after the gate adds a bit |
| `REQ-UPG-02` inspection: no write command, OTA payload or service tool addresses the identity | **Firmware: yes.** No OTP write code exists in the safety image, and no SPI frame carries the record. The manufacturing tool that writes it does not exist yet (`OI-UPG-08`) |
| `REQ-UPG-02` test: a rewrite through every external interface is refused | **Not run.** It needs silicon and the debug-port lock state (`OI-UPG-08`) |

**Not built, and why each is not a detail:**

1. **No unit is T2 yet, by design.** The authority key is a placeholder until a key ceremony generates it
   and a T2 image is built with it. The safety MCU is never updated by automated OTA, so a key rotation
   is a user-confirmed safety-image update. Its custody, and whether the image carries one key or a
   short list, are `OI-UPG-08`'s to decide.
2. **The record is not written by anything.** Manufacturing needs a programming step that writes every
   unit's record, **T1 included**, before final acceptance. The step must reject a unit whose MCU reports
   reason `BLANK`. A T1 unit shipped blank behaves correctly, but its window stays programmable, and a
   window that is programmable in the field is `REQ-UPG-02`'s defect. That, and locking the debug port
   (RDP), are `OI-UPG-08`.
3. **The app half.** `REQ-UPG-01`'s software sentence (sLORETA, FHIR R4, LSL, the scripting API, HIPAA
   cloud, the anonymized session tag unlock only against the signed identity) has no implementation.
   Neither does F4 presentation. **Nothing carries the hub's load status to the app today, for any
   error**: `np_hub_control_main.c` discards `np_runner_load()`'s return. So `NP_HUB_ERR_TIER_F4` exists
   but cannot yet reach the user as F4. Both stay in `OI-UPG-01`.

**Documents changed by Rev 3:**

| Document | Change |
|---|---|
| `firmware/safety_mcu/` | New SW01-M10 (`src/np_tier_identity.c`), its main-loop gate and report, two HAL reads (`platform/np_hal_otp.c`), record layout and placeholder key (`include/np_safety_config.h`), `NP_SAFETY_EN_T2_MASK` (`include/np_safety_protocol.h`), tests `np_tier_identity_tests` and `np_tier_identity_nokey_tests`, and HAL cases in `np_hal_platform_tests` |
| `firmware/common/include/np_spi_wire_types.h` | Tier codes, reason codes, `np_safety_tier_report_t` (window bytes 20–25) |
| `firmware/hub_control/` | `np_protocol_uses_t2_modality()`, `np_protocol_tier_admit()` and its call in `np_runner_load()`; tier report parsing in `np_safety_spi.c`; `NP_HUB_ERR_TIER_F4` / `NP_HUB_ERR_TIER_UNVERIFIED`; admission cases in `np_protocol_tests` |
| `.github/workflows/` (3) · `firmware/CMakeLists.txt` | Class C selection regex and counts 8 → 10, total 40 → 42 |
...
|---|---|---|---|
| **OI-UPG-01** | **Implement `REQ-UPG-01` and `REQ-UPG-02`: a signed tier identity written at manufacture, and a T1 refusal of every T2 modality and T2 software feature** (§7.5). **Rev 3: firmware half IMPLEMENTED** (§7.7): SW01-M10 verifies a device-bound Ed25519 record in OTP and withholds every T2 enable line unless it is a verified T2. The hub refuses T2 protocols at load as F4 from the modality set, which closes `RISK-PWRSRC-10`'s app-flag bypass **for protocols naming a T2 modality** (not for a T1-modality protocol designated T2: §7.7). **What remains here, the app half:** (i) T2 software features (sLORETA, FHIR R4, LSL, scripting API, HIPAA cloud, anonymized session tag) unlock only against the signed identity. The app needs the record, or a signed statement of it, over the transport, and the app's tier is UI state today. (ii) **F4 reaches the user.** The hub discards `np_runner_load()`'s status for every error, so `NP_HUB_ERR_TIER_F4` needs a load-status path to the app before it can be shown as F4 and never as F1. **The key and the manufacturing step are `OI-UPG-08`**, and nothing here is in force until it closes. Costs stated at design time still hold: (i) Class C scope widened by one module (`NP-FMEA-001` §3.10). (ii) Manufacturing key management, now `OI-UPG-08`. **Counsel's answers (`OI-UPG-02`) may tighten the floor. They cannot remove it** | FW (hub) + App | **T2 software release**; F4 presentation of any tier refusal |
