...
> **⚠ SUPERSEDED (2026-07-28) — wire format fixed at 5 slots, do not use for new protocol work.** `np_pbm1064_session_desc_t` (§1) hardcodes `smart_module_mask` (8-bit, effectively 5 used) and `zone[5]` — both retired by `NP-HEX-ZM-001`'s ~30–80-socket hex lattice. This is a real signed wire-format struct (app↔hub protocol, session descriptor v4), not just documentation — a new schema version indexed by socket (not a fixed-size zone array) is needed before this protocol can address the current hardware. Per-zone field semantics (current setpoints, PWM frequency, duty cycle, channel mask) remain the right model per socket; only the fixed-array/slot-count assumption is retired. See `docs/np_hex_zm_001.md`.
>
> **Requirement on the replacement schema (NP-HW-HUB-001 Rev C §10, 2026-07-29): reuse the existing `NP_PROTO_TARGET_SOCKET_MASK` primitive — do not invent a third socket representation.** NP Hub Protocol v2 already defines a 16-byte, 128-bit, LSB-first, 0-based socket bitmap sized to `NP_HEXMAP_MAX_SOCKETS` (`firmware/hub_control/include/np_hub_config.h`). `NP-HEX-ZM-001` §4b's argument for a bitmap over an address list applies verbatim to this descriptor: under inclusive midline membership a socket list carries duplicates to the driver, and a duplicate means **double J/cm² on the same module**. A bitmap cannot express the duplicate, so the dedup guarantee stops depending on every producer remembering it. Any socket-indexed representation would work mechanically; a third one would simply be a third place for that guarantee to be forgotten. Hub-side, Rev C expands the bitmap to per-cluster acquisition frames (one per 8 sockets).
>
> Also note §6.1's "Cortical | 1064 nm (CH_C) | **Up to 5 addressable zones**" — retired with the same 5-slot assumption; T1-C placement is protocol-defined and not capped at a fixed count (`NP-HEX-ZM-001` §4a SMART-1).
