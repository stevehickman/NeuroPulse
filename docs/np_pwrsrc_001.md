...
**Related Issues:** GitHub #440 (Rev 3, `OI-PWRSRC-13`)
...
> **Note 2026-09-24 (`NP-REG-UPG-001` Rev 3 §7.7): the modality gate now exists in firmware, and is
> not yet in force.** The safety MCU verifies a UID-bound Ed25519 tier record in OTP and withholds
> every T2 enable line unless the unit is a verified T2 (SW01-M10). The hub refuses a protocol naming a
> T2 modality on such a unit as F4, and reads `NP_PROTO_FLAG_T2_TIER` nowhere. The image carries a
> placeholder authority key, so every unit is T1 until `OI-UPG-08` closes. **This section's own case is
> still open:** a protocol built only from T1 modalities and restricted to T2 for its dose
> (`clinical-03`) cannot be recognised from its modality set. It needs a signed designation that
> nothing yet carries, and `RISK-PWRSRC-10` stays open for it. This note changes no D-row.
...
5. **The scripts and the runtime must not diverge — D-1.** `check-pbm-power.ts` owns the demand model;
   ...
   agree.**
   *Rev 3 (2026-09-26, GitHub #440): **done.** The source is `hardware/np_pbm_model.json`. The
   audits read it through `scripts/pbm-model.ts`. The C header
   `firmware/hub_control/include/np_pbm_model.generated.h` is emitted from it and included by
   `np_pbm_power_gov.c`. `scripts/check-pbm-model.ts` (`tooling-ci.yml` `pbm-model`) fails CI if
   the header disagrees with the JSON, or if any script binds a model constant to a literal. The
   risk was already real inside TypeScript: the non-PBM overhead was 8.0 in one audit and 7.0 in two
   others, and `FACE_LIMIT_C` and both CEM43 lines were declared twice. Every audit's output is
   byte-identical after the move. The 28 W interim thermal ceiling of item 3 is in the source as
   `THERMAL_BUDGET_W`, provisional. It is not a derived thermal budget, so `OI-PWRSRC-12` stays open.*
...
> of device draw fully distributed and 15.9 W at an authored montage** *(**37.5 / 15.8 W** since the 2026-09-23 re-loft — `OI-THCOOL-21`)*, so the row is an electrical
...
> **31.4 W across the whole lattice, 8.9 W at N = 6 and 17.9 W at +35 °C ambient** *(**30.5 / 8.8 / 17.5 W** since the 2026-09-23 Layer 4 deletion's 3 mm re-loft shrank the exterior — `NP-THERM-SINK-001` Rev 3 §3a, `OI-THCOOL-21`)* — below the
...
| **RISK-PWRSRC-10** | Medium | A T2-gated protocol is bypassed by an app that sets the flag itself | `NP_PROTO_FLAG_T2_TIER` is app-computed and hub-ignored; a gate the app computes and the hub trusts is not a safety gate | Injury-risk protocol runs on an unentitled device | Device-bound signed entitlement redeemed in the safety MCU (§6.7, D-9), never an app flag | Safety FW | **PARTLY MITIGATED 2026-09-24, not in force** (`NP-REG-UPG-001` §7.7). **For protocols naming a T2 modality:** the flag is read by nothing. The hub derives T2-ness from the modality set, and the safety MCU withholds every T2 enable line unless its signed OTP tier identity is T2 (SW01-M10). Not in force until the tier-authority key exists (`OI-UPG-08`). **Still OPEN for a T2-tier protocol built only from T1 modalities** (§6.2's `clinical-03` case): no signed designation exists for either processor to read, and none is designated yet (`OI-PWRSRC-06`) |
...
| ~~**OI-PWRSRC-13**~~ | ~~**One source of truth for `TILE_W`, the overhead constant and the thermal constants, emitted into both the TypeScript audits and the C runtime, with a CI check that they agree.** Three committed scripts and a future hub governor read the same model in two languages~~ **CLOSED 2026-09-26 (Rev 3, GitHub #440).** Source: `hardware/np_pbm_model.json`, 17 constants, each with its source and status (provisional / placeholder / external limit). TS audits read it via `scripts/pbm-model.ts`. The C runtime gets `firmware/hub_control/include/np_pbm_model.generated.h` as scaled integers (mW, 0.1 °C, s). `scripts/check-pbm-model.ts` is a CI gate with a self-test: header ≡ JSON, and no script binds a model constant to a literal. **This guarantees one number, not a correct one:** every value stays provisional until `OI-PWR-01`. A C-side no-literal rule waits for `OI-FWHUB-09` to write the governor body | Firmware + CI. `NP-CONV-001` §8. **CLOSED** |
...
| **OI-PWRSRC-20** | **`NP-PWR-BUDGET-001` and `NP-SES-PWR-001` are not registered in `NP-DHF-001`** — zero occurrences of either serial, against §5's own rule that the index *"must be updated before the new document is released."* Not backfilled here: registering another author's document is an owner-signed QMS action | Quality. Design-control traceability |
| **OI-PWRSRC-21** | **A throttled protocol under-doses silently.** Eight cascaded protocols imply face temperatures above the 42 °C limit; the Path B1 interlock clamps them, so the safety outcome is correct and **the efficacy outcome is not reported anywhere** — not in the app, not in the compiler, not in the session record. §5.5.1. The programme's own lesson is *"under-dosing, not mechanism failure, explains most nulls"* | Firmware + Clinical. Same silent-substitution objection `NP-SES-PWR-001` §3 makes for zone scope |
...
| **2** | **2026-09-23** | NeurOne Systems Engineering | **Annotations only, for the Layer 4 absorber deletion (`OI-THCOOL-21`, GitHub #407; `REQ-CAV-04`, #391).** The two dated notes quoting `NP-PWR-THERM-001` (the §4.1 note and the T1-peak note) carry the current values beside the published ones: **30.5 / 8.8 / 17.5 W** emitter, **37.5 / 15.8 W** device draw (`NP-THERM-SINK-001` Rev 3 §3a — the re-loft shrinks the exterior 4.6 %, `OI-SINK-10`). **§4.1's own 27.6–49.1 W band is not re-stated here**: its conservative end is 11.3 K over the as-was 0.41, and at the current 0.335 it would read 33.7 W (`check-power-envelope.ts` §1) — the band is already superseded as a ceiling by `NP-PWR-THERM-001` §3, and its restatement is this document's owner's call, raised rather than made. §5's CEM43 audit keeps its 0.41 bound deliberately: `check-thermal-dose.ts` now prints the current-path case beside it (worst single session 292 → 291 CEM43; 4 → 3 protocols at the 40 line) and **fails loudly if any protocol's dose rises** — none does. **No conclusion changes.** |
| **3** | **2026-09-26** | NeurOne Systems Engineering + Firmware | **`OI-PWRSRC-13` closed (GitHub #440).** The PBM power and thermal model constants now have one source, `hardware/np_pbm_model.json`. It feeds the TypeScript audits (`scripts/pbm-model.ts`) and a generated C header for the hub governor (`np_pbm_model.generated.h`, included by `np_pbm_power_gov.c`). `scripts/check-pbm-model.ts` is the CI gate that proves they agree. Six scripts were moved onto it (`check-pbm-power`, `check-thermal-dose`, `check-power-source-coverage`, `check-power-envelope`, `check-thermal-bowl`, `check-thermal-network`), which removes 20 literal re-declarations, among them an 8.0 W / 7.0 W split in the non-PBM overhead. **Every script's output is byte-identical before and after**, so no figure in this document moves. §11 item 5 annotated and the `OI-PWRSRC-13` row struck. No other section changed |
