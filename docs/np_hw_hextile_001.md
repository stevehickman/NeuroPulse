...
> **Rev 14 (2026-09-25) — `OI-HEXTILE-25` CLOSED. Protocols command absolute irradiance, CW is continuous, and `REQ-TDRV-02` changes form to allow it. `OI-HEXTILE-26` raised.**
>
> Three principal directions settle all four of Rev 13's consequences:
>
> - **CW means continuous**: 100 % on-time, never 25 % and never 50 % PWM.
> - **Protocols execute as originally specified.**
> - **The power a protocol delivers is stated in absolute terms**, not as a fraction of what an
>   emitter can do.
>
> §4.3.3 is new and records what the hub does with that. NPPS `pbm_transcranial` takes
> `irradiance_mw_cm2` (`NP-NPPS-REF-001` Rev 17), and the hub wire carries mW/cm² (protocol v4).
> The hub converts to a `CUR` code for the module type it is driving (`np_pbm_irradiance.c`). It
> **refuses**, rather than clamps, anything over R-4 for its mode, over R-5 on a T1-C, or beyond
> the tile's reach.
>
> **`REQ-TDRV-02` is re-derived** (D-9, §6.2 U4). Rev 13's gate on-time limit (≤ 50 %) cannot
> coexist with continuous CW: it would cut a DC channel at any current. It becomes a hardware limit
> on the channel's **average current**, sized so that average irradiance is ≤ 200 mW/cm² at the
> emitter's most efficient operating point. Nothing firmware does can move it. **What it costs:**
> Rev 13's limit needed no emitter data, and this one does (`k_max`, `OI-HEXTILE-24`(d)).
> Derivation: `NP-SOUP-LFS-001` §13.15.
>
> `Vascular Baseline` converts to 322 mW/cm² CW, over R-4. It is refused everywhere and awaits
> re-authoring from evidence (`OI-SESPWR-02`). **`OI-HEXTILE-26`:** one irradiance per NPPS block
> means a `660_808_1064nm` block asks CH_C for what only CH_A/CH_B can reach.
...
> **Rev 14 — (b) below is reversed; (a) is settled by §4.3.3.** CW is continuous (principal,
> 2026-09-25), so CW runs as DC at the current that gives the commanded irradiance — at 200 mW/cm²
> the ≈75 mA this paragraph planned — and its WPE advantage stands. `REQ-TDRV-02` was re-derived
> to allow it. For (a), the hub never commands above the request: 400 mW/cm² on T1-A converts to
> code 210 (≈148 mA at design-target flux), not 150 mA (§4.3.3).
>
...
#### 4.3.3 Commanding irradiance — what a protocol states and what the hub does with it (Rev 14, `OI-HEXTILE-25`)

A protocol states **on-state irradiance at the scalp face, in mW/cm², per lit channel**
(`irradiance_mw_cm2`, `NP-NPPS-REF-001` §4.1). It does not state a current, and it does not state
a percentage. A percentage is a fraction of *this* emitter's capability, and that differs by tile
type: T1-A reaches ~403 mW/cm² at full drive, a T1-C 660/808 channel ~269, and CH_C 28. It also
moves with the part `OI-HEXTILE-02` has not selected. The hub is the first place that knows which
tile it is driving, so it does the conversion (`np_pbm_irradiance.c`):

| Step | Rule | Why |
|---|---|---|
| Mode ceiling | refuse `irr` > **400** pulsed or > **200** CW | R-4 (CLAUDE.md §3) |
| Aggregate | on a T1-C, refuse Σ `irr` > **600** | R-5. Not applied to T1-A while `OI-HEXTILE-20` is open |
| Reach | refuse `irr` the tile cannot emit at `CUR` 255 | a protocol that cannot be delivered as written does not run at something less |
| Conversion | `CUR = floor(irr × 255 / E_fs)`, and refuse a result of 0 | `floor` never asks for more than the request; a zero code would silently deliver nothing |
| Duty | CW → 100 %; pulsed → min(duty, 25 %) | CW is continuous; R-4's pulsed duty |

`E_fs`, the irradiance at `CUR` 255 (180 mA), is the §4.3 figure scaled from 150 to 180 mA:
T1-A 483.6, T1-C 660/808 322.8, and CH_C 33.6 mW/cm². **These are design targets, not
measurements**, and they carry every caveat §4.3 does. They are replaced from the selected emitter's
flux curve (`OI-HEXTILE-02`), and per tile from its dual-PD calibration (`OI-HUB-C06`), before a
tile ships. The linear model is not a safety bound; D-9's hardware is (§6.2).

**What this does to §4.3.1's design point (`OI-HEXTILE-25`(a)).** 400 mW/cm² on T1-A converts to
code 210, which is 148.2 mA at design-target flux. The 150 mA "full drive" of §4.3–§9 is therefore
not a commanded point. Figures computed at it overstate draw and heat by ≤ 1.2 %, and are kept as
the conservative bound they now are.
...
| U4 *(Rev 13, D-9; re-derived Rev 14)* | **Average-current limiter**, CH_A/CH_B — senses the channel current (R1/R2), acts on the drive path, not in U1 | **not selected** | `REQ-TDRV-02` (Rev 14): the channel's current averaged over any window `T_w` ≥ 500 ms stays ≤ `I_avg,max` = 200 mW/cm² ÷ `k_max`, at the limiter's own upper tolerance limit. `k_max` is the selected emitter's highest irradiance per unit current over 0…`I_cap`, at its highest-flux bin and coldest junction. It holds when U1 is wedged or wrong, and it allows DC — continuous CW — which Rev 13's gate on-time form did not. A typical form is an RC-filtered sense voltage compared against a fixed reference, cutting the gate; the choice is EE's. ~~Rev 13: gate on-time ≤ 50 % over `T_w` ≥ 250 ms~~ — superseded because it would have cut continuous CW |
...
| **OI-HEXTILE-24** | **Implement D-9: select U3 and U4 and fix their values** (raised Rev 13). (a) `I_cap`: sized from the selected emitter (`OI-HEXTILE-02`) so that the regulation's upper tolerance limit, at the highest-flux bin and coldest junction, gives ≤ 400 mW/cm² (`REQ-TDRV-01`). It will be below 150 mA (`OI-HEXTILE-25`(a)). (b) R1–R3 = `V_ref` / `I_cap`. (c) U3's dropout becomes §8.1.1's `V_dropout`, and it enters the ±0.10 V-bin arithmetic of Rev 10, where N = 8 already needs a dropout of zero, so this may force string length (`OI-HEXTILE-18`/`-19`). (d) U4 (Rev 14, average-current form): `k_max` from the selected emitter's flux curve — the most efficient point in 0…`I_cap`, highest bin, coldest junction — then `I_avg,max` = 200 ÷ `k_max`, the limiter's tolerance and `T_w` ≥ 500 ms. **Check `k_max` ≤ 2 × `k(I_cap)`.** Otherwise the 25 % pulsed presets at 400 mW/cm² trip it (`NP-SOUP-LFS-001` §13.15). A window above ~40 s re-derives its thermal argument. (e) Area on the 22 × 14 mm rigidizer, and the BOM line for §6.4 — uncosted, and every cost figure is a floor (`CLAUDE.md` §2.1). (f) Whether CH_C carries U3/U4 for layout uniformity. (g) Verification: a bench test that U3 holds `I_cap` and U4 trips while U1 commands 100 % duty and full `CUR`. **Until this closes, the thermal cut is the only non-firmware bound** | EE + Safety | `RISK-FWHUB-10`, `RISK-NVRAM-04`; rigidizer tooling; §6.4 |
| ~~**OI-HEXTILE-25**~~ | **✅ CLOSED 2026-09-25 (Rev 14).** Settled by three principal directions: CW is continuous, protocols execute as originally specified, and a protocol states the power it delivers in absolute terms. **(a)** The hub converts commanded mW/cm² to a `CUR` code by `floor` (§4.3.3), so 400 mW/cm² on T1-A is code 210 (~148.2 mA at design-target flux), and nothing asks for the 403 mW/cm² point. The 150 mA figures stand as ≤ 1.2 % conservative bounds. **(b) Reversed:** CW is DC at the current the irradiance needs, and §4.3.1's WPE advantage stands. `REQ-TDRV-02` was re-derived as an average-current limit so hardware allows that. **(c)** CW is 100 % on-time, and the duty ceiling is mode-aware: 25 % pulsed and 100 % CW, clamped per channel by its own frequency code, and re-clamped before a channel leaves CW (`np_pbm_drive.c`, FAI-SM-05). `OI-SESPWR-03`'s undefined CW + duty pair is a parse error. **(d)** The wire carries mW/cm², and the hub refuses anything over R-4, over R-5 or beyond reach before a `CUR` code exists. The register's 0–180 mA reach is below the tile's absolute limits and above R-4 only for T1-A, where the conversion never gets there. Host tests: `np_pbm_irradiance_tests` (falsified 3 ways). *Original text:* **What D-9 does to the rest of the record** (raised Rev 13, found while deriving `REQ-TDRV-01`/`-02`). (a) §4.3.1's full-drive point, 403 mW/cm² at 150 mA, is 0.75 % over the cap D-9 enforces, so every figure computed at "150 mA full drive" (§4.3, §8.1, §9, `OI-HEXTILE-20`) is at a point that is no longer reachable. (b) CW at 200 mW/cm² becomes ~50 % PWM at `I_cap`, not DC at ~75 mA. §4.3.1's WPE advantage is forgone, and the extra heat is a §9.3 term. (c) The hub clamps every mode, CW included, to 25 % duty (`NP_PBM_DUTY_MAX_REG`, `NP-FW-PBM1064-001` §5.3), so R-4's 200 mW/cm² CW ceiling is unreachable through the firmware today. Decide which limit governs CW (`OI-HEXTILE-07`, the tile firmware specification). U4 at 50 % sits behind the firmware's 25 % and does not conflict with it. (d) `CUR` encodes 0–180 mA (`NP-FW-PBM1064-001` §5.1), about 480 mW/cm² at the design-target flux. Under D-9, codes above `I_cap` saturate, and the register map should say so | EE + FW | §4.3.1, §9.3; `NP-FW-PBM1064-001` §5; `OI-HEXTILE-07` |
| **OI-HEXTILE-26** | **One irradiance per NPPS block, three channels of very different reach** (raised Rev 14). `irradiance_mw_cm2` applies to every lit channel, so a `660_808_1064nm` block asks CH_C (reach ~28 mW/cm², §4.3.2) for the same irradiance as CH_A/CH_B (~269). Any value above ~28 is refused on a T1-C, so the tri-wavelength mode is effectively limited to CH_C's reach. No shipped protocol uses it. The fix is per-channel irradiance in NPPS — `NP-FEAS-PBMCH-001`'s per-channel work, now with an absolute unit, as §10.3 there anticipated | FW + NPPS | `NP-FEAS-PBMCH-001` §10–§11 |
