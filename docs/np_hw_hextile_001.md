...
| **OI-HEXTILE-17** | **Two residual naming inconsistencies, recorded rather than fixed (§7.4).** (a) **`SEAT_N` is a third active-low convention** — the 19-position interface now carries `ALERT#` (`#` suffix) alongside `SEAT_N` (`_N` suffix); both unambiguous alone, two conventions on one part together. (b) **`NP-HW-HUB-001` §113 calls pin 17 `ELEC_SIG`** where this document and `NP-DRV-SHELL-002` call it `ELEC`. Neither carries the netlist hazard that decided OI-HEXTILE-16, and renaming either was out of that decision's scope. **Settle before schematic capture** — that is the point at which a name becomes a net | Schematic capture; Hub PCB Rev C netlist |
