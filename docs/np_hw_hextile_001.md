...
> positions; `SYNC` and `DGND` are the fourth and fifth, and `SEAT_N` — this document's own
...
| 12 | /ALERT | N2 | module → socket | pin 12 | open-drain, wired-OR per bus segment; thermal/fault/OCP assertion so the hub need not poll ~80 modules. *(= `ALERT#` in `NP-DRV-SHELL-002` §5.1 — same conductor, two names; **OI-HEXTILE-16**)* |
...
| 18 | **ELEC_SHLD** | N4 | socket → module | pin 14 | driven shield / DRL for pin 17, referenced to the EEG DRL output (CLAUDE.md §4.3). *(= `GUARD` in `NP-DRV-SHELL-002` §5.1 — same conductor, two names; **OI-HEXTILE-16**)* |
| 19 | SEAT_N | — | module → socket | pin 16 | tied to PGND on the module through 1 kΩ; detects *partial* seating (§7.3) |
...
- **Pin 19 (SEAT_N) is not redundant with the I2C presence poll — and the case for it is now stronger.** NP-HEX-ZM-001 §5.4a correctly notes an unseated tile fails its inventory poll. But a *partially* seated tile can answer I2C on two contacts while the PD, NTC, or electrode contacts are marginal. **With N3 restored that failure mode is live rather than hypothetical:** a tile answering I2C while `PD1_K` sits at elevated contact resistance returns an *under-read* photocurrent, which firmware interprets as low optical output and compensates for by driving harder — a silent dose-integrity failure against a stated competitive claim. SEAT_N is positioned at the mechanical extreme of the pad pattern so it is the **last** contact to mate; if it reads low, every other contact is seated.
...
| **OI-HEXTILE-16** | **Two conductors, four names — the only remaining disagreement between this §7.2 and `NP-DRV-SHELL-002` §5.1.4, which otherwise match signal for signal and count for count.** (a) pin 18 is `ELEC_SHLD` here, `GUARD` there. (b) pin 12 is `/ALERT` here, `ALERT#` there. Both are the same conductor under two spellings of the same active-low convention. Harmless in prose; **not** harmless in a netlist, a test fixture, or a firmware pin table, where a silent duplicate net or an unconnected pin is the failure mode. Adopt one name for each across both documents at their next revision — no technical content either way. *(Found by diffing the two pin tables mechanically rather than by reading them; **HT-DRC-23** should be run the same way.)* | Schematic + netlist consistency; pick before Hub PCB Rev C release |
...
| HT-DRC-22 | **19-contact pad array is two staggered rows** and fits inside the 20.0 mm tile inradius, with mis-key asymmetry and `SEAT_N` at the extreme of the last-seating row | Open — CAD; `NP-DRV-SHELL-002` REQ-SKT-01 / SH2-DRC-05a |
| HT-DRC-23 | Socket pinout matches `NP-DRV-SHELL-002` §5.1.4 pin for pin | ✓ at Rev C (§7.2) — **re-verify on either document's next revision**; the two specify one physical interface |
