...
| DI-ID | Category | Input Description | Source | Priority | Scope |
|-------|----------|-------------------|--------|----------|-------|
| DI-REG-01 | Regulatory | IEC 60601-1 general safety for medical electrical equipment: basic safety + essential performance | docs/reference/regulatory-strategy.md | Regulatory | T1+T2 |
| DI-REG-02 | Regulatory | IEC 60601-2-10 particular requirements for nerve and muscle stimulators: BES, tDCS, tACS, VNS current limits and waveform parameters | docs/reference/regulatory-strategy.md | Regulatory | T1+T2 |
| DI-REG-03 | Regulatory | IEC 62471 photobiological safety: visual stimulation MPE limits; PBM irradiance hazard classification; GoggModee F retinal cumulative dose | docs/reference/regulatory-strategy.md; RISK-03 | Regulatory | T1+T2 |
| DI-REG-04 | Regulatory | IEC 62133 battery and power bank safety: hub supercapacitor + USB-C PD power path | docs/reference/regulatory-strategy.md | Regulatory | T1+T2 |
| DI-REG-05 | Regulatory | FCC Part 15 RF emissions: BT 5.3 LE Audio and Wi-Fi 6 radios; antennas in hub not headset | docs/reference/regulatory-strategy.md | Regulatory | T1+T2 |
...
| DI-REG-07 | Regulatory | IEC 62366-1 human factors engineering: Use-Related Risk Analysis (URRA), formative testing, summative testing plan; FDA HFE Guidance 2016 | docs/reference/regulatory-strategy.md | Regulatory | T1+T2 |
| DI-REG-08 | Regulatory | ISO 14971:2019 risk management: NP-RM-001 acceptability matrix S1–S5/P1–P5; residual risk evaluation before 510(k) submission | docs/reference/regulatory-strategy.md; NP-RM-001 | Regulatory | T1+T2 |
| DI-REG-09 | Regulatory | 21 CFR Part 820 quality system regulation: design controls; document control; CAPA | docs/reference/regulatory-strategy.md | Regulatory | T2 |
| DI-REG-10 | Regulatory | FDA cybersecurity guidance: SBOM planned NP-SBOM-001 Year 2; OTA Ed25519 code signing; vulnerability disclosure policy; OTA update process documented | docs/reference/regulatory-strategy.md | Regulatory | T1+T2 |
...
| DI-REG-12 | Regulatory | GDPR Art. 13(2)(f) adaptive stimulation transparency: classified np_adapt_trigger_t enum (17 values); user-visible AdaptiveAdjustmentsCard in Session History; plain-language description of each trigger | docs/status/pending-decisions.md §13.4 (Issue #98) | Regulatory | T1+T2 |
...
| DO-HW-03 | HW Spec | NP-PROC-FPC-001 — FPC Procurement Requirements (LED Vf binning, Hirose FH34S exclusions, BCR421W spec, RA copper) | **Rev 4** *(was cited as Rev 1; the document was at Rev 3 with a stale header — corrected 2026-09-21, GitHub #333)* | Active | T1+T2 |
...
| DO-HW-06 | HW Spec | NP-TOOL-SHELL-001 — Shell Tooling Specification (F-01 zone slot anchor posts ×5, F-02 accessory port anchor posts ×3, F-03 EEG cable routing channel, F-04 temporal wing boss PROVISIONAL) | Rev 3 → **superseded by NP-TOOL-SHELL-002 Rev 1** | **SUPERSEDED 2026-09-25 (GitHub #331, `OI-ART-01`): the design output for shell tooling is now `NP-TOOL-SHELL-002` Rev 1 (DRAFT)**, which carries F-04 as S2-F01 and holds the latch, boss, Hall and lip features as requirements-only pending MECH-1. *Superseded status text:* **PARTIALLY SUPERSEDED 2026-08-18 — reviewed against the file, not inferred.** The ⚠ flag is resolved and **it under-scoped the defect.** **F-01 retired outright** (NP-HEX-ZM-001 §3.4/§4a/§5.1 — no zone slots, no zone identity to colour-code, and sockets moved to the *inner* bowl while F-01 specifies posts on the outer CFRP surface), taking SH-01–SH-05, SH-23, FAI-CV-01–04 and OI-04 with it. **F-03's premise also retired** — not visible from this row's title, which is why the inference-only flag missed it: EEG electrodes now live inside T1-B tiles and NP-DRV-SHELL-002 §5.3 absorbs the EEG harness into the cluster network, so the 8×5 mm channel has no cable to route (§9.1 quotes F-03's own opposite-surfaces rule and calls it unavailable); its <5 µVpp threshold survives as SH2-DRC-16, and this merely aligns the document with `cad/CAD_PARTS_LIST.md`, which already carried EEG-ROUTE-CHANNEL as a RETIRED tombstone. Retired under the current design, not shown impossible — reopening condition is `NP-HW-EEGNET-001` Rev 2 §7.3 (fallback geodesic-net option only; gated on OI-EEGNET-01/-14). **F-02 is architecture-independent but conflicts with NP-TOOL-HUB-001 Rev 1 F-02** on the same hub ports — 50 mm tether vs a safety-derived ≤20 mm maximum (fan-ingestion hazard, FAI-HTOOL-02) — **OI-ART-07** — **closed 2026-09-23: F-02 retired in place (Rev 3), hub port covers owned by NP-TOOL-HUB-001 Rev 3**. **F-04 survives unchanged.** Releasable for F-04 only; re-scope is **OI-ART-01** (blocked on MECH-1) | T1+T2 |
