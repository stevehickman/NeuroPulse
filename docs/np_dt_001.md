...
| DI-USE-05 | Usability | Zone module keying (RISK-15): Layer 1 asymmetric mechanical key; Layer 2 ZONE_ID resistor ladder; Layer 3 ISO 17049 braille + raised numeral; Layer 4 shell tactile dots; Layer 5 bone conduction audio confirmation. **⚠ Superseded 2026-07-28:** Layers 1–2 retired (SMART-1 makes every socket accept every type; detection moved to `np_module_map`) — whole scheme needs re-derivation for the hex-tile lattice, not a line-item fix. See `docs/np_hfe_001.md` CT-01. | CLAUDE.md §7.1 RISK-15 | High | T1+T2 |
