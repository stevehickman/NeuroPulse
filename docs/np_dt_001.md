...
| DI-USE-05 | Usability | Zone module keying (RISK-15): Layer 1 asymmetric mechanical key; Layer 2 ZONE_ID resistor ladder; Layer 3 ISO 17049 braille + raised numeral; Layer 4 shell tactile dots; Layer 5 bone conduction audio confirmation. **⚠ Superseded 2026-07-28:** Layer 1 (mechanical key) confirmed not needed under SMART-1 — no type differentiation left to key against (`docs/np_hex_zm_001.md` §4a). Layer 2 (ZONE_ID) retired, detection moved to `np_module_map`. Layers 3–5 (braille, tactile dots, audio) likely still needed for accessible *position* identification — whole scheme needs re-derivation for the hex-tile lattice. See `docs/np_hfe_001.md` CT-01. | CLAUDE.md §7.1 RISK-15 | High | T1+T2 |
