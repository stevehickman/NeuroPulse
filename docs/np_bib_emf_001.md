...
| **L4** carbon-loaded absorber foam | *stated:* cavity-resonance suppression. *actual:* **tolerance compliance + thermal path** (`REQ-CAV-04`) | **stated benefit REFUTED** — 0.26 dB of the 26.2 dB `REQ-CAV-02` needs (`NP-EMC-CAV-001`) | 18 % of the outward path, but **removing it costs 0.450 vs 0.410** — air is 54 % worse per mm | **Keep, on other grounds — §7.3a.** Same shape as L2: held by dependencies its documentation never named |
...
2. **The station is re-specified, not removed** — `NP-EMC-CAV-001` §8.2 (Rev 2). **Deleting it is a
   thermal *regression*:** vacating 3 mm fills it with stagnant air at 0.115 m²K/W against the foam's
   0.075 — **54 % worse per mm** — so the outward path goes 0.410 → **0.450** unless the outer bowl
   is re-lofted 3 mm, a shell tooling change `NP-REV-SHELL-001` gates. A ceramic-filled substitution
   reaches **0.355 with no tooling change**, and keeps the tolerance compliance the foam actually
   provides across the curved gap — which is why `OI-THCOOL-15` exists at all.

> **So this section's verdict needs restating, and §7.1's row with it. Layer 4 is no longer a removal
> candidate — it is a *mis-justified* layer.** Its stated benefit is refuted (0.26 dB of 26.2), but
> the part is load-bearing for two functions nobody wrote down: **tolerance compliance across the
> inter-bowl gap, and a thermal path that is better than the air replacing it.** `REQ-CAV-04` changes
> its justification of record accordingly. **That is the same defect this audit found in Layer 2**
> (§7.2) — a layer held in place by dependencies its documentation does not name — and it is now the
> audit's second instance of it, not its one removal.
...
| ~~**OI-BIBEMF-08**~~ | ~~State Layer 4's requirement in dB against a named source and frequency band~~ **✅ DISCHARGED 2026-09-20 by `NP-EMC-CAV-001`** (§7.3a). Source named (18 cluster controllers, not radios), band derived (**420 MHz – 3 GHz**, lower edge moving 84 MHz with head circumference), requirement stated (`REQ-CAV-01`/`REQ-CAV-02`, **≥ 26.2 dB**). **Layer 4 supplies 0.26 dB; the wearer's head supplies 49.8 dB.** Neither step-2 branch applies as written: there is no RF function to preserve (2a) and EMC was not silent (2b). **Rev 3: the station is re-specified, not removed** — deleting it is a thermal regression (stagnant air is 54 % worse per mm than the foam, outward path 0.410 → 0.450), so `REQ-CAV-04` retains it and changes its justification of record to **tolerance compliance + thermal path**. The thermal half is unblocked now | EMC + Thermal. **DISCHARGED** | — |
