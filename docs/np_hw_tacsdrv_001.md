...
> **Rev 3 — `OI-TACSDRV-06` is ANALYSED and RECOMMENDED, not decided, in `NP-REG-UPG-001`.** The
> recommendation is the **new-unit** row above: T2 is built as T2, and no T1 unit is converted by
> anyone. Two findings there change how this table reads. **(1) The PAN is tier-specific before A16
> arrives.** Its ADS1299 bank is 8 channels on T1 and 21 on T2 (`NP-DRV-SHELL-002` §3.5), on the
> laminated L1 carrier. So the conversion row above understates what every T1 unit would have to
> carry. **(2) Opening the shell is not what distinguishes a conversion.** Users already separate the
> bowls for every tile swap, behind a latch interlock and SHDR seam trending (`NP-HEX-ZM-001` §5.1,
> §5.5, §5.6). A conversion is distinguished by touching **factory-only** assemblies: the PAN on a
> laminated carrier, the boss, and for TMS the never-opened outer bowl. **Under the recommendation, Q1
> holds as written and (e)1's saving holds in full, Q3's hub-side magnetics survive, and Q2 is
> unaffected** (`NP-REG-UPG-001` §7.3). The row's phrase *"the cap connector's shell opening"*
> assumed an outer-bowl opening that nothing in the record puts there. The cap tail lands at the PAN
> from inside the envelope (`REQ-NET-16`), and its connector is `OI-TCAP-05`'s.
>
> **Rev 4 — `OI-TACSDRV-06` is DECIDED (principal, 2026-09-23): the new-unit row above.**
> `NP-REG-UPG-001` Rev 2 §7.0. No T1 unit is converted by anyone, and none is upgraded piecemeal. **For
> this document:** T1 carries **no A16 content**. There is no populated connector footprint, only
> the boss positions the shared tool makes. (e)1's saving holds in full, and Q3's compliance supply
> is populated on T2 hubs only. **The conversion and user-installed rows above are now of record
> only.** One consequence reaches A16 directly. A T1-identity unit must never grant
> `NP_SAFETY_EN_CLIN_STIM` (`NP-REG-UPG-001` `REQ-UPG-01`). On T1 the stage is physically absent, so
> the refusal duplicates physics. It is not redundant, though: the 21-channel cap may fit a T1 PAN if
> `OI-TCAP-05` picks a connector common to both tiers, and the refusal is what keeps that route closed.
...
> Q2's condition is discharged by `OI-TACSDRV-04`. **Rev 3:** `OI-TACSDRV-06` now carries a
> recommendation (`NP-REG-UPG-001` §7). If the principal takes it, the dependency is discharged
> and `OI-TACSDRV-01` waits only on the principal's call on this subsection and on `OI-TACSDRV-04`.
> **Rev 4: the principal took it. The dependency is discharged.**
...
| ~~**OI-TACSDRV-06**~~ | **CLOSED 2026-09-23 (Rev 4): DECIDED by the principal, (a) new unit, with module carry-over** (`NP-REG-UPG-001` Rev 2 §7.0). No T1 unit is converted by the depot, a partner or the user, and none is upgraded piecemeal. The modules a purchaser bought carry over to the T2 they buy. Consequences are applied in `NP-REG-UPG-001` §7.4. The follow-on work is `REQ-UPG-01…03` and `OI-UPG-01`, `-02` and `-04…07` there. *Original text follows.* **Define the T1 → T2 upgrade path. The record assumes one exists and never specifies it** (§4.1.1 (g)). `NP-THERM-COOL-001` Rev 6 aligned the T1 thermal block *"so a customer upgrading T1 → T2 meets the same usage limit,"* and `CLAUDE.md` §2.2 includes a charger at every upgrade, but no document says whether T2 is a **new unit**, a **service conversion** of the customer's T1 headset, or **user-installed parts**. The answer decides (i) what every T1 unit must be provisioned with for A16, including the PAN mounting and connector, the cap connector's shell opening and the boss positions; (ii) whether §4.1.1 Q3's hub-side magnetics survive, since a conversion would touch two assemblies; (iii) whether a conversion that opens the shielded envelope needs its seams re-verified, and by which service tier; and (iv) the **regulatory status of a wellness unit converted to a 510(k) configuration**, which `docs/reference/regulatory-strategy.md` does not address. It reaches beyond A16: every T2 addition that lives in the shell or hub faces the same question. **Rev 3 — ANALYSED, RECOMMENDED, NOT DECIDED (`NP-REG-UPG-001`).** **Recommendation: (a) new unit.** T2 is built, tested, labelled and released as T2, trade-in is a commercial option, and no T1 unit is converted by the depot, a partner or the user. Answers to (i)–(iv): **(i)** T1 carries no populated T2 content, only what the shared tooling makes anyway. Conversion would need more than this row listed, because the PAN's ADS1299 bank is itself tier-sized (8 vs 21) on the laminated L1. **(ii)** Q3 survives. **(iii)** Users already open the parting plane for tile swaps, so the seam is not the issue. A conversion touches the boss and, for TMS, the never-opened outer bowl. It is manufacturing, so it could only be done at the **depot**, never at Tier B. **(iv)** §6 of that document sets out the regulatory consequences of each path, and seven questions go to the existing RISK-03 counsel engagement (`OI-UPG-02`). Correction: the phrase *"the cap connector's shell opening"* above assumed an opening nothing in the record specifies (§4.1.1 (g), Rev 3 note). ~~Stays OPEN until the principal takes or rejects the recommendation~~ **Taken, Rev 4** | Product + Regulatory + ME | — (was: `OI-TACSDRV-01`; T1 shell and hub provisioning) |
...
- **The T1 → T2 upgrade model (`OI-TACSDRV-06`):** `docs/np_reg_upg_001.md`
...
|---|---|---|---|
| 4 | 2026-09-23 | NeurOne Systems Engineering | **`OI-TACSDRV-06` CLOSED: the principal took the new-unit recommendation** (`NP-REG-UPG-001` Rev 2 §7.0), with module carry-over, and ruled out a rolling upgrade. §4.1.1 (g) gains a Rev 4 note: T1 carries no A16 content and (e)1's saving holds in full. **A T1-identity unit must never grant `NP_SAFETY_EN_CLIN_STIM`** (`REQ-UPG-01`), which matters if `OI-TCAP-05` makes the cap connector common to both tiers. **`OI-TACSDRV-01`'s dependency on `-06` is discharged**, and it now waits only on the principal's call on §4.1.1 and on `OI-TACSDRV-04`. No requirement, figure or firmware constant in this document changes |
| 3 | 2026-09-23 | NeurOne Systems Engineering | **`OI-TACSDRV-06` analysed and recommended, not decided, in the new `NP-REG-UPG-001`.** The recommendation is a **new unit**: no T1 unit is converted. §4.1.1 (g) gains a Rev 3 note recording two findings its table lacked. **The PAN is tier-specific before A16 arrives**, because its ADS1299 bank is 8 channels on T1 and 21 on T2, on the laminated L1. **Users already open the parting plane** for tile swaps, so a conversion is distinguished by touching factory-only assemblies, not by opening the shell. Under the recommendation, **Q1 holds with (e)1's saving in full, Q3 survives, and Q2 is unaffected.** The §9 rows for `OI-TACSDRV-01` and `-06` are updated. The `-06` row's *"cap connector's shell opening"* is corrected: no document puts an outer-bowl opening there. **`OI-TACSDRV-01` and `-06` stay open. No silicon, price, cost figure, firmware constant, requirement or locked decision changes.** |
