...
**Related Issues:** OI-TACS-02 (this document closes half (i); the driver half is `NP-HW-TACSDRV-001`, GitHub Issue #332); OI-TACS-01 (closed 2026-09-07); OI-EEGNET-07; OI-ART-06
...
**Supersedes:** NP-HW-TCAP-001 Rev 1. Rev 1 removed `k_driver_channel[]` (`firmware/sloreta_hdtdcs/src/np_hd_montage.c`) from the role of interim authority for the electrode-to-channel map.
...
| The 21-channel driver's silicon, compliance voltage and sense-path architecture | Never selected (§7.1). **Rev 2: the driver now has an owning document** — `NP-HW-TACSDRV-001` Rev 1 (artifact **A16**) states the envelope a selection must meet and registers the stage; it names no silicon either, and adds that **no compliance voltage and no cap electrode impedance exist anywhere**, which it treats as one decision and a **precondition** for the selection | `OI-TCAP-03`; `OI-TACSDRV-02` |
...
| **`OI-TCAP-03`** | **The 21-channel tACS driver has no named silicon**, no compliance-voltage specification and no sense-path architecture, so §7 can bound its cost but not price it. Analog counterpart of `OI-HEXTILE-02`. `REQ-TCAP-04`/`-05` (4 mA per conductor, concurrent on 21) are the input requirements for that selection. **Rev 2 (2026-09-20):** those input requirements, and the rest of the envelope, are now stated in one place — **`NP-HW-TACSDRV-001`** §3, which also finds that the selection has a missing precondition: **no compliance voltage and no T2 cap electrode impedance exist in the record** (`OI-TACSDRV-02`). The item is unchanged in substance and now has a document to be answered into | EE Lead | T2 BOM sign-off; `OI-TACS-02` half (ii) |
...
|---|---|---|---|
| 2 | 2026-09-20 | NeurOne Systems Engineering | **Pointer revision (GitHub #332). The driver side of this interface now has an owning document.** `NP-HW-TACSDRV-001` Rev 1 states the envelope a driver selection must meet — including §4's `REQ-TCAP-04`/`-05` restated on the driver side — and registers the stage as artifact **A16** in `NP-ART-001` §2.2. §8's out-of-scope row and `OI-TCAP-03` are updated to point at it, and to carry its finding that **no compliance voltage and no T2 cap electrode impedance exist anywhere in the record**, which that document treats as one decision and a **precondition** for silicon selection (`OI-TACSDRV-02`) rather than a consequence of it. **No requirement, map entry, coordinate, limit, bound or cost figure in this document is changed**; `OI-TCAP-01`…`-07` keep their scope and wording except for the pointer added to `-03`. |
