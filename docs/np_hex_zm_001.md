...
**Related Issues:** GitHub #401 (`OI-EMCCAV-04`, §5.6.1); GitHub #399 (`OI-EMCCAV-02`, §5.6.1 `EMF-1-SINK-1`/`-2`)
...
> **★ FLUXGATE SITING — stated once, here (2026-09-23, `OI-EMCCAV-11`, GitHub #404). This paragraph
> owns where the fluxgate magnetometers are; every other document cites it rather than restating it.**
>
> 1. **The sensor body is on L1, the inner bowl, inside `NP-HELMET-GEOM-001` §2's "Inner-bowl socket
>    wall + FPC channel" radial station. It is never in the inter-bowl Gap.** *"Near the scalp"* in
>    (c) above is **comparative** — inner bowl rather than outer, for §5.3.1's reason (*"sensors sample
>    where the brain is"*). It does not put the sensor at the scalp face, which the tiled module field
>    occupies. Every source that sites the body agrees: §5.1, §5.3.1, §5.7; `NP-HELMET-GEOM-001` §2,
>    §3.2, §4; `NP-DRV-SHELL-002` §4.3 and `REQ-EMI-10`; `NP-HW-EEGNET-001` §5.4. **The only contrary
>    statements** are `completed-decisions.md` §221 (line 221 of that log) and `NP-THERM-COOL-001` §6.9.1.
>    Both come from the same 2026-08-31 gap-pad analysis, and both cite **this** section for a claim it
>    never made.
> 2. **So the fluxgates do not contribute to the Gap.** Only a **radial** obstruction sets the Gap
>    (`OI-EMCCAV-09`), and the body sits inboard of it.
> 3. **No magnetic keep-out envelope is specified, and this document does not specify one.** The
>    record has no keep-out radius, volume or planform clearance. It also has no sensor part, package,
>    count or site pattern that one could be derived from. Every fluxgate protection in force is a
>    **material** rule, not a **geometric** one: `REQ-EMI-10` (no conductive addition to L1 without
>    re-qualification), `REQ-NET-04` (the net), and `NP-THERM-COOL-001` §6.9.1 (the gap pad must be
>    electrically insulating and non-magnetic). **A keep-out, if one is ever set, is a planform
>    exclusion.** It cannot set the Gap's radial dimension. It can only cap gap-pad coverage
>    (`OI-THCOOL-15`, `OI-EMCCAV-12`, GitHub #405). **Whether one is needed is left open as the rest
>    of `OI-EMCCAV-11`, an EE decision.** The two readings and what each does to pad coverage are in
>    `NP-EMC-CAV-001` §8.6.1.
> 4. **Condition on item 1: the siting is recorded, but the fit is not verified.** No fluxgate part is
>    selected, so nobody has checked whether a 3-axis sensor fits the 2.0–2.5 mm station. If it does
>    not fit and protrudes gap-ward, it becomes a **local** radial Gap contributor at its sites, and item
>    2 must be revisited. This check rides with the rest of `OI-EMCCAV-11`.
...
- **★ DECIDED 2026-09-20 (principal direction) — FLUSH-1: the cluster lever is FLUSH when closed,
  ...
  **(b) Therefore the lever must be flush — or sub-flush — in its closed position**, within the
  ...
  >
  > *(**Corrected 2026-09-23, `OI-EMCCAV-11`:** the fluxgate sensors are **not** a Gap contributor.
  > They sit inside L1's socket-wall station, inboard of the Gap, per §5.3(c)'s fluxgate-siting
  > paragraph. The list above was already superseded by `BOSS-1` and `NP-EMC-CAV-001` §8.7. It is kept
  > as written, in the tense of its date.)*
...
> (`EMF-1e`), but none had been written into the plan they ride. This subsection is that plan. A sixth sweep,
> **`EMF-1f`** (the foam-fitted comparison), was added by principal direction on 2026-09-24. **It is
...
against.** Every sweep runs with both bowls clamped, all four latches reporting closed (§5.5) and the
posterior boss mated. Every sweep except `EMF-1f` also runs on the **4-layer passive stack as designed
since 2026-09-23**: CFRP, mu-metal
...
describes the design.** The fitted half returns as **`EMF-1f`**, on a separate article (principal
direction 2026-09-24; below). Re-stated against the deleted stack, the sweeps no longer measure
...
| **`EMF-1f`** | **Foam-fitted comparison** (principal direction 2026-09-24). A **second outer bowl at the pre-re-loft radius**: conductor 3 mm further out, scalp-to-conductor 26–32 mm (`NP-EMC-CAV-001` §2), otherwise the as-designed stack. It is swept in two states: with a **3.0 mm carbon-loaded absorber liner** against L3 at the deleted station (`NP-HELMET-GEOM-001` §2), then with the liner removed. Each state runs **empty and with the 57 cm phantom**, on the same probes, positions and calibration as `EMF-1a`. **The liner's complex permittivity is measured on a coupon from the same lot and recorded**: no absorber material was ever specified, and §6.2's ε = 2.0 − j1.0 is an assumption | **300 MHz – 6 GHz** | Per mode, the change in Q and in peak \|S₂₁\| between liner fitted and removed, empty and head-fitted | **None: comparison only.** The layer is deleted, so no requirement is written against what it contributed. The result is used three ways. **(1) Below 3 GHz** it tests §6.2's prediction at the *measured* permittivity: 0.26 dB at the design loading, < 1 dB below 1 GHz. §6.1 says no loading can do much better, so a larger measured contribution is a finding against the analysis. **(2) Head-fitted, 420 MHz – 3 GHz:** if the liner-fitted state meets `REQ-CAV-02` where the as-designed article in `EMF-1b`/`EMF-1c` does not, that is a finding against `REQ-CAV-04`. Both findings go **to the principal; neither reinstates the layer by itself.** CLAUDE.md §4.3's only documented return path for the station is `OI-EMCCAV-08`'s insulating pad, never an absorber. **(3) 3–6 GHz:** it gives `OI-EMCCAV-06` the size of what the deletion gave up where the foam was electrically thick (§6.2: 11–41 dB at 3 GHz) |
| **`EMF-1-SINK-1`** | *(listed Rev 6, GitHub #399.)* Coil-drive → fluxgate transfer function, magnitude and phase, three axes, **spreader film fitted vs. not**, at one coil temperature. A fluxgate/Helmholtz bench, **not** the VNA set-up above | 10–300 Hz, and to the active loop's bandwidth if one is ever stated | Change in the transfer function the `SPEC-SINK-04` film causes | **Owned by `NP-THERM-SINK-001` §7.1(1)**, including its prediction (`|ΔH/H| ≤ 0.041` at 300 Hz, mostly quadrature) and what it closes |
| **`EMF-1-SINK-2`** | *(listed Rev 6, GitHub #399.)* External illumination, E-field at the electrode plane, **bare outer bowl vs. spreader film fitted with its edges terminated as built**; plus a radiated-emissions pre-scan in both states | As `EMF-1a` (to 6 GHz) | Whether the film's terminations at the rim lip, the `BOSS-1` emboss and the TMS cut-out degrade the RF shielding | **Owned by `NP-THERM-SINK-001` §7.1(2).** Pass: no worse than the bare bowl at any frequency. Needs the film's edge treatment chosen first (`OI-SINK-01`) |

**Order.** `EMF-1a` runs first, because it is the coupling proof for `EMF-1b`/`EMF-1c`. `EMF-1d` runs
last: it needs `EMF-1c`'s worst size and `OI-EMCCAV-01`'s criterion. `EMF-1f` needs only `EMF-1a`'s
probes and calibration, and its own article. It does not gate the deletion or the shell re-scope. §8.2
asked for it before the re-loft is cut into CAD (`OI-ART-01` / #331), and it is cheapest then, while
the pre-re-loft radius is still the current geometry. None of the six gates the Layer 4
...
> **✅ DECIDED 2026-09-24 (principal direction, PR #412): the foam-fitted comparison IS run, as
> `EMF-1f` above.** `NP-EMC-CAV-001` §8.2, written before the decision, asked for `EMF-1a` at 6 GHz
> *"with the absorber fitted and removed"*. That half now returns as its own sweep. The reasoning
> that framed the choice is kept as written:
>
> - **For:** above 3 GHz the foam was no longer electrically thin (§6.2 gives 11–41 dB at 3 GHz).
>   That is the only region where the deleted layer did measurable work, and it is the region
>   `OI-EMCCAV-06` asks about.
> - **Against:** below 3 GHz it cannot change anything (§6.1). Above 3 GHz there is no requirement
>   to test the difference against.
> - **The article:** a separate outer bowl built at the **pre-re-loft radius**, not a temporary liner
>   in the as-designed bowl. The re-lofted bowl has no station, so a 3 mm liner there would take
>   3 mm out of the 23–29 mm dielectric region (→ 20–26 mm) and shift the very modes it is meant
>   to compare. Only the pre-re-loft article compares foam against no foam at the same conductor
>   position, which is what §6.2 calculated. **This choice of article was made in writing `EMF-1f`;
>   it was not part of the principal's direction.**
...
| EMF-1 | Prototype 2-layer attenuation ≥ single-shell baseline. **Test plan: §5.6.1 (Rev 5)**, which adds the cavity sweeps **`EMF-1a`–`EMF-1d`** (`NP-EMC-CAV-001` §7, re-stated against the as-designed 4-layer stack; pass criteria from `REQ-CAV-01` / `REQ-CAV-02`, with `EMF-1a`/`EMF-1b` swept to 6 GHz for `OI-EMCCAV-06`) lists **`EMF-1e`** (`OI-THCOOL-06`), and adds **`EMF-1f`**, the foam-fitted comparison on a pre-re-loft article (principal direction 2026-09-24). **Never run** | Shield claim; `REQ-CAV-04` justification stated as *measured* (`EMF-1b`–`EMF-1d`); `OI-EMCCAV-06` (3–6 GHz data) |
