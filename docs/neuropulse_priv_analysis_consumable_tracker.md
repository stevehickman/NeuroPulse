...
**Document:** NP-PRIV-ANALYSIS-003 Rev A  
**Date:** 2026-06-08  
...
#### [LOW] Notification permission requested before consent onboarding completes
...
1. **[LOW, 30 min]** Change notification lock-screen title to a generic `"NeuroPulse"` string; move consumable-specific text to `content.body` (which requires unlock on most iOS configurations). Exact change: `content.title = "NeuroPulse"` / `content.subtitle = "Action required"` / `content.body = reminder.headline`.

2. **[LOW, 1 hr]** Defer `requestNotificationPermission()` from `ConsumableTracker.init()` to `ConsumableView.onAppear`. Add a brief in-view explanation for why the permission is being requested. Remove the call from `init()`.
...
These are all minor improvements on an otherwise well-architected feature. No blocking items for internal beta or pre-production use.
