...
> **Enable-line levels in this document are stated at the safety-MCU pin (restated Rev 11,
> 2026-09-25, GitHub #437).** All ten stimulation enable pins are **active-LOW open-drain**:
> **LOW = enabled, HIGH = disabled (safe)** (`np_safety_config.h:7-8`, `np_safety_hal.h`). Before
> Rev 11 several rows wrote the opposite ("drive all stimulation GPIO LOW" for the cutoff), which
> describes the downstream active-high net, such as `PBM_CRANIAL_PERMIT` (`NP-HW-HUB-001`
> HUB-REQ-C06), never the pin. Every such row is restated in place, and its original wording is
> recorded in the Rev 11 revision row. **No failure mode, score or mitigation changed with the
> wording.** The three rows whose mitigations described code that does not exist (M01-01, M01-05,
> M02-05) were then re-authored under **OI-FMEA-11**, in the same revision, and their scores
> changed. See §3.1 and §3.2.

### 3.1 SW01-M01 — Stimulation Enable GPIO Management (`np_gpio_enable.c/.h`)

> **Rev 11: FMEA-M01-01 and FMEA-M01-05 are re-authored against the module that exists
> (OI-FMEA-11).** The module is `firmware/safety_mcu/src/np_gpio_mgr.c`, not `np_gpio_enable.c/.h`.
> The heading keeps its Rev 1 name so that citations of "§3.1" still resolve. There is no
> `IDLE → ENABLED → FAULT` state machine: faults are status bits, and `granted_mask` is re-derived
> every heartbeat. The two rows' residuals **changed** (M01-01 S2×P1 → S4×P1 ACCEPTABLE; M01-05
> S2×P1 → **S5×P1 ALARP**). FMEA-M01-02 and -04 are not re-authored here.
...
|---|---|---|---|---|---|---|---|---|---|---|
| FMEA-M01-01 | Stale enable bitmask not cleared on fault entry — one or more enable pins remain asserted (LOW = enabled) after fault | Stimulation continues after fault condition detected (e.g., overheat fault fires but one PBM zone LED continues driving) | S4 | P2 | 8 (ALARP) | **Re-authored Rev 11 against the code (OI-FMEA-11).** (i) **Overwrite, not accumulate.** Each valid heartbeat *replaces* `granted_mask`: it takes `requested_mask` when no all-channel fault is active, and 0 otherwise (`np_spi_watchdog_tick()`). No bit survives a tick that the tick did not re-derive. (ii) **Each interlock clears its own bits in the iteration it trips**: thermal, charge, cardiac, impedance, signature and tier (their host suites). (iii) **`np_gpio_mgr_apply()` rewrites all ten pins from `granted_mask` on every main-loop iteration, unconditionally.** It is not change-gated, so a mis-written pin cannot hold past one iteration. (iv) Heartbeat loss zeroes the mask (FMEA-M02-05). *Verification:* `np_gpio_mgr_tests` (one-hot mapping of all ten bits, all-off, all-on, unallocated bits, rewrite-every-call, tick overwrite), plus the per-interlock suites; each new assertion was mutation-checked. *Withdrawn:* the Rev 1 claims of a "state machine fault entry function", 100% branch coverage, and "a separate ISR that operates on GPIO directly, bypassing bitmask". None exists. A **hung main loop** is not this failure mode, and nothing backstops it (`NP-RISK-002` OI-RISK2-05) | S4 | P1 | 4 | ACCEPTABLE |
| FMEA-M01-02 | SPI enable grant processed after fault state entered — spurious late enable overrides fault | A fault event and an in-flight SPI enable grant race; an enable pin re-asserted (driven LOW = enabled) after fault deassertion | S4 | P2 | 8 (ALARP) | SPI command handler checks state machine state before processing any enable grant; `FAULT` state silently discards all enable grants and returns `ENABLE_REJECTED`; state machine entry order: fault fires → state set to `FAULT` → SPI handler checks state | S2 | P1 | 2 | ACCEPTABLE |
...
| FMEA-M01-05 | State machine initialised incorrectly — device boots with stimulation enabled | Patient receives stimulation without consent or session start | S5 | P2 | 10 (UNACCEPTABLE initial) | **Re-authored Rev 11 against the code (OI-FMEA-11).** (i) `np_hal_gpio_init()` presets every enable pin HIGH (disabled) via `BSRR` **before** switching `MODER` to output, so no driven-LOW window exists in software (`NP-SW-CI-001` §4.4.2). (ii) `np_gpio_mgr_init()` then writes all ten pins disabled. (iii) `s_state` is zeroed, so `granted_mask` = 0. Nothing is granted before a valid heartbeat, and nothing is granted while `SIG_PENDING` is set before the session signature verifies. All of this runs before `np_hal_spi_slave_init()` (`np_safety_main.c:202-208`). (iv) Between the power rail rising and the first instruction, the pins are held disabled **only by the external pull-up** (`NP-SW-CI-001` OI-SWCI-27; `NP-HW-HUB-001` HUB-REQ-C06 (b)). *Verification:* `np_hal_platform_tests` (the preset is observed, mutation-verified); `np_gpio_mgr_tests` (init writes all ten disabled). *Withdrawn:* `np_gpio_enable_init()` and the **boot read-back self-test** did not exist. **No enable pin is read back**, so a pin stuck LOW or a missing pull-up is undetected at boot. *Why the residual rose from S2 to S5:* no control here limits the harm once it occurs, so severity stays S5, following the Rev 8 precedent for FMEA-M05-06. **ALARP justification is incomplete:** a boot read-back is a feasible further reduction. **Decided 2026-09-25 (`NP-RISK-004` OI-RISK4-06), for the cranial line only:** `NP-HW-HUB-001` HUB-REQ-C07 reads back `PBM_CRANIAL_PERMIT` and holds an independent vault-feed cut off until it reads disabled. That is **not built** (OI-HUB-C23), so no score moves yet; the cranial case is re-derived when it is. **For the other nine lines no read-back was chosen**, so their ALARP justification must be written as it stands | S5 | P1 | 5 | **ALARP — cranial: re-derive on OI-HUB-C23; other nine: justification to be written** |

---

### 3.2 SW01-M02 — SPI Heartbeat Watchdog (`np_spi_watchdog.c/.h`)

> **Rev 11: FMEA-M02-05 is re-authored against the code (OI-FMEA-11). The rest of §3.2 is NOT.**
> The description below says the watchdog timer is "a hardware TIM peripheral (not a software
> counter)". The code compares a SysTick-backed millisecond counter in software
> (`np_hal_get_tick_ms()`). FMEA-M02-02 and FMEA-M02-04 rely on a hardware IWDG, and none is
> configured. Those rows stay as written and are **not to be relied on** until `NP-RISK-002`
> **OI-RISK2-05** re-derives them.

**Description:** SW01-M02 monitors the 200 ms SPI heartbeat from SW-02 (the i.MX RT1062 main processor). If no valid heartbeat is received within 1,500 ms, the watchdog fires and commands SW01-M01 to drive all stimulation enable pins HIGH (disabled) within ≤50 ms. This is the primary interlock against SW-02 software crashes, hangs, or communication failures during a session.
...
| FMEA-M02-04 | SNVS register write fails on warm reset — watchdog countdown state lost | Watchdog timer resets its countdown on a warm reset even if SW-02 is still unresponsive; stimulation continues | S4 | P2 | 8 (ALARP) | Watchdog is a hardware IWDG — on any reset (warm or cold) the IWDG resumes counting from its initial value; the Safety MCU also drives all stimulation enable pins HIGH (disabled) during the reset vector before re-entering normal operation | S2 | P1 | 2 | ACCEPTABLE |
| FMEA-M02-05 | Watchdog fires but SW01-M01 fault handler is not called — enable pins not driven HIGH (disabled) | SW-02 crash not handled; stimulation continues | S5 | P2 | 10 (UNACCEPTABLE initial) | **Re-authored Rev 11 against the code (OI-FMEA-11).** There is no separate fault handler to fail to call. `np_spi_watchdog_check()` and `np_gpio_mgr_apply()` both run **unconditionally on every main-loop iteration**. At `NP_SAFETY_WDG_TIMEOUT_MS` (1,500 ms) after the last valid heartbeat, `check()` zeroes `granted_mask` and sets `WATCHDOG`/`CUTOFF`, and `apply()` drives all ten pins HIGH in the same iteration. The mask stays 0 until a valid heartbeat arrives. The comparison is wrap-safe unsigned arithmetic. *Verification:* `np_gpio_mgr_tests` §5: no cutoff at 1,499 ms, cutoff at exactly 1,500 ms, all ten lines disabled by the next `apply()`, the mask held at 0, and correct behaviour across the 32-bit tick wrap; each was mutation-checked (`>=` → `>` is caught). Bench latency is `OI-FMEA-01`. *Withdrawn:* the "hardware IWDG reset", `np_gpio_enable_cutoff_all()` and "cutoff inside the ISR" did not exist; **the safety MCU configures no IWDG**. *Residual scenario:* **a hung or stalled main loop runs neither `check()` nor `apply()`, and the pins hold their last state, which may be enabled.** Nothing independent backstops that. Severity stays S5 (Rev 8 precedent). **ALARP justification is incomplete:** an IWDG is a feasible further reduction (`NP-RISK-002` **OI-RISK2-05**) | S5 | P1 | 5 | **ALARP — justification pending OI-RISK2-05** |
...
FMEA-M03-02's residual score depends on, were **specified but not built** until `NP-FW-HUB-001`
Rev 9 (§8.3.1, 2026-09-25) built them for BES/tACS and tDCS. What is still missing is carried as `OI-FMEA-09`: a derived threshold, a real delivered-current read, and coverage for VNS and cervical VNS. It was `OI-FMEA-07`'s
...
**Description:** SW01-M05 is the highest-criticality module in the Safety MCU firmware. It implements the cardiac interlock for the T2 cervical VNS accessory (stimulation near the carotid sheath). The module monitors the `RPEAK_IN` GPIO pulse from SW-02 (5 ms pulse per detected R-peak), computes instantaneous heart rate from the R-R interval measured by TIM2 (1 MHz, 1 µs resolution), maintains a rolling 5-second HR window, and de-asserts the cervical VNS enable (drives the active-low `NP_EN_CVNS` pin HIGH = disabled) within one TIM6 ISR period (5 ms, 200 Hz) when the HR change exceeds 15 BPM from baseline. Total worst-case cutoff latency is <5.1 ms (well within the 100 ms specification).
...
**Description:** SW01-M06 performs AC impedance measurement at 1 kHz on stimulation electrodes before enabling any session. The measurement is synchronous and blocking — no stimulation enable pin can be asserted (driven LOW = enabled) until impedance passes. For electrical stimulation modalities (tDCS, BES, VNS, cervical VNS), electrode impedance must be within the specified range for the electrode type. For PBM (optical), impedance of the EEG electrode contacts is checked (not optical path). Impedance results are reported to SW-02 via SPI for UHDR logging (raw values) and SHDR logging (pass/fail boolean).
...
|---|---|---|---|---|---|
| SW01-M01 | Stimulation Enable GPIO Management | S5×P2 = 10 (FMEA-M01-05) | UNACCEPTABLE | **S5×P1 = 5 (FMEA-M01-05, Rev 11)** | **ALARP** — cranial read-back decided (`NP-HW-HUB-001` HUB-REQ-C07), not yet built; the other nine lines need a written ALARP justification |
| SW01-M02 | SPI Heartbeat Watchdog | S5×P2 = 10 (FMEA-M02-02, -03, -05) | UNACCEPTABLE | **S5×P1 = 5 (FMEA-M02-05, Rev 11)** | **ALARP** — justification pending `NP-RISK-002` OI-RISK2-05 (no IWDG). M02-02's S3×P1 also rests on the missing IWDG |
...
**All residual risks are ACCEPTABLE per NP-RM-001 §4.3 acceptability criteria.** *(Not true since Rev 8, and less so at Rev 11: SW01-M01, SW01-M02 and SW01-M05 carry ALARP residuals, see the rows above.)*
...
flag. **Rev 10: that control is now built** (`NP-FW-HUB-001` Rev 9 §8.3.1), and the rating **stays
conditional** (`OI-FMEA-09`). Its thresholds are unvalidated placeholders, the delivered-current read
it compares against (`OI-STIM-06`) is a HAL stub with no hardware behind it, and auricular and
cervical VNS have no delivered-current read to compare. It is recorded as conditional rather
...
| Delivered charge diverging from commanded | SW01-M03 (residual); hub cross-check + SHDR divergence flag (`NP-FW-HUB-001` §8.3.1; built for BES/tACS and tDCS, placeholder thresholds, `OI-FMEA-09`) | FMEA-M03-02 |
...
|---|---|---|---|
| OI-FMEA-01 | Hardware bench validation of SW01-M02 watchdog timeout timing (FMEA-M02-02, M02-05): oscilloscope measurement of cutoff latency from heartbeat timeout (FMEA-M02-05 as re-authored at Rev 11; *"from IWDG reset"* until then, and **no IWDG exists**, `OI-RISK2-05`) until **every stimulation enable pin reads HIGH** (the `…_EN#` pins are active-low, `np_safety_config.h:7-8`) **and every downstream active-high enable net reads LOW** (for the cranial lattice, `PBM_CRANIAL_PERMIT`, `NP-HW-HUB-001` HUB-REQ-C06); must confirm ≤50 ms at **both** nodes. **Criterion restated Rev 11 (2026-09-25, GitHub #437).** It read *"…to all stimulation GPIO LOW"*, but at the MCU pin LOW is the *enabled* level, so as written the criterion passed on the unsafe state (`NP-RISK-004` §2.2, adopted). The same inverted wording in §3.1, §3.2, §3.5, §3.6 and §3.7 was restated in the same revision (see the §3 note). Requires hardware bench (FAI-HUB bench). | HW team | Full G1 gate closure for SW01-M02 |
...
| ~~OI-FMEA-11~~ | **✅ CLOSED 2026-09-25 (Rev 11), at the principal's direction as interim Quality authority.** FMEA-M01-01, -M01-05 and -M02-05 are re-authored against the code. The missing verification is built (`np_gpio_mgr_tests`, the eleventh Class C host target, mutation-checked), and the residuals are re-derived: M01-01 → S4×P1 ACCEPTABLE; M01-05 and M02-05 → **S5×P1 ALARP**, with justification pending `NP-RISK-004` OI-RISK4-06 (read-back) and `NP-RISK-002` OI-RISK2-05 (IWDG). No firmware behaviour changed. Original text: **Three mitigations, and the residual scores resting on them, describe safety-MCU code that does not exist** (found 2026-09-25 while restating enable-pin polarity, GitHub #437). **FMEA-M01-01** claims the watchdog cutoff is "a separate ISR that operates on GPIO directly, bypassing bitmask". It is not: `np_spi_watchdog.c` clears `granted_mask` and `np_gpio_mgr_apply()` writes the pins. **FMEA-M01-05** claims `np_gpio_enable_init()` and a boot self-test that reads the enable pins back. Neither exists, and no enable pin is ever read back. **FMEA-M02-05** claims `np_gpio_enable_cutoff_all()` in the reset vector and a cutoff "inside the ISR". Neither exists; the real path is `np_hal_gpio_init()`'s `BSRR` preset in `main()`. §3.1's heading also names `np_gpio_enable.c/.h`, and the module is `np_gpio_mgr.c`. **Re-derive each residual (S/P) against the code as built. Do not treat the stated residuals as supported until then**, per the Rev 4 precedent. M01-05's residual S2/P1 in particular cites the read-back, and whether to build one is `NP-RISK-004` **OI-RISK4-06**. No score was changed when this was raised | Quality / Firmware | Full G1 gate closure for SW01-M01, SW01-M02 |
...
| 10 | 2026-09-25 | NeurOne Firmware | **FMEA-M03-02's control built; no risk score changed (`OI-FMEA-09`, GitHub #386).** The hub now compares delivered BES/tACS and tDCS current with commanded current and raises one SHDR divergence flag per channel per session (`np_stim_xcheck.c`, `NP-FW-HUB-001` Rev 9 §8.3.1). It also writes a UHDR commanded-dose record per dispatched command and a synced SHDR session-open marker. §3.3's mitigation cell, the §4 footnote and §5.3's table now say *built*, not *not yet built*. The residual **stays conditional**, and the reasons are named: placeholder thresholds, a stub delivered-current read (`OI-STIM-06`), no VNS or cervical VNS coverage, and a flag that does not cut the channel. Re-scoring is Quality's decision once those close |
| 11 | 2026-09-25 | NeurOne Systems Engineering + principal | **Enable-pin polarity wording restated throughout; no failure mode, score or mitigation changed (GitHub #437).** New §3 note: all enable levels are stated at the safety-MCU pin, which is active-LOW (HIGH = disabled). **OI-FMEA-01's** pass criterion measured cutoff to "all stimulation GPIO LOW", which is the *enabled* pin level, so it passed on the unsafe state. It now requires the pins HIGH **and** the downstream active-high nets (`PBM_CRANIAL_PERMIT`) LOW, both within 50 ms (`NP-RISK-004` §2.2 adopted; `NP-HW-HUB-001` HUB-REQ-C06). **Restated in place, with the original wording given here:** FMEA-M01-01 "GPIO remain HIGH after fault" and "before driving GPIOs LOW"; M01-02 "GPIO re-asserted HIGH"; M01-05 "initialised LOW (disabled)" and "reads back as LOW"; §3.2 "drive all stimulation GPIO LOW"; M02-04 "drives all stimulation GPIO LOW during the reset vector"; M02-05 "GPIO not driven LOW"; §3.5 "asserts the cervical VNS disable GPIO LOW (active-low enable)", which drove an active-low enable to its *enabled* level; §3.6 "no stimulation GPIO can be asserted HIGH"; §3.7 "keeps all GPIO LOW"; M07-01 "all GPIO LOW". **OI-FMEA-11 raised and closed in this revision, at the principal's direction as interim Quality authority.** The mitigations of M01-01, M01-05 and M02-05 cited code that does not exist: a state machine, an ISR cutoff, `np_gpio_enable_init()`, `np_gpio_enable_cutoff_all()`, a boot read-back self-test, and a hardware IWDG. **All three rows are re-authored against the code, and their residuals CHANGED:** M01-01 S2×P1 → **S4×P1 ACCEPTABLE**; M01-05 S2×P1 → **S5×P1 ALARP**; M02-05 S2×P1 → **S5×P1 ALARP**. Severity no longer falls with mitigation where no control limits the harm, following the Rev 8 precedent. Both ALARP justifications are incomplete, because a feasible reduction exists for each: `NP-RISK-004` OI-RISK4-06 (read-back) and `NP-RISK-002` OI-RISK2-05 (IWDG). **Verification built:** `np_gpio_mgr_tests`, the eleventh Class C host target (18 assertions; 5 mutations of `np_gpio_mgr.c`/`np_spi_watchdog.c` each caught; CI count 10 → 11, total 48 → 49). §3.1 and §3.2 gain banners. §3.2's "hardware TIM" claim and M02-02/-04's IWDG are **not** re-authored and are routed to OI-RISK2-05. §4's summary follows. OI-FMEA-01's latency origin is restated. **No firmware behaviour changed.** |
