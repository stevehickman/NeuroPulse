...
> wording.** Where a mitigation describes code that does not exist, it is marked ⚠ and routed to
> **OI-FMEA-11**.
...
|---|---|---|---|---|---|---|---|---|---|---|
| FMEA-M01-01 | Stale enable bitmask not cleared on fault entry — one or more enable pins remain asserted (LOW = enabled) after fault | Stimulation continues after fault condition detected (e.g., overheat fault fires but one PBM zone LED continues driving) | S4 | P2 | 8 (ALARP) | State machine fault entry function unconditionally clears all bits in enable mask before driving the enable pins HIGH (disabled); code path is unit-tested with 100% branch coverage; watchdog cutoff path is a separate ISR that operates on GPIO directly, bypassing bitmask. ⚠ *Not what the code does:* the SPI watchdog clears `granted_mask`, and `np_gpio_mgr_apply()` writes the pins from the main loop (`np_spi_watchdog.c:7`, `np_gpio_mgr.c`). No ISR bypasses the bitmask (**OI-FMEA-11**) | S2 | P1 | 2 | ACCEPTABLE |
...
| FMEA-M01-05 | State machine initialised incorrectly — device boots with stimulation enabled | Patient receives stimulation without consent or session start | S5 | P2 | 10 (UNACCEPTABLE initial) | All enable pins initialised HIGH (disabled) in `np_gpio_enable_init()` before any other module initialises; boot self-test verifies GPIO state reads back as HIGH (disabled) before any SPI communication begins; ⚠ *Not what the code does:* no `np_gpio_enable_init()` exists. The pins are preset HIGH by `np_hal_gpio_init()` (via `BSRR`, before `MODER`) and then `np_gpio_mgr_init()`, both before `np_hal_spi_slave_init()` (`np_safety_main.c:202-208`). **No boot read-back self-test exists**; `np_hal_pin_read()`'s only caller is the SPI NSS watch (**OI-FMEA-11**; `NP-RISK-004` OI-RISK4-06); initial state is `IDLE` with empty enable mask | S2 | P1 | 2 | ACCEPTABLE |
...
| FMEA-M02-05 | Watchdog fires but SW01-M01 fault handler is not called — enable pins not driven HIGH (disabled) | SW-02 crash not handled; stimulation continues | S5 | P2 | 10 (UNACCEPTABLE initial) | Watchdog fires via hardware IWDG reset; reset vector unconditionally calls `np_gpio_enable_cutoff_all()` at the earliest possible point in boot sequence before any SPI initialisation; GPIO cutoff happens inside the ISR, not via a function call from a separate module. ⚠ *Not what the code does:* no `np_gpio_enable_cutoff_all()` exists, and the cutoff is not in an ISR. After an IWDG reset, `main()` calls `np_hal_gpio_init()`, which presets every enable pin HIGH before any SPI initialisation (`np_safety_main.c:202-203`). On a heartbeat timeout **without** a reset, the watchdog clears `granted_mask` and `np_gpio_mgr_apply()` writes the pins (**OI-FMEA-11**) | S2 | P1 | 2 | ACCEPTABLE |
