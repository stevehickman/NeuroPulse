...
| 11 | 2026-09-25 | NeurOne Systems Engineering + principal | **OI-FMEA-01's pass criterion restated; no failure mode, score or mitigation changed (GitHub #437).** It measured cutoff to "all stimulation GPIO LOW". At the MCU pin, LOW is the *enabled* level (`np_safety_config.h:7-8`), and only after the inverting buffer is LOW the safe level. So the criterion passed on the unsafe pin state. It now requires the enable pins HIGH **and** the downstream active-high nets (`PBM_CRANIAL_PERMIT`) LOW, both within 50 ms, following `NP-RISK-004` §2.2 as adopted and `NP-HW-HUB-001` HUB-REQ-C06. **Found and not edited:** §3.2's description, FMEA-M01-01 and FMEA-M02-05 use the same pin-level-unstated wording. They are recorded on OI-FMEA-01 for Quality to restate. |
