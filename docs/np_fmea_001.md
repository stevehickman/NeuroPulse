...
>
> **The ADC premise is not a simple oversight to reinstate.** The substitution is deliberate and
> recorded in the source: commanded current is SHDR, whereas ADC-measured actual current *"would
> be UHDR-class"* (CLAUDE.md §5.1). A monitor fed commanded current can only verify what was
> *asked for*, never what reached the scalp, so it cannot detect a driver or electrode fault that
> makes delivered charge diverge from commanded — a different safety argument from the one this
> section makes, and one no document has yet stated.
>
