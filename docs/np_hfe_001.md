...
|---|---|---|---|---|
| CT-01 | Insert PBM zone module into correct slot / orientation | Wrong zone inserted; module inserted at wrong contact angle giving poor thermal/optical contact | RISK-15 five-layer keying (asymmetric mechanical key + ZONE_ID resistor ladder + braille/raised numeral + tactile dots + bone-conduction audio confirmation) — errors are prevented by keying, not merely detected. **⚠ SUPERSEDED 2026-07-28:** ZONE_ID detection is retired (see `docs/np_fw_za_001.md`); under the hex-tile lattice and the SMART-1 decision (every socket accepts any type), the "wrong slot" failure mode this task tests for may no longer exist for type — re-scope this task once the hex-tile insertion UX is designed. | Formative (confirm keying is discoverable without instructions) |
