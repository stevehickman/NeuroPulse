...
| Usability | Incorrect zone module insertion; confusion between modalities; failure to notice thermal warning | **Primary control — `np_module_map_check_placement()`:** the live UID-derived inventory is validated against the running protocol's required module map, and a mismatch **blocks the session** (`docs/np_hex_zm_001.md` §4a; gate SW-1). Orientation is enforced by a non-rotationally-symmetric mating feature. Accessible position identification per **NP-HFE-002** (`docs/np_hfe_002.md`): tactile landmark grid on the shell, module-*type* tactile marking, companion-app guided placement with spoken/braille/haptic confirmation. (RISK-15, **REDESIGNED 2026-07-28** — mechanical type/position keying retired under SMART-1; **accessible position-ID gap closed 2026-07-31 by NP-HFE-002**; see `docs/np_hfe_001.md` CT-01.) Bone conduction is confirmed unusable at insertion — it requires head contact, unavailable while accessing a socket (`docs/superseded/np_fw_za_001.md`) — and is retained for while-worn alerts only. Non-dismissible safety alerts |
...
>
> NP-RISK-001 was scoped, in its own title, to the Zone Module FPC — an assembly the 2026-07-15 hex-tile decision (`NP-HEX-ZM-001`) deleted. It nonetheless remained the ISO 14971 baseline for four months, because this paragraph named it and nothing else did.
>
> **The risk file of record is now `NP-RISK-002` (index and disposition) together with `NP-RISK-003` (hex-tile module) and `NP-RISK-004` (shell, socket, interconnect and hub), effective 2026-08-11.** NP-RISK-001 Rev 3 is superseded and retained in `docs/superseded/`.
>
> **No risk was removed by the re-baseline.** `NP-RISK-002` §3 dispositions all twenty-six RISK IDs individually: nine RETIRED (the mechanism no longer exists in the design), fifteen CARRIED into the per-artifact registers with their original IDs and meanings, and one — RISK-24 — CLOSED-CONFIRMED, having been investigated and found true, with the ≥15 mm PBM-to-EEG separation requirement withdrawn and replaced by the <5 µVpp artifact threshold it was a proxy for.
>
> **The `RISK-NN` sequence is closed at 26 and remains append-only** (`NP-CONV-001` §6). Hazards arising after the re-baseline take artifact-prefixed IDs (`RISK-HEX-nn`, `RISK-SHELL-nn`, `RISK-HUB-nn`); no retired number is reused, because a reused number would silently re-point the thirty-eight references that cite the old one.
>
> **Nothing was re-scored.** Disposition and re-rating are separate acts under §8.2, and mixing them would hide which judgements changed. Two carried risks plainly no longer hold their original ratings — RISK-16 (perimeter seals 5 → ~30) and RISK-22 (extraction ≤1 N at a lever tip → 34.2–57.0 N of plate load) — and both are held for change orders under `NP-RISK-002` OI-RISK2-01.
>
> **Gap surfaced by the re-baseline:** three shipping T1 modalities — intranasal PBM, neural audio entrainment, and the auricular VNS/HRV clip — have **never had a hazard analysis** in any revision of this risk file. They were invisible while the file was zone-module-shaped. Tracked as `NP-RISK-002` OI-RISK2-02, ~~blocked behind `NP-ART-001` OI-ART-04 (those artifacts have no specification to analyse)~~ — **unblocked 2026-09-20 (GitHub #332): all three now have owning specifications** (`NP-HW-AUDIO-001`, `NP-HW-NASAL-001`, `NP-HW-VNSCLIP-001`), each supplying an unscored hazard-input list in its §7. **The analyses themselves have still not been performed**, and the gap in this risk file is therefore unchanged; see `NP-RISK-002` Rev 3 §4.1. The item is also extended to A15, which has no risk register entry of any kind.
...
- 1 risk: **MITIGATED — HARDWARE BENCH PENDING:**
  - **RISK-25:** Cervical VNS cardiac reflex. Safety MCU firmware specification complete (NP-FW-CVNS-001 Rev 1). FAI-CV02 hardware timing bench required to confirm <100ms cutoff. Remains software-baselined; full closure requires T2 prototype bench.

**Subsequent register note — RISK-15 (usability, incorrect zone module insertion): control redesigned, severity re-rating PENDING as a change order (2026-07-31).** The five-layer keying scheme RISK-15 was written against is retired (SMART-1, `docs/np_hex_zm_001.md` §4a), and its accessible position-identification gap is closed by **NP-HFE-002 Rev 1**. NP-HFE-002 §10 records a consequence that affects this risk's rating and is flagged here rather than silently applied: the wrong-site-dosing path RISK-15 originally guarded is now closed in software by `np_module_map_check_placement()`, which blocks a session whose live inventory does not match the protocol's required module map. Where a map names a type at a socket, misplacement is detected and blocked before any energy is delivered; where a map is DONT-CARE, misplacement is therapeutically equivalent by that map's own declaration. The residual harm from a placement error is therefore a blocked session and rework — recommended re-rating **S2**, not S3+. **This re-rating is not made here.** Per §8.2 it requires a change order under NP-QMS-DC-001 §8.1 recording pre- and post-change ratings with Quality Lead approval (NP-HFE-002 OI-HFE2-07). One authoring dependency is load-bearing and is tracked as NP-HFE-002 HFE-R-15: a required map must not use DONT-CARE at any socket where module type materially changes the delivered wavelength (a T1-C substituted for a T1-A stays inside firmware dose ceilings but delivers a wavelength the protocol's evidence base does not describe).
...
|---|---|---|---|
| 1 | 2026-05-13 | Interim Quality (CEO) | Initial release. Risk Management Plan established at QMS formation. RISK-01 through RISK-25 formally entered under QMS change control. |
| A (in-doc note) | 2026-07-31 | Interim Quality (CEO) | §7 usability hazard row and §8.1 updated for **NP-HFE-002 Rev 1**: RISK-15's accessible position-identification gap is closed by the layered design (tactile landmark grid, module-type tactile marking, companion-app guided placement) with `np_module_map_check_placement()` as the primary control. Severity re-rating to S2 is **recommended and flagged, not applied** — it requires a change order per §8.2 / NP-QMS-DC-001 §8.1. No rating in the risk file has been changed by this note. |
| B (in-doc note) | 2026-09-20 | Interim Quality (CEO) | §8's *"Gap surfaced by the re-baseline"* note is updated: `OI-RISK2-02` is **unblocked** — A11, A12 and A13 gained owning hardware specifications on 2026-09-20 (GitHub #332), so the stated blocker (*"those artifacts have no specification to analyse"*) no longer holds, and the item is extended to A15. **The gap itself is unchanged: no hazard analysis has been performed for any of them**, and no risk is added, removed, re-scored or re-classified by this note. File of record for the change is `NP-RISK-002` Rev 3 §4.1. |
