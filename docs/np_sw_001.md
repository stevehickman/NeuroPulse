...
| LittleFS | **v2.11.3**, tag commit `6cb4e865` (pinned + vendored 2026-09-14, `OI-LFS-01` CLOSED). **PINNED AND PRESENT — NOT YET INTEGRATED OR VERIFIED**, see the verification cell | SW-02 | Class B (conditionally — see the note below) | **SOUP record and hazard analysis: NP-SOUP-LFS-001 Rev 2; vendored record `firmware/vendor/littlefs/VERSION`.** Vendored byte-exact from tag `v2.11.3` — `lfs.c`, `lfs.h`, `lfs_util.c`, `lfs_util.h`, `LICENSE.md` (BSD-3-Clause per the SPDX identifier in every file) — with **per-file SHA-256** verified by two independent downloads, upstream's `bd/`, `tests/`, `benches/`, `runners/` and `scripts/` deliberately excluded. **The tag choice is a safety argument**: `v2.11.3` is the first release carrying upstream `488e84bb`, which fixes data corruption arising from two open write handles on one file — a defect on exactly the axis claims `L-1…L-4` rest on. **Configuration is part of the record** and is NeurOne-authored (`firmware/hub_control/include/np_lfs_config.h`, `np_lfs_instance.h`, `src/np_lfs_instance.c`), on the FreeRTOSConfig.h precedent: `LFS_NO_MALLOC` with static buffers, `LFS_THREADSAFE` on, assertions retargeted to a halt that hands control to the safety MCU, `block_cycles` **500** set explicitly and checked *by value* because `-1` passes every non-zero test and silently disables the wear levelling claim `L-6` rests on, and `np_lfs_config_validate()` as the mount-time check claim `L-5` names — littlefs's own assertions check that a configuration is self-consistent and **cannot check that it is NeurOne's**. **WHAT NEURONE HAS RUN: `np_lfs_config_tests`** (Class B host target 28), which recomputes every vendored file's SHA-256 against this record, compares the build configuration against it, pins the `EMMC-FS-01` instance parameters and requires **19 single-field perturbations to be rejected**; falsified in seven ways before being trusted, per `NP-CONV-001` §8. **WHAT NEURONE HAS NOT RUN, and what this cell must not be read as claiming: any power-loss behaviour at all.** Nothing calls the library — `OI-LOG-05..07` are unimplemented — and `IEC 62304 §7.1.2` is **still not discharged**: the published anomaly list for `v2.11.3` has not been read or assessed against the seven claims in NP-SOUP-LFS-001 §3. That, plus a NeurOne power-loss injection test against `L-1…L-4` falsified in both directions, is **`OI-LFS-02`, which inherited `OI-LFS-01`'s BLOCKING status** for any reliance on `NP-FW-NVRAM-001` §4, `EMMC-FS-01` or `NP-FW-HUB-001` §6.5. Pinning unblocked that work; it did not perform it (NP-SOUP-LFS-001 §7.5). The hazard analysis IS performed and is version-independent; its conclusion is in the Class B note below. Feeds #75 (`NP-SBOM-001`) — **which this now satisfies**, an SBOM entry reading `2.x` having been the original complaint. |
