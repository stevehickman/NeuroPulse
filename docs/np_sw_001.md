...
| LittleFS | **NOT PINNED — and NOT INTEGRATED** (see `OI-LFS-01`) | SW-02 | Class B (conditionally — see the note below) | **SOUP record and hazard analysis: NP-SOUP-LFS-001 Rev 1.** The previous cell read *"2.x"* / *"Power-loss testing per LittleFS test suite"*, which named no version and recorded nothing NeurOne did — it asserted that an upstream project tests its own code. `OI-NVRAM-12` raised the component as the only Class B SOUP item neither vendored, pinned nor anomaly-evaluated; checking it found something stronger: **it is not in the tree at all.** `grep -rn "lfs_" firmware/` returns two hits, both comments in `np_log_backend.h` naming `lfs_file_write`/`lfs_file_sync` as what the `OI-LOG-06/07` HAL seams will call. There is no source under `firmware/vendor/`, nothing links it and nothing calls it — it is **nominated** SOUP, not integrated SOUP, and every power-loss atomicity guarantee in `NP-FW-NVRAM-001` §4, `EMMC-FS-01` and `NP-FW-HUB-001` §6.5 currently rests on it. **IEC 62304 §7.1.2 is deliberately NOT discharged** (NP-SOUP-LFS-001 §7): the clause evaluates the anomaly list for the version *actually in use*, and none is; recording a conclusion against a major-version family would be a record that resolves to nothing. **The hazard analysis IS performed** (NP-SOUP-LFS-001 §4–§6) and does not depend on a version, because what a filesystem anomaly can *reach* is a property of the callers. Its conclusion is in the Class B note below. Verification before any reliance: pin + vendor (`OI-LFS-01`), then the §7.1.2 evaluation plus a NeurOne power-loss injection test against claims `L-1…L-4`, falsified in both directions first per `NP-CONV-001` §8 (`OI-LFS-02`). Feeds #75 (`NP-SBOM-001`) — an SBOM entry reading `2.x` is not an SBOM entry. |
