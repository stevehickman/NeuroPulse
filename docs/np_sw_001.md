...
**Rev 9 (2026-09-24):** **§9.4 SOUP: the LittleFS row cites NP-SOUP-LFS-001 Rev 4** (GitHub #382). The caller rules the anomaly evaluation found the Class B argument depends on are built — `np_cfg_store` and `np_lfs_log_instance`, two new Class B host targets — and `REQ-LFS-01` is enforced by `scripts/check-lfs-caller-rules.ts`. `OI-LFS-03`, `-05`, `-06`, `-08`, `-09` closed; `OI-LFS-10` and `-11` raised; `OI-LFS-07` open. No classification changed: Class B stands on the same caller condition, which is now code rather than prose.
...
|---|---|---|---|---|
| FreeRTOS-Kernel | V11.3.0 (FreeRTOS-LTS 202604.00-LTS) | SW-02 | Class B | Vendored byte-exact in `firmware/vendor/freertos/` (SOUP record `VERSION`, MIT license); subset = ARM_CM7 r0p1 port + heap_4 (shipped) + POSIX port (host test only); `croutine.c` excluded. Configuration `firmware/hub_control/include/FreeRTOSConfig.h` enables `configCHECK_FOR_STACK_OVERFLOW=2` + `configUSE_MALLOC_FAILED_HOOK=1` (hooks in `np_hub_freertos_hooks.c`, fail-safe halt). Host verification: `np_freertos_smoke_tests` compiles the kernel + this config against the POSIX port and runs the scheduler (event group + `vTaskDelayUntil` + prioritised tasks) — passes in CI — today in `firmware-cross-build.yml`'s Class B host-test job, which absorbed the retired `firmware-host-tests.yml` (NP-SW-CI-001 OI-SWCI-09; the target's class is recorded in `ci/host-test-partition.txt`). FreeRTOS-Kernel known-anomalies list to be reviewed and recorded at G2. **On-target ARM_CM7 build is no longer pending (2026-09-01, NP-SW-CI-001 §4.8):** the kernel, this configuration and the ARM_CM7 r0p1 port are compiled for the i.MX RT1062 and linked into `np_application.elf`, whose FreeRTOS heap (`ucHeap`, `configTOTAL_HEAP_SIZE` = 64 KiB) is the single largest object in the image. `vPortSVCHandler`/`xPortPendSVHandler`/`xPortSysTickHandler` resolve against the vendored MCUX vector table through the `FreeRTOSConfig.h` aliases, with no FreeRTOS-specific entries added to the startup file. **The image links; it does not run** — the SW-02 platform layer is 94 traps (NP-SW-CI-001 §4.8.4, §4.10). **`configCPU_CLOCK_HZ`'s 600 MHz is still not established by anything in the boot path, but it is no longer assumed** (2026-09-02, NP-SW-CI-001 §4.10, OI-SWCI-41 partially closed): the board clock configuration is a counted platform seam (`np_platform_clock_init()`, the first platform call the image makes), and `main()` verifies the result against the CCM via the vendored `SystemCoreClockUpdate()` and against `configCPU_CLOCK_HZ` before `vTaskStartScheduler()`, halting into the safety-MCU cutoff path on any mismatch. This matters for this SOUP entry specifically: the ARM_CM7 port derives `configSYSTICK_CLOCK_HZ` from `configCPU_CLOCK_HZ`, so a wrong core clock rescales every kernel interval uniformly rather than failing — at the SDK's own 528 MHz default a 200 ms heartbeat becomes 227 ms and stays inside the 1.5 s watchdog. Host test `np_app_clock_agreement_tests` asserts those figures. |
| LittleFS | **v2.11.3**, tag commit `6cb4e865` (pinned + vendored 2026-09-14, `OI-LFS-01` CLOSED; anomaly-evaluated and power-loss tested 2026-09-14, `OI-LFS-02` CLOSED). **PINNED, EVALUATED AND TESTED — STILL NOT INTEGRATED**, see the verification cell | SW-02 | Class B (conditionally — see the note below) | **SOUP record and hazard analysis: NP-SOUP-LFS-001 Rev 7; vendored record `firmware/vendor/littlefs/VERSION`.** *Rev 4 (2026-09-24, §13): the caller rules §11 found the applicable anomalies depend on are built and tested — `np_cfg_store` (one handle per file, content-verified reads with a remount after any read error, no remove/rename, two-copy `ukmd.rec`) and `np_lfs_log_instance` (UHDR/SHDR parameters); `REQ-LFS-01` is a CI gate (`scripts/check-lfs-caller-rules.ts`); upstream #1205 reproduced and shown not to reach a caller. `OI-LFS-03`, `-05`, `-06`, `-08`, `-09` closed; `OI-LFS-10` (the Config instance disagreed with EMMC-FS-01 — closed in Revs 5–6; ECR-EMMC-002 applied, NP-FW-EMMC-001 Rev 3) and `OI-LFS-11` raised (closed in Rev 7 — UHDR logs one file per session; `OI-LFS-12` raised); `OI-LFS-07` open. Still not integrated — `OI-LOG-05..07`.* Vendored byte-exact from tag `v2.11.3` — `lfs.c`, `lfs.h`, `lfs_util.c`, `lfs_util.h`, `LICENSE.md` (BSD-3-Clause per the SPDX identifier in every file) — with **per-file SHA-256** verified by two independent downloads, upstream's `bd/`, `tests/`, `benches/`, `runners/` and `scripts/` deliberately excluded. **The tag choice is a safety argument**: `v2.11.3` is the first release carrying upstream `488e84bb`, which fixes data corruption arising from two open write handles on one file — a defect on exactly the axis claims `L-1…L-4` rest on. **Configuration is part of the record** and is NeurOne-authored (`firmware/hub_control/include/np_lfs_config.h`, `np_lfs_instance.h`, `src/np_lfs_instance.c`), on the FreeRTOSConfig.h precedent: `LFS_NO_MALLOC` with static buffers, `LFS_THREADSAFE` on, assertions retargeted to a halt that hands control to the safety MCU, `block_cycles` **500** set explicitly and checked *by value*, and `np_lfs_config_validate()` as the mount-time check claim `L-5` names. **WHAT NEURONE HAS RUN.** (1) `np_lfs_config_tests` (Class B host target 28): recomputes every vendored file’s SHA-256 against this record, compares the build configuration against it, pins the `EMMC-FS-01` instance parameters and requires **19 single-field perturbations to be rejected**; falsified in seven ways first. (2) **`np_lfs_powerloss_tests` (Class B host target 29, repo 35 → 36) — the power-loss injection test `OI-LFS-02` asked for**: NeurOne’s own injecting block device (upstream’s `lfs_emubd` deliberately not vendored, because the defective cell this row replaces cited upstream's suite), a cut swept across **every** medium-touching op of three commit sequences under three tear models — **456 interrupted runs, 0 violations**, every attempt remounting cleanly — and falsified in both directions per `NP-CONV-001` §8: three unsafe orderings the same verifiers are required to catch, plus six hand perturbations of which five were caught and one was not and is recorded as a negative result (NP-SOUP-LFS-001 §12.4). (3) **The IEC 62304 §7.1.2 anomaly evaluation, performed and recorded: NP-SOUP-LFS-001 Rev 3 §11** — five queries against the upstream tracker (littlefs publishes no numbered errata document, and has no `bug` label, so an empty result there is not a clean one), plus the release notes and the `v2.11.2`→`v2.11.3` differential; thirteen items assessed against the seven claims in §3; **three applicable and none on a path to an emission** — #1086 becomes a halt the safety MCU converts into a stimulation cutoff, #1205 becomes an absence because every stored value is CRC- or AEAD-checked at the point of use, and #1210 is stopped by nothing that exists today. Re-evaluation required on any tag change. **WHAT THIS CELL MUST STILL NOT BE READ AS CLAIMING.** Nothing calls the library — `OI-LOG-05..07` are unimplemented. `L-1…L-4` hold against the **`struct lfs_config` contract**, which is what they were written against, and **not against the eMMC**: the injector interrupts the contract at `prog_size` granularity, and the medium is managed flash behind an XTS layer whose tear and erase semantics are not raw-flash semantics (`OI-LFS-07`, needs hardware). `L-3`’s result is a property of the write-temp-then-rename **ordering**, not of the component. `L-6` is configured, never measured. Two caller obligations from §11 are unmet: `OI-LFS-08` (#1210’s silent file loss, worst case `ukmd.rec`, whose loss makes a user’s UHDR permanently unmountable) and `OI-LFS-09` (validate by content on every read). `NP-FW-NVRAM-001` §4, `EMMC-FS-01` and `NP-FW-HUB-001` §6.5 may be relied on exactly as far as NP-SOUP-LFS-001 §12.5 states. The hazard analysis IS performed and is version-independent; its conclusion is in the Class B note below. Feeds #75 (`NP-SBOM-001`) — **which this satisfies**. |
...
**Class B SOUP note — LittleFS, added 2026-09-13, and it is a different shape from the two notes below.** The MCUX row's Class B argument is **reachability**: the component is compiled into `np_application` and nothing else, so an anomaly in it cannot reach SW-01. That argument is not available for LittleFS, which is why it gets its own record (**NP-SOUP-LFS-001**) rather than a line. A device-header layer fails loudly — a wrong register address — and its blast radius is one SW item. A filesystem carries **integrity guarantees for stored values that other code later trusts**, so it fails quietly, and the question is what trusts those values. Three consumer families were enumerated (NP-SOUP-LFS-001 §5): the session log, which nothing on the device reads back, so an anomaly there is records loss and not emission; UID-keyed dose calibration, which **refuses to fall back** and therefore converts corruption into no-drive rather than wrong-drive; and the `"NPMP"` module-map blob, which carries Map 1's **per-element safe ranges** and is therefore the one path from a filesystem anomaly toward drive magnitude.

**The conclusion, and the condition attached to it.** Class B stands. But it stands on a property of the **caller**, not of LittleFS: a CRC failure makes `np_module_map` discard the blob and re-enumerate from the sockets, so *every integrity failure leaves the inventory empty* (`NP-FW-NVRAM-001` §9) — an absence, never a wrong value. `NP-FW-NVRAM-001` §7.2 establishes, for independently correct reasons, that **Map 3 must have the opposite policy** — tail-additive forever, never discard-and-rebuild, because a cache and a record need opposite policies. Both decisions are right, and together they make this classification load-bearing on which file is which. Hence **`REQ-LFS-01`: no value that bounds an emission may be stored under a tail-additive policy.** If that is ever breached the classification must be re-derived, and `OI-NVRAM-10`'s residual — a thermal cutoff standing in for an optical limit — becomes the only remaining bound rather than the third of three. `OI-LFS-03` asks for the CI check that makes the rule observable rather than remembered.
