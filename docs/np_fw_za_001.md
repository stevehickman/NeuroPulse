...
>
> **Coordinate frame (2026-07-31).** A socket's physical location is its position in 3-space, millimetres, in **aircraft body axes**, origin at the centre of the approximately oblate spheroid the helmet forms a partial shell of: **+x forward** (toward the face), **+y right**, **+z down**. Right-handed. The vault sits above the origin, so every socket has z ≤ 0 and the crown is the most negative z. `scripts/sync-socket-map.ts` derives these from the scan-measured extents (interior 267 × 229 mm, rim→crown 157 mm) and emits them to `hardware/np_socket_map.json` and `socketMap.generated.ts`.
>
