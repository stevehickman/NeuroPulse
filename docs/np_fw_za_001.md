...
>
> - **SOCKET_MAP** — read ONCE when the app links. 7-byte records: 1-based socket id, packed lobe|side, flags (wired-in-shell), x_mm and y_mm as int16 little-endian. This is the authoritative, permanent description of where every socket physically is.
> - **ZONE_MODULE_STATUS** — emitted on every insertion/removal/fault. 3-byte records: socket id, module type, flags. Nothing else.
>
> Where a socket IS cannot change when a module is swapped, so it does not belong in a change notification. Putting it in a map read once per link means (a) each change costs three bytes instead of four, and (b) the map can grow — 10-20 labels, per-socket element counts, curvature — **without changing the size of anything on the hot path**. Static data is paid for once per link; dynamic data stays minimal permanently.
>
> The device remains the single source of truth for geometry either way: the app never bundles its own socket→anatomy table, which matters because this lattice has already been re-cut twice (30 → 78 → 80) and a cached copy would drift silently at the next re-cut. The app therefore does not persist the map across links.
>
> Two further properties worth naming:
> - **Frames fragment.** An 80-socket map is ~564 bytes and a full status snapshot ~244, against a 20-byte guaranteed ATT payload. The app commits a run only when the last fragment lands; a gapped run is abandoned rather than committed with a hole (a missing socket reads as "not installed" — the wrong answer for a placement gate). One `FragmentRunAssembler` governs both shapes so the ordering rules cannot diverge.
...
>
> (v1 briefly carried lobe|side inline in every 4-byte status record. It was never deployed; v2 replaced it once the recurring cost of a fixed fact was called out.)
>
...
>
> **Verification.** `np_zone_notify_tests` (370 host checks) added to the ctest suite — `zone_announce` had no host test target at all before this, being ARM-cross-only; plus `np_zone_announce_tests` (41 checks) asserting behaviourally that insertion notifies the app and starts no audio; full firmware host suite **20/20**, all four `zone_announce` sources Cortex-M7 `-Werror` clean. App suite **357/357**, with coverage for both frame shapes, kind-bit rejection, fragment reassembly, socket-map application and label resolution, change-only event emission, and the spoken confirmation.
>
