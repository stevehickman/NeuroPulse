...
>
> **Wire format (new, v2) — the static/dynamic split.** `np_zone_notify.h` defines TWO shapes over one 4-byte header (version, flags, fragment index, record count), distinguished by a KIND bit:
>
> - **SOCKET_MAP** — read ONCE when the app links. 8-byte records: 1-based socket id, flags (wired-in-shell), then x/y/z as int16 little-endian. This is where every socket physically is.
> - **ZONE_MODULE_STATUS** — emitted on every insertion/removal/fault. 3-byte records: socket id, module type, flags. Nothing else.
>
> Where a socket IS cannot change when a module is swapped, so it does not belong in a change notification. Putting it in a map read once per link means each change costs three bytes, and the map can grow — 10-20 registration, element counts, surface normals — **without changing the size of anything on the hot path**.
>
> **Coordinate frame (2026-07-31).** A socket's physical location is its position in 3-space, millimetres, in **aircraft body axes**, origin at the centre of the shell: **+x forward** (toward the face), **+y right**, **+z down**. Right-handed. The vault sits above the origin, so every socket has z ≤ 0 and the crown is the most negative z.
>
> **⚠ The frame is settled; the numbers in it are interim.** Principal direction, 2026-07-31: the ellipsoid is a convenient approximation, useful as a *description* — not a fact. The reference of record is the measured interior scan, `cad/HelmetScan.3dm`, until exact CAD drawings exist. Coordinates emitted today still come from an ellipsoid fitted to the measured extents (267 × 229 mm, rim→crown 157 mm), and are labelled as interim in `hardware/np_socket_map.json` (`_coordinateBasis`), the generator header, and `np_zone_notify.h`.
>
> Moving onto the scan is real work, not a substitution, and two of the decisions in it are a person's rather than a script's:
> - The scan is a **raw Scaniverse capture** — 98,003 vertices, 181,944 faces, including the surroundings it was captured with, segmenting into **341 disconnected components**; the helmet is not one of them. A RANSAC fit recovers a shell at **R ≈ 157 mm**, agreeing with the documented rim→crown, but the radial histogram about that centre shows **one broad peak (r ≈ 145–175 mm) on heavy background**, not the two clean peaks an interior and an exterior surface would give. **Which surface the points are is unestablished**, and the inner face is the datum every other layer offsets from (NP-HELMET-GEOM-001 §1).
> - The scan frame is arbitrary. Rim plane and crown are recoverable, so the vertical axis and the fore-aft *line* are too — but **which end of that line is the face is not**, and getting it backwards mirrors every socket front-to-back. That is a wrong-site error, not a display one.
>
> The wire format does not change when the datum does: a socket map re-derived from the scan is the same eight bytes per socket with better numbers in them. That is the point of sending coordinates rather than an index.
> **There is no lobe and no side anywhere on this wire.** ZONE-1 (2026-07-30) deleted the lobe concept from the codebase; the 3-space change removes side for the same reason. Neither is a property of the hardware. A socket's anatomical meaning is its **zone membership**, authored by a human in `protocols/predefined/00-zones.npps` — arbitrary, overlapping, user-extensible sets of sockets. The helmet does not know about zones and must not pretend to: it reports position, and the app resolves that to whatever zones name it, via `SocketZones.generated.swift` (generated from the same zone file the web app and the structural checks read). The app names a socket by the **smallest** zone containing it, so socket 26 is "Motor / SMA" (7 sockets) rather than "Frontal Left" (20).
>
> That split also decides who caches what. The app never persists the socket map: a firmware update can re-cut the lattice (30 → 78 → 80 already) and a stale copy would place sockets wrongly with no way to notice. Zone names are the opposite — authored, versioned with the app, meaningless to the device.
>
...
>
> **⚠ Known geometric disagreement, reported every generator run.** Since 2026-07-31 the fit is measured against the scanned interior itself rather than an ellipsoid through the three measured extents, and **one** row does not fit at full 40 mm pitch: row 11 (4 wide) is compressed to **84.3%** — tiles sit 33.7 mm apart instead of 40 mm, so they overlap by 6.3 mm. Row 1, which the ellipsoid model had compressed to 96.9%, now **fits**; row 11 went the other way (86.4% → 84.3%) because the real rear rim narrows faster than the ellipsoid implied. The generator prints this with numbers on every run and hard-fails below the **0.80** floor (`RING_FIT_HARD_FLOOR` in `scripts/sync-socket-map.ts`, re-baselined from 0.85 on 2026-07-31 when the check moved onto the scan), rather than clamping — the earlier clamp piled the outermost sockets on top of each other and hid it. Row 11 at 84.3% is therefore **above** the floor: the disagreement is reported on every run but does not fail CI. Resolving it is **REG-1** (register the lattice against shell CAD), not a matter of choosing kinder constants. Flagged loudly because this project's two previous lattice failures — the 78-socket lattice no tile width could produce, and `NP_HEXMAP_MAX_SOCKETS` guessed at 64 — were both unchecked constants nothing ever contradicted out loud.
>
> (v1 carried lobe|side inline in every 4-byte status record; it was never deployed.)
>
> **Conformed to NP-HFE-002 Rev A (2026-07-31).** That document landed while this work was in flight and names it directly: §7.5 defines "the separate work item redirecting insertion-time confirmation to the companion app" and binds it to §7.2's channel and grammar. Two things had to change:
>
> - **The channel is the platform screen reader, not our own TTS.** The first implementation used `AVSpeechSynthesizer` when VoiceOver was off. §7.2 rejects in-app bespoke TTS outright, and not on style: it discards the user's speech rate, voice and verbosity, and it misses **refreshable braille display routing**, which §7.2 identifies as the deaf-blind user's only channel. `ZoneModuleAnnouncer` now posts `UIAccessibility` announcements and nothing else, plus the haptic §7.2 requires as a non-audio, non-visual acknowledgement. With VoiceOver off there is deliberately no speech — the socket list on screen is the sighted user's confirmation.
> - **The zone leads; the socket number does not.** §7.2: "raw socket numbers are never the primary form", tested by HFE-R-07. The utterance is now "Frontal Left, EEG module connected"; the id stays on screen. It is spoken only when no authored zone names the socket, where the number is the only handle there is.
>
> **Not claimed:** this is the *confirmation*, not L2 guided placement. The full landmark grammar (landmark → band → row → side → count), the exception-structured work list, corrective-movement error phrasing, and per-cluster rather than per-module confirmation granularity (§7.2, C-4) are DO-HFE-02 and are not implemented here. Per-module events are what the firmware currently emits; batching them to cluster-clamp granularity depends on MECH-2.
>
> **App.** `NeurOneGATTManager` publishes a `SocketMap` (read at link, cleared on disconnect) alongside `zoneModules`, which is now a `ZoneModuleConfiguration` — a sparse map keyed by socket id — replacing the `[UInt8] = [0,0,0,0,0]` fixed array this note flagged. It is handled in the SHDR early-exit block (socket occupancy and module health are component facts, per `np_module_map.h`), so it can never fold into a UHDR session record. A per-socket event fires only on an actual presence/fault change, and `HardwareSetupManager` speaks it via `ZoneModuleAnnouncer` during the `zoneModules` step — `AVSpeechSynthesizer`, or a `UIAccessibility` announcement when VoiceOver is running so the two do not talk over each other. `SETUP_INSTR_ZONE_MODULES` and `SETUP_ERROR_ZONE_MISSING` were rewritten; both still told the user to listen for a confirmation tone that will never play.
>
> **Verification.** `np_zone_notify_tests` (373 host checks) added to the ctest suite — `zone_announce` had no host test target at all before this, being ARM-cross-only; plus `np_zone_announce_tests` (42 checks) asserting behaviourally that insertion notifies the app and starts no audio; full firmware host suite **20/20**, all four `zone_announce` sources Cortex-M7 `-Werror` clean. App suite **359/359**, with coverage for both frame shapes, kind-bit rejection, fragment reassembly, socket-map application and label resolution, change-only event emission, and the spoken confirmation.
>
