...
- [ ] **SessionHistoryStore session timestamp migration (NP-PRIV-ANALYSIS-002 MEDIUM-07)** — `SessionRecord.completedAt` timestamps are UHDR-class data (CLAUDE.md §5.1 classifies "session timestamps → UHDR") but currently stored in `UserDefaults`, which is included in iCloud/iTunes backups and accessible via MDM. Session timestamps for neurological treatment reveal clinical patterns and constitute consumer health data under WA MHMD (RCW 70.372). Remediation: coarsen to session-day granularity for the `UserDefaults` display record, or migrate to UHDR-partition-only storage and load at display time. Required before US beta.
- [ ] **Watch bridge payload review — `SessionState.toWCMessage()` (NP-PRIV-ANALYSIS-002 MEDIUM-08)** — `PhoneSessionManager.forward(_:)` sends the full `SessionState` to the Watch on every BLE notification; `toWCMessage()` was not reviewed. If it includes raw HRV coherence floats or EEG values, they enter the watchOS WatchConnectivity queue. Required before Watch beta: verify payload contains only display-safe values; coarsen any raw biometric fields.
- [ ] **Certificate pinning for SHDR fleet endpoint (NP-PRIV-ANALYSIS-002 LOW-11)** — `SHDRUploader` uses `URLSession.shared` without certificate pinning. MITM on a clinical network could intercept SHDR device telemetry. Add `URLSessionDelegate` SPKI pinning for `fleet.neuropulse.internal` before fleet endpoint goes to production.
- [ ] **SessionProtocolSigner Keychain load query — add `kSecAttrAccessible` filter (NP-PRIV-ANALYSIS-002 LOW-12)** — load query is missing `kSecAttrAccessible: kSecAttrAccessibleWhenUnlockedThisDeviceOnly`; add filter to prevent attacker-written keys with different accessibility from matching. Low-effort, one-line fix.
- [ ] **ConsentDashboardView contact email redaction (NP-PRIV-ANALYSIS-002 LOW-10)** — research contact email/phone displayed in plain text summary row; redact to `•••@domain.com` in summary row. Full value accessible via Research Preferences screen.
...
- **Privacy analysis and repair — NP-PRIV-ANALYSIS-002 (2026-06-05, PR #112):** Second-pass code-level privacy audit of iOS app. 12 findings (1 Critical, 3 High, 5 Medium, 3 Low); 6 repaired on branch `privacy/analysis-and-repair-20260605`. Full analysis: `docs/neuropulse_priv_analysis_20260605.md`. Six repaired decisions below.
...
| Privacy Analysis and Repair (second pass) | `docs/neuropulse_priv_analysis_20260605.md` | NP-PRIV-ANALYSIS-002 Rev A (2026-06-05, PR #112) — Code-level privacy audit of iOS app source. Jurisdiction scope: US federal + state (BIPA IL, WA MHMD, CCPA/CPRA), EU/EEA GDPR, UK GDPR, Canada PIPEDA. 12 findings: 1 Critical, 3 High, 5 Medium, 3 Low. Six repaired on branch `privacy/analysis-and-repair-20260605`: (1) UHDRKeyManager PBKDF2 hardcoded password → `#error` compile gate + `.deviceOwnerAuthentication` PIN fallback; (2) SHDRUploader `identifierForVendor` → opaque Keychain warranty token; (3) SHDRUploader staging file excluded from backup; (4) UHDRBackupScheduler backup dir excluded from iCloud/iTunes + `keyFingerprint` removed from manifest; (5) AnalyticsGate `reset()` wired to `ConsentStore.revokeAnalyticsConsent()`. Six open: SessionHistoryStore timestamp migration, Watch bridge payload review, BIPA locale detection (OI-PA-03), fleet certificate pinning, dashboard email redaction, Keychain load query filter. |
