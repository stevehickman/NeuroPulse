...
| App Analytics and Crash Reporting Policy | `docs/neuropulse_app_telemetry_001.md` | NP-APP-TELEMETRY-001 Rev A (2026-06-02) — analytics vendor and crash reporter requirements; permitted event properties (9 approved types); prohibited properties (health-inferrable strings, identity fields, biometric values); SDK initialisation gate (post-consent-flow only); crash reporter configuration (all captures disabled except stack trace + metadata); custom linting rule for prohibited event property detection; annual review cadence. |
