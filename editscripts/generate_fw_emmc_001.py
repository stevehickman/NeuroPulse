...
add_normal(doc,
    ...
    'protocol, and write-endurance monitoring for the NeuroPulse Hub. It governs '
...
refs = [
    ...
    ('CLAUDE.md §5.3', 'Research data anonymization architecture: on-device, per-study, fresh per request; NeuroPulse never holds raw UHDR'),
...
partitions = [
    ...
    ('6', 'UHDR', '6,903', 'LittleFS', 'AES-256-XTS, user biometric-derived key (see §6)', 'No hardware WP (UHDR appended continuously during sessions). NeuroPulse NEVER holds the decryption key.'),
...
add_normal(doc,
    ...
    'definition.')
add_normal(doc,
    'EMMC-UHDR-01 (MANDATORY): NeuroPulse infrastructure shall NEVER hold, request, '
...
argon_params = [
    ...
    ('Associated data (ad)', 'b"NeuroPulse-UHDR-v1" (18 bytes)', 'Domain separator — prevents key reuse between firmware versions or future key types.'),
...
add_normal(doc,
    ...
    'Lockout state is stored in Config partition (SHDR key, so NeuroPulse can read if '
...
add_normal(doc,
    ...
    'be transmitted to NeuroPulse servers. The salt is device-local. If a user factory-'
...
add_normal(doc,
    'EMMC-SHDR-01 (MANDATORY): NeuroPulse may read SHDR data for warranty, predictive '
    'maintenance, and fleet analytics. SHDR is transmitted to NeuroPulse fleet database '
    'on USB-C connect (with warranty consent). The SHDR encryption key is held by '
    'NeuroPulse (§7.3). This is intentional — SHDR contains no user biology, only '
...
add_bullet(doc, '(3) Verify the target bank firmware image signature (Ed25519 signature over SHA-256 of firmware binary, signed with NeuroPulse firmware signing key). Reject unsigned or invalid images.')
...
ota_steps = [
    ...
    ('2', 'Main processor verifies Ed25519 signature over SHA-256 of firmware binary using NeuroPulse firmware public key (stored in Firmware Bank A firmware, not in bootloader — bootloader uses its own embedded copy). If signature invalid: delete OTA package from Scratch, log SHDR fault, abort.', 'Invalid signature → abort. Never flash an unverified image.'),
...
add_normal(doc,
    ...
    'device (USB DFU 1.1 specification). The NeuroPulse recovery tool (macOS, Windows, '
...
add_bullet(doc, 'USB-C local storage: a connected USB storage device (USB-A adapter via USB-C hub, or USB-C drive) receives an encrypted incremental backup archive. Backup is encrypted with the same UHDR AES-256-XTS key (K1, K2) — the backup archive is readable only with the user\'s biometric/PIN. NeuroPulse cannot decrypt a backup archive.')
add_bullet(doc, 'E2E encrypted cloud: UHDR backup archive transmitted to user-selected cloud provider (iCloud, Google Drive, or NeuroPulse-partnered encrypted backup service). Encrypted on-device before transmission. Cloud provider receives only ciphertext — they and NeuroPulse cannot decrypt. User-held key is the same biometric-derived Argon2id key.')
add_normal(doc,
...
add_bullet(doc, 'shdr_blocks_free < 10% → notification "SHDR storage low — contact NeuroPulse support". Snooze max 3×.')
add_normal(doc,
...
add_bullet(doc, '(1) NeuroPulse server delivers signed study descriptor (study ID, approved UHDR element list, anonymization parameters) via USB-C or BT/Wi-Fi while user is authenticated. App verifies cryptographic signature on descriptor.')
...
add_bullet(doc, '(4) Anonymized extract transmitted from Scratch to NeuroPulse research infrastructure. Extract is signed with device research key (separate from firmware signing key — prevents linkage) and includes study ID + extract HMAC.')
...
add_normal(doc,
    ...
    'only data transmitted to NeuroPulse research infrastructure is the post-anonymization '
...
add_normal(doc,
    ...
    'prevents the device from accepting any future study descriptors from NeuroPulse. '
...
xrefs = [
    ...
    ('CLAUDE.md §5.1', 'UHDR AES-256 biometric-derived key; NeuroPulse does not hold decryption key; SHDR separate encryption', 'ADDRESSED — §6 (UHDR key), §7 (SHDR key)'),
...
doc.save('/home/user/NeuroPulse/docs/neuropulse_fw_emmc_001.docx')
print('Done: docs/neuropulse_fw_emmc_001.docx written')
