...
(retires the module braille / tactile-dot keying features)** + **`docs/np_tool_shell_002.md` (two-bowl shell tooling; supersedes `docs/superseded/np_tool_shell_001.docx`)**
...
>
> Rev 1 was written the same day `NP-DRV-SHELL-002` Rev 1 landed and before `NP-HW-HEXTILE-001`
> existed. Those two documents, plus `NP-HW-HUB-001` Rev 3, **replace the mechanical basis of the
...
| TEMPORAL-WING-BOSS | Temporal wing anchor boss (mastoid pad mount) | 2 (bilateral) **[PROVISIONAL]** | Recessed snap-fit receiver, 34×34×5mm envelope, 30mm diameter opening (mates 40Hz mastoid LRA pad accessory). Snap release 3–6N axial. Withstands 15N axial / 10N lateral shear (FEA before first cut). Flush/recessed when pad not attached; tethered dust cover when empty. Non-labelled in Rev 1 tooling pending HOPE Phase 3 clinical result. |

> **Retired module-keying features (NP-HFE-002 Rev 1, 2026-07-31).** The five-layer RISK-15 keying
...
| QEEG21-CAP-ELECTRODE | qEEG-21 wet-gel cap electrode housing (T2) | 21 sites (reuses TILE-T1B mechanical family at higher density) | Dual-rated Ag/AgCl per §C; wet-gel vs. semi-dry is a consumable difference, not a new mechanical part. Includes FC3/FC4 (TMS targeting), Oz (photoparoxysmal detection) and A1/A2 (shared with CLIP-A1A2-CONTACT function, on VNS clips). Note HD-tDCS uses 3.5mm electrodes, which matched neither of the two conflicting tDCS electrode-area constants in the codebase. **OI-CHARGE-04 (closed 2026-09-09) removes the conflict but does not settle this drawing:** tDCS pad area is now a per-protocol authored value carried in the signed descriptor, so software no longer assumes any single pad geometry — which means a drawing cannot inherit one from the codebase either. The T1 pad geometry is a hardware decision that must be made on its own merits and then become the app's authoring default; the wire field admits 0 < A ≤ 65.535 cm². |
...
## K. L1 socket interconnect — NEW (`np_drv_shell_002`, `np_hw_hub_001` Rev 3)
...
NP-HW-HUB-001 Rev 3 §3.1/§8.3's distributed cluster-controller analysis prevails over the
...
| L-9f | T1 tDCS pad geometry is not specified by any drawing. **No longer a software divergence** — OI-CHARGE-04 (closed 2026-09-09) made the area a per-protocol authored value enforced by the safety MCU against whatever is declared, so the old 25 cm² (MCU) vs 35 cm² (app) conflict is gone. What remains is that nothing in CAD says what pad ships, and software now correctly declines to assume | QEEG21-CAP-ELECTRODE / electrode pad geometry on a drawing | — (hardware decision; OI-CHARGE-04 closed) |
...
|---|---|---|
| L-10a | **The bezel HEIGHT is decided (L-2); the face-temperature CEILING it implies is not.** `NP-THERM-BEZEL-001` Rev 1 (2026-07-21, untouched since) computes a ~45.5 °C module-face ceiling at `h_b` = 1.0 mm for a tile whose electronics sat on the *hub*. `NP-HW-HEXTILE-001` D-3 has since put a driver on **every** tile and D-4 adds a TIA + ADC, and `NP-HW-HUB-001` OI-HUB-C17c names *"continuous on-tile dissipation inside the 42 °C face / 62 °C junction envelope"* as an open gate. **Choosing 1.0 mm does not close this — it makes the ceiling more attainable (45.5 vs 44 °C) but does not verify it against dissipation the analysis never saw.** THERM-1a (CFD) and THERM-1b (bench) are the gates that close it. | TILE-SHELL face temperature; HEATSINK-ASSY sizing; the D-4 on-module-TIA decision |
...
1. **Current mechanical basis** for the helmet is the **hex-tile universal-socket architecture**
   ...
   `np_hw_hub_001.md` Rev 3): ~80 sockets, one 40mm hex module mould, orientation-only keying, full
   ...
   `np_drv_shell_001.docx` (NP-DRV-SHELL-001), and **was not reconciled** in
   `np_tool_shell_001.docx` (written against 5 zone slots), which is now superseded by
   `np_tool_shell_002.md` (2026-09-25, GitHub #331).
